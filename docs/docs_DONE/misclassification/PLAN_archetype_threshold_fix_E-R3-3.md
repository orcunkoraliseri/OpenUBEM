...
- [x] **T11** — Full Phase-E 8,160-bldg re-run (E-R3-3) — FLEET COMPLETE 2026-07-02 (12/12, 8,154/8,160); CP-3 delivered; **CP-3 ACCEPTED + T11.7 PROMOTED 2026-07-03** (`phaseE_er33` → committed `phaseE` baseline, figures + REPORT §14 addendum + explanation doc + memory updated)  ⟶ **CHECKPOINT 3 CLOSED — arc complete**
...
- **Update 2026-07-01 21:34 — ROOT CAUSE WAS A REAL CODE REGRESSION (not transient); FIXED + RELAUNCHED (autonomous):** Watcher `bgebfeqs2` fired `WATCH_PIPELINE_EXIT` — resume4 exited **rc=1**. Ground-truth from the log: NOT a transient this time. The turn-surviving driver finally survived long enough to reach the **reroute-aware pass** of `verify_and_repair` (main fleet REUSE-harvested, 226 `.end` fetched, single stubborn `way_319507579` rerouting to `one_zone_per_floor`) and crashed there: `TypeError: verify_and_repair.<locals>.<lambda>() takes 3 positional arguments but 4 were given` at `builder.py:325`. **Root cause:** commit `e063865` (simulation-resolution-mode switch) added a 4th positional arg to the strategy call — `decide_zoning_strategy(arch, footprint_area, num_floors, self.resolution_mode)` — but the reroute monkey-patch stub at `v12_cell_pipeline.py:520` was still `lambda arch, area, floors: "one_zone_per_floor"` (3 args). So the reroute path is a **guaranteed crash for ANY cell that reaches reroute** — a latent, fan-out-critical regression that the earlier three "deaths" masked (they died at the launch-lifecycle boundary *before* reaching reroute; only the turn-surviving driver exposed it). This revises the running diagnosis: "never completes" was **two stacked bugs** — (a) launch-lifecycle teardown of employee-turn children [fixed by turn-surviving launch], and (b) this reroute lambda signature regression [fixed now]. **Fix (manager-owned — delicate, fan-out-critical orchestration per the role split; 1 line, no `openubem/` feature logic touched):** `v12_cell_pipeline.py:520` → `lambda arch, area, floors, *_a, **_k: "one_zone_per_floor"`. Validated locally: `ast.parse` OK + 4-arg call returns `one_zone_per_floor`. Only line 520 needed it — the real `decide_zoning_strategy` has `resolution_mode="auto"` default, so the 3-arg standalone-repair-script callers are unaffected. **Collision cleanup:** the old OS-detached durable job (`complete_la_centre_durable.sh`, PID 28052) was still alive and wedge-polling (poll 27, 21:33) — killed it (`TaskList` empty since it was nohup-detached, so killed by PID) to prevent a re-launch collision. Cluster queue confirmed **empty** (`squeue -u o_iseri` → 0 lines; repair 1055055 drained; main-fleet 226 sims persist on `/speed-scratch` → REUSE valid). **Relaunch:** `bash scratchpad/rerun_la_centre_resume5.sh` as a **manager-owned `run_in_background` task `bpssim62x`** (turn-surviving **and** notifies on real exit — no `nohup`, so the exit signal isn't lost), plus independent stall/success watcher `b9ey9vmff` (`watch_la_centre_resume5.sh`). Confirmed healthy at launch: step2 classify done (E-R3-3 distribution), `sim_out`/`fleet_staging` cleared, in **step3 IDF regen** (2 python). Path forward: regen → REUSE (as resume4 proved — it only submitted a 1-bldg repair array, never a 226-array) → **fixed reroute** → re-sim-or-tolerance-drop the stubborn building → step5 → `final_dir`. **Fan-out is now unblocked fleet-wide** (the fix must be present for all 12 cells; any cell reaching reroute would have crashed identically). Checks 3–5 + gates report pending pilot terminal state; T11.7 promotion remains **USER-SIGN-OFF ONLY**.
- **Update 2026-07-01 23:14 — PILOT (la_centre) COMPLETE + CLEAN; 11-CELL FAN-OUT LAUNCHED (autonomous):** resume5 (`bpssim62x`) exited **rc=0** — `final_dir` `docs/validations/overAll/results/phaseE_er33/la_centre` fully populated (05_results.csv/geojson/gpkg + 04 manifest + gates report + figures), **226/226 generated, 226/226 simulated, 0 dropped** (the fixed reroute pass recovered the stubborn `way_319507579` via a 1-bldg reroute array, job 1055073). **All 5 pilot checks PASS:** (1) frozen-geometry — step1 "loading cached GDF", 226 bldgs, no OSM fetch; (2) E-R3-3 office down-tier — LargeOffice 50→37, MediumOffice 30→26, SmallOffice 24→41 (offices conserved 104↔104); (3) live_smoke PASS (unknown 15/226=6.6%<20%) + array drained; (4) output in `phaseE_er33` tree with committed baseline `docs/docs_VALIDATION/.../phaseE/la_centre` **untouched** (verified — baseline mtimes Jun 27–28, pre-dating today); (5) fresh remote dir `phaseE_er33_la_centre`. **Before/after (la_centre, report-only CBECS gates):** total EUI 130.10→129.41 (≈flat), R² 0.884→0.885 (flat), CV 60.46→60.51 (flat), NMBE −25.26→−28.55 (−3.3, more negative — expected office down-tier signature), KS 0.330→0.326 (slightly better). Incidental win: E-R3-3 run is *more* complete than the committed baseline (226/226 vs 225/226 — baseline dropped 1 to zone-mismatch; the reroute pass recovered it). **Fan-out:** all 12 work-dirs confirmed pre-seeded (gpkg byte-sizes exact-match committed baselines → no OSM fetch fleet-wide). Launched the remaining **11 cells** via manager-owned turn-surviving `run_in_background` task **`bcjz97x9w`** (`scratchpad/fanout_11_cells.sh`) — sequential (no local/cluster contention), idempotent (skips cells with existing `final_dir/05_results.csv` → resumable), continue-on-failure (one bad cell can't block the rest), ordered small→large (la_rural→…→nyc_urban), each `PYTHONUNBUFFERED=1 OPENUBEM_RECONSTRUCT_SERVICE_LOADS=0 … --output-subdir phaseE_er33`. Progress/stall watcher `b6h8c1jin` armed (`watch_fanout_11.sh`; fires on `FANOUT_ALL_DONE` or >40-min log-idle; 16h ceiling). First cell la_rural confirmed engaged (149 bldgs, step3 IDF gen). ETA ~8–12h. **On fleet-complete → T11.6 CP-3 before/after compare (manager job). T11.7 promotion remains USER-SIGN-OFF ONLY.**
- **Update 2026-07-02 22:07 — FLEET COMPLETE (12/12); T11.6 CP-3 BEFORE/AFTER COMPARE DONE (autonomous, Sonnet manager):** Fan-out driver `bcjz97x9w` exited rc=0 — `FANOUT_ALL_DONE ok=11 fail=0 skip=0` (22:06:55), all 11 cells `rc=0`. Combined with the pilot `la_centre`, **12/12 cells** have `05_results.csv`. Fleet-wide sim success: **8,154/8,160 (99.9%)** — 6 `not_simulated`: 5 in `la_rural` (matches the pre-existing, already-documented B2 OSM-vertex-winding drop, `REPORT_phaseE_final.md` §7 limitation #5 — not a new regression) + 1 new in `la_urban` (`way/402215469`, unexplained, not investigated further per "keep it numeric, flag don't diagnose" instruction — flagging for user). Ran the CP-3 compare (`scratchpad/cp3_before_after_compare.py`, read-only, mirrors `phaseE_rescore.py` loading/scoring idiom; did not touch the committed baseline or `REPORT_phaseE_final.md`):

  | City | n(before) | n(after) | median EUI before | median EUI after | measured | Δ% before | Δ% after |
  |---|---|---|---|---|---|---|---|
  ...
  | City | metric | before | after |
  |---|---|---|---|
  | NYC | NMBE% | −10.5 | −14.7 |
  | NYC | R² | 0.890 | 0.888 |
  | NYC | CV(RMSE)% | 37.8 | 38.6 |
  | NYC | KS_D | 0.2563 | 0.2221 |
  | LA | NMBE% | −21.5 | −25.5 |
  | LA | R² | 0.925 | 0.920 |
  | LA | CV(RMSE)% | 61.0 | 61.8 |
  | LA | KS_D | 0.2389 | 0.2561 |
  | Austin | NMBE% | −11.9 | −16.2 |
  | Austin | R² | 0.718 | 0.720 |
  | Austin | CV(RMSE)% | 47.5 | 49.6 |
  | Austin | KS_D | 0.3018 | 0.2730 |

  Archetype-mix shift (fleet-wide, success rows): SmallOffice 2,848→3,504 (+656), MediumOffice 948→412 (−536), LargeOffice 390→270 (−120) — office count conserved (2,848+948+390=4,186 vs 3,504+412+270=4,186), consistent with a pure down-tier redistribution, matching the CP-2-predicted direction. NMBE moved more negative in all 3 cities (−4.2/−4.0/−4.3 pp) — expected direction per CP-2 isolation (lower-intensity SmallOffice template pulls the fleet mean down further below the CBECS anchor). R² ~flat (±0.005). CV(RMSE) ~flat-to-slightly-worse (+0.8/+0.8/+2.1 pp). KS_D mixed: NYC and Austin tighter (−0.034/−0.029), LA looser (+0.017). Per V-R5-5/M-R2-4, CV(RMSE) and KS_D are report-only, not gating; NMBE and R² are the hard gates and both stayed within the same qualitative range as CP-2 predicted. **No verdict rendered — presented to user for T11.7 sign-off.**
- Test status: `py -3 scratchpad/cp3_before_after_compare.py` ran clean (pandas `FutureWarning` only, no errors); loaded 8,160/8,160 before-rows and 8,160/8,160 after-rows (8,154 success) via the `phaseE_rescore.py`-mirrored loader; `assert combined["cell"].nunique() == 12` passed both sides.
- Deviations: none from PLAN §6 T11.6 methodology. Script is new (`scratchpad/cp3_before_after_compare.py`, not under `docs/` — no `.py`-under-docs violation) and is read-only (no writes to `docs/docs_VALIDATION/…`, `REPORT_phaseE_final.md`, or any committed baseline path).
- Notes: **T11.7 (promotion) NOT executed — awaiting explicit user sign-off**, per the hard gate (§6 T11.7, RESUME doc §7). Two items flagged for the user alongside the table: (1) the 1 new `la_urban` `not_simulated` building, unexplained; (2) LA's KS_D moved the "wrong" direction (looser) though report-only. This finalizes the T11 entry (fleet complete, CP-3 delivered); the entry stays **IN PROGRESS** at the process level until the user rules on T11.7.

#### T11.6b — Manager move-away diagnosis + classifier-vs-drift decomposition — completed 2026-07-03
- Trigger: before ruling on T11.7 the user asked *why* the model moves further below measured. Manager (Opus) ran read-only diagnostics (`scratchpad/diag_er33_moveaway.py`, `scratchpad/diag_decompose.py`) matching before↔after by `osm_id` on the success set (7,504 rows both-present, non-Unknown).
- Findings: **(1) Mechanism** — 936 archetype flips, dominated by **662 MediumOffice→SmallOffice** (median EUI 179.1→107.3, −71.8). End-use decomposition of those flips: fans 40.0→9.9 (**−30.1**, VAV→PSZ system swap), equipment 44.1→27.8 (−16.3), heating 42.7→27.4 (−15.3), dhw −4.2, cooling −3.4, lighting flat — i.e. a template **discontinuity** at the 2,322 m² cut-point, not a smooth re-scale. SmallOffice share after: NYC 58.3% / LA 10.5% / Austin 74.4% (explains LA barely moving). **(2) Decomposition** (counterfactual on matched median; exact linear on mean): NYC −16.5 = classifier **−14.7 (89%)** + code-drift −1.6 (11%); LA −0.7 = 100% classifier; Austin −8.2 = 100% classifier. Mean NYC −8.70 = −7.97 (92%) + −0.73 (8%). **(3) Isolation caveat** — 205 same-archetype buildings (143 SmallOffice, median −18, one −40; `levels` unchanged) drifted from commits landed since the 2026-06-27 baseline harvest (zoning/service-load/resolution). Frozen geometry stopped OSM drift, not code drift; same-signed, never flips direction/ranking/verdict.
- Conclusion delivered to user: the move-away is a **correctness fix exposing a pre-existing DOE-SmallOffice-template gap** (not a regression), ≥89% attributable to the classifier; promotion defensible with the quantified caveat recorded.
- Test status: both diagnostic scripts ran clean (read-only, no baseline touched).

#### T11.7 — User sign-off + baseline promotion — completed 2026-07-03 — **ARC COMPLETE**
- Trigger: user ruled **"promote"** after the T11.6b diagnosis (2026-07-03).
...
- Notes: **E-R3-3 arc CLOSED.** Committed Phase-E baseline is now the E-R3-3-corrected fleet. Memory (`project_archetype_threshold_E-R3-3`, `project_phaseE_full_realism`) + `PROJECT_CHECKLIST.md` updated.

---

*OpenUBEM — manager-authored plan. Erratum E-R3-3. No code changed by this document. 2026-06-30 (CP-2 close-out appended 2026-07-01; T11 dispatch appended 2026-07-01; T11.7 promotion + arc close appended 2026-07-03).*
