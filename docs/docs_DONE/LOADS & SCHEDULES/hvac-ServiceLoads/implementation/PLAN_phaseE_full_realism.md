...
## 1. What this is — start here

**Phase-E in plain terms.** OpenUBEM estimates energy use for every building in a city by running one EnergyPlus simulation per building. Phase-E makes those simulations physically realistic: each archetype gets its proper HVAC system (with real fans + pumps), and hot water, cooking, and refrigeration are modelled *inside* the simulation instead of added by spreadsheet afterward. We then re-run the full 12-cell / ~8,160-building matrix (3 cities × 4 density rings) and score the totals against real benchmarks (CBECS nationally; LL84 / EBEWE per city). **The build work (T01–T16) is finished and proven; what remains is running the simulations and reporting.**

**Where we are:** the `la_urban` pilot passed its hard gate (CP-D2), the user approved the fan-out, and the remaining 11 cells are being simulated one at a time. Live status = the §8 progress log + `docs/RESUME_opus_manager.md`.

**Your job from here — cheap-model handoff.** From now on a **Sonnet** session runs/watches the simulations and keeps the progress log current; Opus is only needed for plan or validation *decisions*. Concretely:

1. **Run / watch a cell** with `py -3 scripts/validation/v12_cell_pipeline.py <cell> --output-subdir phaseE`. Cluster work goes through `sbatch` **only** — never run blocking compute on the Speed login node (§2). Check at most **once per 30 min**; prefer waiting for completion over polling.
2. **When a cell lands,** confirm `docs/validations/overAll/results/phaseE/<cell>/05_results.gpkg` exists and skim its `dropped_buildings.csv` — a few logged drops are normal; a cell that `exit 2`s on a *systematic* failure is **not** → stop and report it, don't suppress.
3. **Append a §8 progress-log entry** for every cell/task you finish, in the exact format §8 already uses (task id · artifacts · deviations · test status · notes). The §8 log is the binding record — update the doc, not just the chat.
4. **Do NOT start T18** (the final 3-city + national re-score + figures + report) — it is **HELD for the user.** Never `git commit`/`add` (handled externally). When a value or decision isn't spelled out, **STOP and ask** — don't invent.

Full rules → §2 · Task list → §6 · Live log → §8.
...
#### T16 — 1-cell pilot (la_urban) re-sim + re-score — completed 2026-06-27 (CP-D hard gate)
- Artifacts: `docs/validations/overAll/results/phaseE/la_urban/05_results.gpkg` (+ summary/csv/geojson/figures, 9 files), `docs/docs_DONE/LOADS & SCHEDULES/hvac-ServiceLoads/pilot/REPORT_phaseE_pilot.md` (auto), `docs/docs_DONE/LOADS & SCHEDULES/hvac-ServiceLoads/pilot/RESULT_phaseE_CPD_gonogo.md` (manager ruling). No feature-code changes; no commits (git external).
- Execution: pilot ran as a detached local process (`scripts/validation/phaseE_pilot.py`) self-driving Steps 3→5 via `v12_cell_pipeline.run_cell` with `output_subdir="phaseE"`. Shipped 618 buildings to Speed (`sbatch --array`, ABSOLUTE rule honored — no login-node compute). E+ simulated **617/618**.
- Deviation (recovery): `run_cell`'s zero-fail `sys.exit(2)` (v12_cell_pipeline.py:1025-1030) terminated the run BEFORE Step 5 because 1 building (`way/402215469`, Warehouse, zero-volume degenerate zone) was unsimulatable. Manager recovered by re-running Step 5 + S4 rescore from the 617 on-disk `.sql` files (scratchpad `phaseE_reparse.py`; `build_sim_manifest`→`step5_results`→`copy_final_deliverables`→`s4_rescore`→`s5_write_report`). No re-sim, no cluster contact. `aggregate_results` completed in 740 s; gpkg + report written.
- Test/parse status: F12 gate summary pct_parse_success 100% (617/617), EUI plausibility 99.51% (614/617, 3 outliers max 2175), zone_mismatch 0, IOD mean 0.210. S0 wiring proof PASS (LargeOffice VAV+chilled/hot-water+WaterHeater; FSR cooking gas+DHW; SuperMarket Case+Rack).
- Headline numbers: median total **104.68** kWh/m² (vs LA EBEWE 113.6 = **−7.9%**); CBECS Pacific **NMBE −3.1% PASS** (Phase-D2 was −39.3%), CV(RMSE) 58.2% FAIL, KS 0.318 FAIL, R² 0.40 FAIL (Phase-D2 R² 0.71). Per-archetype audit: pumps correct (nonzero for all central-plant archetypes, zero for packaged); refrigeration SuperMarket 115.9 PLAUSIBLE (R-CP-B-1 holds); cooking correct (FSR 212/QSR 475).
- Notes for auditor: see ruling below. Two blockers + one gate-spec fix + one watch-item before T17.

#### MANAGER RULING CP-D — NO-GO; HOLD before fan-out — 2026-06-27
- **VERDICT: NO-GO for the 12-cell fan-out (T17). HOLD at CP-D.** Full analysis in `pilot/RESULT_phaseE_CPD_gonogo.md`.
- **Gate 1 (sim-success): MET** — 617/618 (99.84%) vs Phase-D 618/618; the lone fail is a degenerate-geometry Warehouse, not autosizing.
- **Gate 2 (fans+pumps 12–16): physics PASS, gate-spec artifact** — whole-cell median 7.32 is OUTSIDE only because la_urban is 73% MidriseApartment (PTAC, no plant). Pumps are correctly nonzero for every central-plant archetype (LO 10.3 / Highrise 3.9 / Tall 8.4 / School 18.1); central-plant-subset fans+pumps median ≈ 34. Re-specify the gate per-archetype/subset.
- **Gate 3 (no blowup): MIXED → one real blowup.** Refrigeration PASS (SuperMarket 115.9/311.4). Cooking PASS. **🔴 PrimarySchool heating runaway** (760–1256 kWh/m², fans 285–444, total up to 2175 in mild LA) — reheat/simultaneous-H&C pathology in the central-VAV+HW-reheat school family; completes E+ with 0 severe so zero-fail can't catch it; systematic across schools → would corrupt the fan-out.
- **WIN:** physical service loads centered the bias (CBECS NMBE −39.3% → −3.1% PASS); median −7.9% vs EBEWE with zero fitting. R² regressed (outlier-driven; B1 fix should recover). Shape gates (CV(RMSE)/KS) remain report-only FAIL as expected.
- **Required before T17:** **B1** root-cause+fix school heating runaway (re-sim the 2 la_urban schools to confirm); **B2** make `run_cell` tolerate a small number of *logged* geometry drops (PLAN forbids silent drops, not logged); **G2** re-specify the fans+pumps gate; **W1** verify MidriseApartment(electric) vs HighriseApartment(gas) DHW fuel against DOE prototypes. After B1+B2 re-pilot la_urban, then fan out the remaining 11 cells.
- **Delegation note:** user's "auto-proceed if clean" condition NOT met (Gate-3 blowup + Gate-2 re-spec + pipeline blocker) → correctly held per "any fail → HOLD, write Go/No-Go." Awaiting user greenlight on the B1/B2/G2/W1 plan.

#### CP-D REMEDIATION DISPATCHED — 2026-06-27
- **User greenlit** (2026-06-27) the "fix B1+B2 → re-pilot la_urban → fan out" sequence (G2 + W1 folded in).
- **Remediation plan written:** `docs/docs_DONE/LOADS & SCHEDULES/hvac-ServiceLoads/remediation/PLAN_phaseE_CPD_remediation.md` (manager-authored; gates this plan's **T17 fan-out**). Tasks R-B1.1 (diagnose) / R-B1.2 (fix: SAT reset on VAV/PVAV/CRAH, dual-max secondary lever pre-authorized) / R-B1.3 (verify local, no regression) / R-B2 (tolerate ≤max(5,1%) logged drops) / R-G2 (per-archetype fans+pumps gate) / R-W1 (verify DHW fuel — expected no change) → CP-R1 audit → R-RP full la_urban re-pilot = CP-D2 hard gate.
- **Manager findings that shaped the plan (grepped):** (1) PrimarySchool setpoint schedule is sane (`doe_schedules.json:2865-2897`) + Courthouse uses the SAME PVAV-HW-reheat family and ran fine → B1 is the VAV cold-SAT reheat penalty (no SAT reset in `hvac.py` `_emit_pvav`/`_emit_buildup_vav`), not a schedule/setpoint bug. (2) W1 is NOT a bug — RESULT_03 Table 2 (`:54`) specifies Midrise=Electricity / Highrise=NaturalGas and the JSON already matches → verify-and-document. (3) B2 fix point = `v12_cell_pipeline.py:1025-1030`; G2 = `phaseE_pilot.py:32-33,288-291`.
- Executor (Sonnet) dispatched on Phase R1 (R-B1.1 → R-W1, stop at CP-R1). No cluster contact in R1 (local 1–3 IDF runs only). Re-pilot (R-RP) gated behind manager CP-R1 audit.

#### CP-D REMEDIATION COMPLETE + CP-D2 GO — 2026-06-27
- Full remediation log (R-B1.1/R-B1.2/R-B1.3/R-B2/R-G2/R-W1/CP-R1/R-B2.2/R-RP2) in `docs/docs_DONE/LOADS & SCHEDULES/hvac-ServiceLoads/remediation/PLAN_phaseE_CPD_remediation.md §8`.
- **B1 fix:** SAT reset added to all VAV/PVAV/CRAH emitters (`hvac.py`); school heating collapsed 265->9.6 / 1256->16.5 kWh/m²; regression guards GREEN (test_hvac.py).
- **B2 fix (two patches):** (1) `run_cell` B2 gate tolerates `<=max(5,1%)` logged drops; (2) `verify_and_repair` defers rather than hard-exits on exhausted repair (R-B2.2, manager-authored per user exception [[feedback_opus_writes_delicate_code]]).
- **B2 mkdir fix:** `results_dir.mkdir(parents=True, exist_ok=True)` before `to_csv` (caught by R-RP2 re-pilot; would have crashed every fan-out cell that drops a building).
- **G2 gate:** per-archetype — (a) pumps_eui>0 for all central-plant archetypes; (b) pumps_eui<1 for packaged; (c) LargeOffice fans+pumps 12-16 kWh/m².
- **W1:** DHW fuel verified Midrise=Electricity / Highrise=NaturalGas — matches RESULT_03 Table 2; no JSON change.
- **CP-D2 scored (honest, all bugs fixed):** CBECS NMBE -17.6% / R² 0.91 / CV(RMSE) 58% (report-only) / EBEWE -8.6% / G2 PASS. The CP-D "-3.1% NMBE PASS" was a bug artifact (school blowups propped the commercial mean). Residual = structural DOE-prototype-vs-CBECS office offset (R6-4B confirmed; zero-fitting principle upheld).
- **User ruling (2026-06-27): Accept + fan out.**

#### T17 — 11-cell Phase-E fan-out — completed 2026-06-27
- Fan-out launched background task `bb7vpmqix` (11 cells, `OPENUBEM_RECONSTRUCT_SERVICE_LOADS=0`, fire-and-forget). 10/11 cells exit=0 on first pass; la_rural exit=1 (Unicode bug — see below), fixed and rerun to completion (exit=0).
- **All 11 cells DONE (`05_results.gpkg` confirmed for all):**

| Cell | n_sim | n_parsed | pct_parse | drops |
|---|---|---|---|---|
| nyc_centre | 738 | 737 | 99.86% | 1 zone-mismatch (parse) |
| nyc_urban | 1779 | 1779 | 100.00% | none |
| nyc_suburban | 1589 | 1589 | 100.00% | none |
| nyc_rural | 198 | 198 | 100.00% | none |
| la_centre | 226 | 225 | 99.56% | 1 zone-mismatch (parse) |
| la_suburban | 1343 | 1343 | 100.00% | none |
| la_rural | 144 | 142 | 98.61% | 5 B2-dropped (E+ fatal geometry) + 2 zone-mismatch (parse) |
| austin_centre | 413 | 413 | 100.00% | none |
| austin_urban | 425 | 425 | 100.00% | none |
| austin_suburban | 437 | 437 | 100.00% | none |
| austin_rural | 245 | 245 | 100.00% | none |

- **la_rural incident:** `UnicodeEncodeError` on `≤`/`→` chars in the B2 drop-tolerance `print()` at v12_cell_pipeline.py:1042 (Windows cp1252 console). Simulations completed 144/149 (5 persistent E+ fatals); B2 threshold = max(5, ceil(0.01×149)) = 5; 5 ≤ 5 so within tolerance, but print crashed before Step 5 ran. Fix: replaced Unicode chars with ASCII `<=`/`->` at line 1042. Third latent B2 bug (pilot bypassed this path: mkdir crash hit first, Step 5 run via recovery script). Rerun exit=0, gpkg confirmed.
- **Deviations:** none (fix is cosmetic print-only; no logic change).
- **Artifacts:** `docs/validations/overAll/results/phaseE/<cell>/05_results.gpkg` × 12 (11 fan-out + la_urban pilot); `dropped_buildings.csv` for la_rural.
- **Test status:** all 11 gate reports pct_parse ≥ 98.61%; all gpkg present. 12-cell matrix complete.
- **Notes:** la_rural 5 dropped buildings root cause: OSM inverted vertex winding → negative zone volume (−3987 m³) → heat-balance divergence → E+ fatal (+238 °C / −280 °C). Not a pipeline bug; B2 tolerance designed for this. Full diagnosis: `REPORT_phaseE_final.md` §5.
- **T18 GO — user greenlit 2026-06-27.** `scripts/validation/phaseE_rescore.py` dispatched.

#### T18 — Final 3-city re-score + figures + report — completed 2026-06-27
- **Artifacts:** `scripts/validation/phaseE_rescore.py`; `openubem/outputs/phaseE_city_comparison.png`, `phaseE_enduse_breakdown.png`, `phaseE_cbecs_scatter.png`; `docs/docs_ACTIVE/hvac-ServiceLoads/REPORT_phaseE_final.md`.
- **Inputs:** `docs/validations/overAll/results/phaseE/<cell>/05_results.csv` × 12 (8,160 buildings total; 8,150 success, 99.9%).
- **Fleet:** nyc 4303/4304 · la 2327/2336 (la_rural 5 OSM-vertex drops) · austin 1520/1520.
- **City anchors vs measured:**
  - NYC Overall: 165.7 vs 219.2 → **−24.4%** (D2 was +2.1%); Office 147.0 vs 183.9 → −20.1%; MF 204.5 vs 226.2 → −9.6%.
  - LA Overall: 107.3 vs 113.6 → **−5.6%** (D2 was −3.7%); Office 131.8 vs 121.5 → +8.5%; MF 105.5 vs 115.8 → −8.9%.
  - Austin Overall: 120.4 vs 162.0 → **−25.7%** (D2 was −8.6%); Office 116.4 vs 162.3 → −28.3%.
- **CBECS gates:** NYC NMBE=−10.6% R²=0.895; LA NMBE=−20.5% R²=0.924; Austin NMBE=−11.9% R²=0.718. NMBE fails all 3 (hard gate ±10%). R² excellent.
- **End-use medians (city, excl. OpenUBEMUnknown):** NYC total=165.7 (heat 60.7, cool 12.2, light 26.5, equip 43.4, fans 15.0, dhw 6.3); LA total=107.3 (heat 13.9, cool 4.8, light 4.0, equip 43.4, fans 6.8, dhw 33.3); Austin total=120.4 (heat 15.3, cool 28.2, light 26.5, equip 27.8, fans 11.7, dhw 4.4).
- **Deviations:** none.
- **Test status:** script ran clean (pandas FutureWarning only — cosmetic); all 3 figures + report written.
- **CP-E finding:** Under-prediction vs measured anchors (NYC −24.4%, Austin −25.7%) is the expected consequence of retiring the reconstruction overlay. Phase-D2 was "carrying" the "Other" service-load category (elevators, process loads, ~42% of gap per R6-4B); Phase-E covers DHW/cooking/refrigeration physically but not "Other." R² 0.895–0.924 confirms shape is correct. CBECS NMBE regression (−10.6%/−20.5%/−11.9%) expected for same reason. Closing further = fitting → rejected per zero-fitted-params rule. Phase-E is the scientifically honest baseline. **ARC COMPLETE.**
