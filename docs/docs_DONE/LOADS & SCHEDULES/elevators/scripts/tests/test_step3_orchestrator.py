...
class TestStep3Orchestrator:
    ...
    def test_medium_office_idf_contains_elevator_equipment(
        self, synthetic_10_gdf, synthetic_schedule_library
    ):
        """T04: a built elevator archetype IDF must carry the Elevators ElectricEquipment.

        The 10-row synthetic fixture has no LargeOffice row; MediumOffice (row R6) is the
        nearest available archetype confirmed present in elevators_by_archetype.json (T01
        anchor: 8517.82 W) and is used here instead.
        """
        manifest, tmpdir = self._run(synthetic_10_gdf, synthetic_schedule_library)
        try:
            row = manifest[manifest["archetype_id"] == "MediumOffice"].iloc[0]
            idf = IDF(row["idf_path"])
            elev_eqs = [
                e for e in idf.idfobjects["ELECTRICEQUIPMENT"]
                if e.EndUse_Subcategory == "Elevators"
            ]
            assert len(elev_eqs) == 1
        finally:
            shutil.rmtree(tmpdir, ignore_errors=True)
