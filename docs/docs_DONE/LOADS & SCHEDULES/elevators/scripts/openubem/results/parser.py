...
def _parse_meters_sql(sql_path: Path) -> dict[str, float]:
    ...
    meters: dict[str, float] = {
        ...
        "Elevators:InteriorEquipment:Electricity": 0.0,
    }
    try:
        with sqlite3.connect(f"file:{sql_path}?mode=ro", uri=True) as conn:
...
def _compute_eui(
    ...
    # equipment_eui above. Break it out into its own end-use (10th) and subtract it
    # from equipment so the breakdown is mutually exclusive and total is invariant.
    elevators_kwh = _m("Elevators:InteriorEquipment:Electricity")
    ...
    eui["equipment_eui_kwh_m2"] -= eui["elevators_eui_kwh_m2"]  # de-fold elevators from equipment

    # D9: total = all 10 end-use EUIs summed (Phase-E whole-building site energy).
    # Value is invariant vs the pre-elevator 9-way total: elevators_eui was subtracted
    # from equipment_eui and re-added here.
