...
#### M19 — Arc paused pending externally-run literature validation — 2026-07-21 (manager)
- **Status at pause:** v1 (T01–T09) fully executed and GO on all four modes (`auto`/`building`/`floor`/`fast_zone`); see T09/CP-resim above. The only item left open is the follow-on comparison of the T08 cross-mode EUI deltas against the `literatureValidation/V01–V06` prompt results (prompts authored, not yet run/returned).
- **User decision (2026-07-21):** the user is running the V01–V06 deep-research prompts themselves externally, using Gemini Antigravity — not delegated to a Claude session. `zone` mode stays deferred as a separate later arc (§12, unchanged).
- **Next action:** none pending in this session. **WAIT** for the user to report the V01–V06 results back; then compare T08 deltas against the returned envelopes (§9 direction check), and only then close/sign the arc and file it to `docs_DONE`.
- Deviations: n/a (manager log entry only, no code/doc changes beyond this entry).

#### M20 — V01–V06 results returned (external, Gemini Antigravity); audited against real T08 data — 2026-07-21 (manager)
- **Trigger:** user ran the 6 `literatureValidation` prompts externally and reported completion; results landed in `deepResearch/literatureValidation/RESULT_V01..V06.md`.
- **Read all 6 in full.** Verdict as self-reported: all six axes "IN-ENVELOPE." However each report's own "Table 4 — OpenUBEM cross-check" was seeded with OpenUBEM figures already known at prompt-authoring time (the CP-resim per-cell `building/floor` table, §9's predicted-effects table) — i.e. **not a blind check**. Per the manager-audit convention, independently re-verified the headline claims against the raw per-building harvest (`openubem/outputs/comparisons/t08_all_modes_eui.csv`, 4,526 buildings with all 4 modes present) rather than accepting the self-reported tables.
- **Confirmed (real data matches literature):**
  - Office cohort: `LargeOffice` building/floor total-EUI ratio **0.752** (−24.8%), `MediumOffice` **0.865** (−13.5%), `TallBuilding` **0.805**, `SuperTallBuilding` **0.794** — squarely inside V06's High-Sensitivity envelope (−10% to −28%).
  - `SmallOffice` **0.941** (−5.9%) — inside V06's Moderate envelope (−4% to −15%).
  - `Warehouse` **0.980** (n=7, small sample) — inside V06's Low/Insensitive envelope (0% to −3%).
  - District-scale washout (V05): population-weighted fleet total_eui, all 4,526 buildings — `building` **−3.94%**, `floor` **+0.75%**, `fast_zone` **+2.50%** vs `auto`. Small, bounded — consistent with V05's washout claim, though not identical in sign/magnitude to the published −2.3%/+2.1% (OpenUBEM's `floor`/`fast_zone` land *above* `auto`, not below — plausible since `auto` already runs `perimeter_core` for large offices, so forcing it onto residential/warehouse too, which `auto` does not do, nets slightly positive).
- **Two genuine findings NOT surfaced by any of the 6 reports (manager-only, from independently querying the raw CSV):**
  1. **HVAC single-zone guard confounds the `building`-mode comparison.** `openubem/idf/hvac.py:660-691` ("Option-C single-zone guard," user ruling 2026-06-27, **predates and is independent of this plan**) downgrades VAV/PVAV-reheat → PSZ-AC whenever a building resolves to `len(zones)==1` (reheat-runaway prevention on degenerate single-zone geometry). `building` mode *always* forces `single_zone`, so this fires for every non-residential archetype normally on VAV. Measured consequence: `MediumOffice`/`LargeOffice`/`TallBuilding` heating is **higher** in `building` than in `floor` (+18.3%, +15.5%, +8.6%) — the **opposite direction** V02's own out-of-envelope criterion #1 ("Heating Direction Reversal") flags as a bug signal. Root cause is not a bug: the `building`↔`floor`/`fast_zone` comparison is not pure zoning-geometry (as V01/V02/V03 assume — "holding... HVAC efficiency identical") because `building` mode also swaps HVAC system family for offices. The pure geometry effect (core/perimeter cancellation) still dominates in the `fast_zone` comparison (LargeOffice building-vs-fast_zone heating **−10.0%**, correctly directioned) — it's the `floor` comparison (no core/perimeter split, HVAC-type confound uncushioned) where the sign flips.
  2. **Residential archetypes show near-zero cross-mode sensitivity**, contradicting V06's own High-Sensitivity classification for high-rise residential (published −8% to −22%). Real data: `MidriseApartment` building/floor ratio **1.000** (n=1,048, effectively no variance), `HighriseApartment` **0.971** (n=14). Cause: both archetypes are already in `_ONE_PER_FLOOR` under `auto`, `floor` mode forces the same `one_zone_per_floor` strategy, and `fast_zone`'s forced `perimeter_core` mostly falls back to `one_zone_per_floor` on narrow real apartment footprints anyway (confirmed in T06 LIVE_SMOKE). Net effect: apartments essentially never receive a genuine core/perimeter split in any v1 mode — consistent with, and reinforcing, §12.1's conclusion that Core-as-Corridor (the deferred `zone`-mode Layer 1) is where the real residential accuracy gain lives, not a defect in v1.
- **Verdict: neither finding blocks the arc.** Both are pre-existing, independently-justified architectural decisions (Option-C HVAC guard; `_ONE_PER_FLOOR` archetype routing) whose *interaction* with the new `resolution_mode` switch was not previously visible until a cross-mode, per-archetype, per-end-use query was run. Recommend documenting both as **known caveats** on `building`-mode heating interpretation (offices) and on residential zoning insensitivity (all coarse modes) — not as defects to fix in this plan.
- Artifacts: none written to `openubem/` (read-only analysis in this session + the two `Bash`/`python` queries above, not persisted as scripts).
- Deviations: n/a (manager analysis).

#### M21 — Arc CLOSED + SIGNED — 2026-07-21 (manager, user-confirmed)
- **Final status: 🔒 COMPLETE + SIGNED.** v1 scope (T01–T09) fully executed; all four modes (`auto`/`building`/`floor`/`fast_zone`) GO across the 12-cell fleet (8,160 buildings); external literature validation (V01–V06) run and cross-checked against real per-building T08 data (M20) — confirmed in-envelope with two documented, non-blocking caveats (HVAC single-zone-guard confound on `building`-mode heating for offices; near-zero cross-mode sensitivity for residential archetypes, both pre-existing/independently-justified, not defects).
- **User decision (2026-07-21):** close now, caveats documented rather than investigated further. `zone` mode (§12/§12.1) remains a separate, later, not-yet-scheduled arc.
- **Physical filing:** per standing convention, the physical move of this folder (`docs/docs_ACTIVE/simulation-Resolution/resolution_sets/`) to `docs_DONE` is left to the user (as with the imputation and elevator-loads arcs) — not performed by this session.
- Deviations: n/a.
