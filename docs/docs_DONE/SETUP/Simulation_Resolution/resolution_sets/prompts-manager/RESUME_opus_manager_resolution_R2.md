...
# RESUME — Opus MANAGER session · resolution_mode sweep (T08 → CP4) · **R2**

**Paste this whole file as the first message to a fresh Opus manager session.** It is self-contained:
read it top-to-bottom and you have the full state without re-reading the transcript.
Opened: 2026-06-30. **R2 supersedes** `RESUME_opus_manager_resolution.md` (queue advanced 48 → ~7 arrays;
Sonnet 5 now available as the executor model — use it for the cheap babysitter/harvest dispatch).
...
1. `docs/docs_ACTIVE/simulation-Resolution/PLAN_resolution_mode_switch.md` — the active PLAN. **§8 is the
   binding progress log** (latest entry = M17 drain-tracker). §7 = CP4 acceptance. §9 = expected cross-mode
   physics.
...
| **CLUSTER (Linux, 5 cells)** | nyc_centre, nyc_urban, nyc_suburban, nyc_rural, la_centre | ⏳ **RUNNING** on Speed — 20 sbatch arrays nearly drained behind the user's own job; harvest pending (M17) |

---

## 2. EXACT live state (as of the R2 hand-off moment)

- **Cluster: queue nearly drained — last observed ~7 t08 array ELEMENTS** PENDING/RUNNING (down from 48 at
  the start of monitoring: 48 → 47 → 45 → 43 → 35 → **7**). **nyc_centre was the long pole** (largest cell —
  its `fast_zone`/`floor`/`auto` legs the bottleneck) and is finally clearing. **Still 0 of 5 cells fully
  drained** at last check, so the harvest has NOT fired yet. The next tick or two should hit **0**.
- **The harvest is ALL-OR-NOTHING by design.** P4b Step 0 guard 3 requires the *entire* t08 queue to hit
  **0** before the local fetch+parse runs, producing ONE combined `t08_all_modes_eui.csv` + figures. There
  is **no per-cell incremental harvest** — you can only log queue-drain milestones until the single harvest
  fires.
- **Recurring cron `1613accb`** (schedule `13,43 * * * *`, session-only, auto-expires 7 days) is LIVE. Each
  tick it dispatches a fresh cheap executor to run P4b. When the queue empties, that executor runs the
  harvest, writes the cluster §8 entry, and PushNotifies. **Verify it's still live in the new session with
  `CronList`** — if it expired (7-day window) or the session changed, the reactive user ticks (§4) still
  drive the loop; you can re-create the cron if the user wants unattended polling.
- **Executor model: use Sonnet 5** (newly available) for the P4b dispatch — it is the cheap babysitter tier.
  Still `subagent_type "claude"`; set `model` to the Sonnet 5 id.
...
1. Dispatch ONE cheap executor (Agent tool, `subagent_type "claude"`, **`model` = Sonnet 5**) with the P4b
   prompt (the user's tick message contains it verbatim). **Do NOT do harvest work in Opus** — only dispatch
   and relay the agent's one-line queue status.
2. When it returns, relay the one-line count (e.g. "7 t08 arrays still queued/running — waiting").
...
The cron's executor only harvests the cluster half and stops. **You** assemble the combined 12-cell CP4
deliverable. Inputs:
...
## 7. First actions in the fresh session

1. `CronList` — confirm cron `1613accb` is still live (or note it expired).
2. Wait for the user's next **"P4b cluster-harvest tick"** (or the cron's notification), then run the §4
   loop with a **Sonnet 5** executor.
3. The queue was at ~7 arrays — be ready: the next tick may hit **0** and fire the harvest. The moment the
   agent reports `t08_all_modes_eui.csv` has all 5 cluster cells, `CronDelete 1613accb` and start §5 CP4
   assembly, then PING the user with the go/no-go.
