"""T19 — Full 12-cell layout_assign cluster sweep, post-structural-fixes (Steps 1-3 locally + fire-and-forget sbatch).

Minimal derivative of t18_layout_assign_full_sweep.py (structural-fixes plan §5
T11: reuse T18's proven infrastructure byte-for-byte, changing only the fleet
tag / output paths so this run's job-ids file and remote fleet dirs don't
collide with T17's or T18's already-completed ones -- same "minimal derivative"
pattern T18 itself used when it forked from t17_layout_assign_full_sweep.py).

Diffs vs. t18_layout_assign_full_sweep.py (all documented here, per the same
convention T18 used for its own diff-vs-t17 docstring):
  1. _FLEET_TAG "t18" -> "t19" (remote dir / job-name prefix only; the sbatch
     ...
     task's remote fleet dirs with T17's/T18's already-harvested ones; it does
     not change array sizing/node config/packaging mechanics in any way).
  2. work_base tmp dir "ubem_t18_sweep" -> "ubem_t19_sweep" (own job-ids file,
     does not collide with/overwrite T17's or T18's own sweep state).
  3. job_id_path filename "t18_job_ids.json" -> "t19_job_ids.json".
  4. CELL_CONFIGS, ALL_MODES (single-element ["layout_assign"]), _verify_orient_gate(),
     ...
     to T18 -- this run applies the structural-fixes plan's T01/T03/T06/T08 fixes
     (Daylighting:ReferencePoint scaling, thermal_mass=True default for
     layout_assign, DataCenter WSHP autosize-to-scaled-literal, EpBunch.objls
     padding), which are already live in the imported openubem package -- no
     script-level change needed to pick them up.

This run is against the SAME 12 cells / SAME 8,160-building fleet as T17/T18,
with this plan's 3 structural fixes applied on top of the debug plan's own
fixes (all CP-D-signed as of 2026-07-24); E-LA-15/16/17/18/19 (this plan's own
newly-surfaced secondary findings) and E-LA-06/E-LA-14 (both plans'
out-of-scope residuals) remain OPEN exactly as at T10's local-regression gate
-- this sweep does not change layout_assigner.py/builder.py/envelope_patcher.py,
it only re-runs the pipeline with the already-committed code from CP-D.
...
Then harvest:
    py -3 scripts/cluster/t19_harvest_layout_assign.py

Usage (local machine only):
    py -3 scripts/cluster/t19_layout_assign_full_sweep.py
    py -3 scripts/cluster/t19_layout_assign_full_sweep.py --cells la_rural   (subset for testing)
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="T19 full layout_assign x 12-cell sweep (post-structural-fixes)")
    ...
    work_base = Path(tempfile.gettempdir()) / "ubem_t19_sweep"
    work_base.mkdir(parents=True, exist_ok=True)

    print("=" * 72)
    print(f"T19 layout_assign full sweep (post-structural-fixes) — {len(cells)} cells x {len(modes)} mode(s)")
    ...
    job_id_path = work_base / "t19_job_ids.json"
    ...
    print("\n" + "=" * 72)
    print(f"T19 FIRE-AND-FORGET SUBMISSION COMPLETE ({elapsed/60:.1f} min total)")
    print(f"  Work dir:     {work_base}")
    ...
    print("  py -3 scripts/cluster/t19_harvest_layout_assign.py")
    print("=" * 72)
