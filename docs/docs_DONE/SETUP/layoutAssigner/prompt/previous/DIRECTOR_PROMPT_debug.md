...
2. `docs/docs_DONE/SETUP/layoutAssigner/DONE/DONE-implementation_plan.md` §9 (Error Log) — read the full `E-LA-06`/`E-LA-07`/`E-LA-08`/`E-LA-09`/`E-LA-10` entries in the CLOSED arc's own words; these are the root-cause investigations this plan's fixes are built on. **Do not re-derive these facts — they are already measured and cited by line number in the debug plan's §4.**
