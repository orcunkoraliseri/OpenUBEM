...
- Update `docs/docs_DONE/SETUP/layoutAssigner/figures/OpenUBEM_results_LayoutAssigner.md` with the new post-fix numbers (append a new `## 6.` section — do not rewrite `## 3.`'s original T17 table or `## 5.`'s T18 table, both stay as historical record).
