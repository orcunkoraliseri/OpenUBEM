...
## Progress — simple checklist (read this first)

> Plain-language state of the arc. One line per phase. Full detail lives in §0 below.
> **Last updated:** 2026-07-13.

- [x] **Phase A — statistical imputer + provenance** — DONE; gate CP-1 passed (75/75 tests + 25/25 IDFs byte-identical). Results: `results/phase_A/RESULTS_phaseA.md`.
- [x] **Phase B — "OpenUBEM AI" router + validation harness** — DONE; gate CP-2 passed on real cities. Results: `results/phase_B/RESULTS_phaseB.md`.
- [✗] **Phase C — classical-ML imputer (6 methods)** — BUILT and tested, but CLOSED **built-but-off**: it failed the ship gate (EUI bias −5.51%), so the user kept it built-but-off and parked it. Results: `results/phase_C/RESULTS_phaseC.md`.
- [x] **Phase D — external-data fusion (Overture / LiDAR / assessor)** — **🔒 CLOSED + SIGNED (2026-07-13).** T12.1–T12.6 + CP-4a + CP-4 (real Overture LIVE_SMOKE, release `2026-06-17.0`, 1,667 buildings via anonymous DuckDB/S3; `height_m` 87.6% ground-truth fill) + T12-ship all done, manager-audited + self-signed (user-delegated ship decision). `fusion` is now an enabled-by-default imputation tier, byte-identical for any run without a configured source (two byte-identity tests), gate suite **171/0**. `enrich_semantics` reroute + EUBUCCO/GHSL/imagery + real per-municipality LiDAR/assessor config deferred to future arcs (see §9 backlog). Details: `docs_Done/PLAN_phaseD_fusion.md` §8.
- [x] **Phase E — advanced data-driven (GAN / GNN / LLM / TabPFN)** — **DOCUMENTED-DEFERRED (T13 delivered 2026-07-13).** Deep-generative / GNN / LLM ruled **out of core scope with evidence** (M05/M06/M10); TabPFN ruled **NOT READY** → opt-in isolated experimental track only; **none enter the default pipeline.** Ruling: `results/phase_E/RESULTS_phaseE.md`.

> **Where we are now:** Phase A/B shipped; Phase C closed (ML built-but-off); **Phase D CLOSED + SIGNED (2026-07-13)** — `fusion` external-data-fusion tier shipped enabled-by-default (byte-identical without a configured source; real NYC-centre `height_m` 87.6% fill when Overture is configured); **Phase E DOCUMENTED-DEFERRED (2026-07-13)** — advanced frontier ruled out with evidence, TabPFN experimental-only. **The input-imputation arc's scoped work (Phase A–D executed + Phase E documented) is COMPLETE — ready to file to `docs_DONE`.** Every phase now has a self-contained `results/phase_{A,B,C,D,E}/` doc. **All open / deferred / out-of-scope items are tracked in one place — §9 backlog (nothing there blocks closure).**

---

## 0. Status at a glance — monitoring checklist

> Single-glance tracker for this arc. Legend: `[x]` done/greenlit · `[~]` in progress · `[ ]` not started · `[!]` blocked/needs decision · `[—]` deferred / out of scope (deliberately not built) · `[✗]` evaluated & CLOSED not-met (decision taken, does not ship). Detail for every line lives in §6 (tasks), §7 (checkpoints), and §8 (progress log).
>
> **Last updated:** 2026-07-13 — **🔒 ARC COMPLETE.** All scoped phases resolved: **Phase A** COMPLETE (CP-1 MET), **Phase B** COMPLETE (CP-2 fully MET, real-city cluster-confirmed), **Phase C** CLOSED built-but-off (CP-3 not fully met — attribute leg marginal, EUI do-no-harm FAILS −5.51%, user-accepted), **Phase D** CLOSED + SIGNED (CP-4 MET on a real Overture LIVE_SMOKE, `fusion` shipped enabled-by-default byte-identical without a configured source, gate 171/0), **Phase E** DOCUMENTED-DEFERRED (advanced frontier ruled out with evidence, TabPFN experimental-only, none ship). Only **T10** (optional `--replicates` uncertainty mode) is deliberately un-built — never a gate condition, out of arc scope. Every phase has a self-contained `results/phase_{A,B,C,D,E}/` doc. · **Current position:** **arc scoped work (A–D executed + E documented) COMPLETE + fully self-documented — ready to file to `docs_DONE`** (physical relocation deferred to the user given the prior docs_DONE/docs_ACTIVE churn). Completed sub-plans filed to `docs_Done/`; all open/deferred items tracked in **§9 backlog**.
...
**Phase B — "OpenUBEM AI" subsystem + validation harness (user tier 2) → CP-2 (LIVE_SMOKE; gates all data-driven work)**  ✅ **COMPLETE** *(CP-2 fully MET, real-city cluster-confirmed)*
...
- [—] **T10** — optional uncertainty mode `--replicates M` (`config.py`, orchestrator) — **DELIBERATELY NOT BUILT / OUT OF ARC SCOPE.** Never a CP-2 (or any) gate condition; an optional convenience feature only. Does not block arc completion; available as a future enhancement.
...
- [x] **T09-CC** — real-OSM-city cluster A/B (CP-2 CONFIRMATORY gate) — **DONE 2026-07-03: CP-2 CONFIRMED fully MET** (nyc_centre N=32 NMBE +0.49% / CV(RMSE) 1.71%; la_urban N=124 NMBE +0.08% / CV(RMSE) 0.61% — both PASS). **USER 2026-07-02: queued this FIRST (before Phase C planning).** Feasibility-first: Phase 1 = inventory observed `year_built`/`levels` coverage of cluster-side real cities (scp down + inspect locally; NO login-node python), report + STOP; manager picks target set (levels-only if year_built too sparse); Phase 2 = build+submit sbatch A/B. **SUBMITTED 2026-07-02:** target PINNED = `year_built` (real OSM carries it well; levels too sparse); cells = nyc_centre GATE (real NYC 725053 EPW, N=32, jobs 1058656/1058657) + la_urban robustness (real LA EPW, N=124, jobs 1058653/1058654). Phase-2a wiring smoke PASSED; manager caught+fixed a Chicago-placeholder-EPW defect on the gate. All 4 PD behind T11 (untouched) behind another project's CPU-cap array; low-freq Sonnet monitor harvesting per-cell held-out-only NMBE/CV(RMSE) on completion. Never touch T11.

**Phase C — classical-ML imputer (user tier 3) → CP-3**  ⏸ **CLOSED — built-but-off** *(CP-3 not fully met, user-accepted 2026-07-03)*
- [x] **T11** — `build_ml_imputer` / §3E ML tier BUILT + tested (`imputation.py`). **Detailed execution plan: `docs_Done/PLAN_phaseC_ml_imputer.md`** (decomposes T11 into T11.1–T11.7 + CP-3a/CP-3; user 2026-07-03 scope = full M04 six-method menu behind one registry, EUI check = local IDF field-diff primary / cluster on divergence). **T11.1–T11.5 DONE + CP-3a MET** (6-method registry `missforest`/`mice`/`knn`/`rf`/`histgbm`/`linear` + per-target floors + `ML_<METHOD>_<TIER>` tokens + opt-in config, `test_ml_imputer.py` 40/40). **T11.7 production wiring stays USER-SIGN-OFF-only and is NOT pursued** (tier kept off). Results: `results/phase_C/RESULTS_phaseC.md`.
- [✗] **CP-3** — **CLOSED NOT-FULLY-MET 2026-07-03 (user-accepted, tier kept built-but-off).** Two-leg gate: attribute-recovery leg MET but marginal (`knn` beats Phase-A on `year_built`/`levels`); **EUI do-no-harm leg FAILS** — real cluster A/B NMBE **−5.51%** (all 167/167 buildings shift down, systematic bias) vs the ±5% gate. Ships only if BOTH legs pass → does not ship. `ml` stays OUT of default `IMPUTE_ENABLED_TIERS` (opt-in per-target only); CP-1 byte-identity intact.

**Phase D — fusion-first external joins (research-driven) → CP-4**  🔒 *CLOSED + SIGNED 2026-07-13 (CP-4a MET · CP-4 LIVE_SMOKE MET · T12-ship shipped, gate 171/0)*
- [x] **T12** — external-data fusion precedence layer (Overture/LiDAR/assessor, runtime-fetch) — **DONE 2026-07-13.** **Detailed execution plan: `docs_Done/PLAN_phaseD_fusion.md`** (decomposes T12 into T12.1–T12.6 + CP-4a/CP-4 + T12-ship; user 2026-07-13 set scope = **Overture-first + LiDAR/assessor config-path adapters** in one registry, EUBUCCO/GHSL/imagery documented-deferred; **CP-4 = join-correctness + fill-rate + license/bundle guard**, no cluster/EUI leg). **T12.1–T12.5 DONE + CP-4a MET** (`_fusion_tier` filled in, suite 170/0, no-fusion path byte-identical, license guard bites; `FUSED_<SOURCE>_<TIER>` tokens ratified into §5G above). **T12.6 CP-4 LIVE_SMOKE MET** (real Overture query — see CP-4 line below). Enabled-by-default after T12-ship, byte-identical without a configured source.
- [x] **CP-4** — LIVE_SMOKE join against a real external slice + license/bundle guard — **MET 2026-07-13** (Sonnet #2: real Overture release `2026-06-17.0`, 1,667 buildings via anonymous DuckDB/S3, ~70s; `height_m` fusion closed 87.6% of real gaps with ground truth, all `FUSED_OVERTURE_HIGH`; misses fell through to hotdeck/groupmode correctly; license guard green on a real 279 KB CDLA-Permissive slice; surfaced + fixed a real `year_built`-absent-from-Overture schema bug). Manager audited (code read, no Opus pytest re-run).
- [x] **T12-ship** — **DONE + SIGNED 2026-07-13** (user-delegated decision). `config.IMPUTE_ENABLED_TIERS` → `("fusion","spatial","statistical")`; byte-identical for any run without a configured source (two byte-identity tests: the routing test + the repurposed `test_fusion` guard test); gate suite **171 passed / 0 failed**. `enrich_semantics` reroute kept OUT of scope (future arc). Manager-audited (code read, no Opus pytest re-run).

**Phase E — advanced / data-driven frontier (user tier 4)**  📄 *documented-deferred*
- [x] **T13** — **DONE 2026-07-13** — frontier ruling authored at `results/phase_E/RESULTS_phaseE.md`: four-filter test (zero-fitted-params · reproducible/offline · provenance · viable at our cell scale); deep-generative **SKIP**, GNN **REJECT**, LLM **FIRM DISQUALIFICATION**, TabPFN **NOT READY** → opt-in isolated experimental track (MAR geometry/semantics only, never HVAC/U-values, same CP-3-style gate before any promotion). None enter the default pipeline.
...
| `FUSED_<SOURCE>_<TIER>` — literal `FUSED_OVERTURE_HIGH`, `FUSED_LIDAR_HIGH`, `FUSED_LIDAR_MED`, `FUSED_ASSESSOR_HIGH` (grammar reserves `FUSED_OVERTURE_MED`/`FUSED_ASSESSOR_MED` — no derived path in Phase-D scope, never emitted today) | `semantic/fusion.py::fuse` → `imputation.py::_fusion_tier` (Phase D, opt-in) | `provenance_<attr>` + `append_flag` | HIGH (direct external field join) / MED (value *derived from* a joined field, e.g. `levels = max(1, floor(LiDAR_height/FLOOR_TO_FLOOR_M))`) — **LOW never stamped** (a join hits→HIGH/MED or misses→null→fall through) | fusion tier enabled for `attr` **and** a source (`FUSION_OVERTURE_SLICE_PATH`/`_ENDPOINT`, `FUSION_LIDAR_NDSM_PATH`, `FUSION_ASSESSOR_PATH`) is configured + available: authoritative external join on `height`/`levels`/`year_built`/`use_class` |

**⚠️ Phase-C token ratification (manager, 2026-07-03 — CP-3a audit):** the `ML_<METHOD>_<TIER>` row above is the **manager-ratified** T11.2 vocabulary (executor recorded the exact strings + cut-points in `docs_Done/PLAN_phaseC_ml_imputer.md` §8 T11.2 for approval; added here per that plan's §5-F protocol). **Cut-points (fixed, cited, never swept):** classifier — top-class `predict_proba` share ≥0.8→HIGH, ≥0.5→MED, else LOW; regressor — `score=1/(1+cv)`, `cv=|dispersion/prediction|`, same ≥0.8/≥0.5 thresholds — **identical formula/thresholds to `spatial_impute._confidence_tier` (T06)**, reused not invented. Audit caveat carried to CP-3: for `year_built` the `cv=std/|mean|` score is near-insensitive at a ~2000 baseline (empirically all-HIGH — an inherited `spatial_impute` property), so LOW-discard effectively never fires for `year_built` and the ML fill always wins over statistical when enabled+above-floor; the CP-3 leaderboard (not the confidence gate) is therefore the operative discriminator.

| `DEBIAS_NEWERSKEW_QMAP` / `DEBIAS_NEWERSKEW_QMAP_GLOBAL` / `DEBIAS_SKIPPED_THINSTRATUM` | `imputation.py::_ml_tier` (Phase C, T11.8/T11.8b, opt-in) | appended to `data_quality_flag` **alongside** the row's `ML_KNN_<TIER>` token (never replaces it) | *(flag, no independent tier — rides on the ML fill's own HIGH/MED)* | de-bias enabled (`config.IMPUTE_DEBIAS_NEWERSKEW.get(attr)`) + method `knn`: `_QMAP` = stratified qmap applied; `_QMAP_GLOBAL` = unstratified/global qmap applied (no stratifier column present, T11.8b); `_SKIPPED_THINSTRATUM` = correction declined, stratum `<min_donors`, raw ML fill kept |

**⚠️ Phase-C de-bias token ratification (manager, 2026-07-14 — CP-3b-local audit):** the `DEBIAS_NEWERSKEW_*` row above is **manager-ratified** (executor recorded the exact strings in `docs_Done/PLAN_phaseC_ml_imputer.md` §8 T11.8 for approval; added here per the same §5-F protocol). These are **flags, not impute tokens** — a de-biased fill still carries its own `ML_KNN_<TIER>` confidence token (the qmap corrects the *value*, not the provenance grade); the flag only records that quantile-mapping ran (and in which regime). No `provenance.py`/`parse_token` change (flags ride on `data_quality_flag`, like `SUSPECT_ZERO_*`). Opt-in (`config.IMPUTE_DEBIAS_NEWERSKEW` all-False default) → byte-identical for every run that does not set it, incl. opt-in-`ml` runs.

**⚠️ Phase-D token ratification (manager, 2026-07-13 — CP-4a audit):** the `FUSED_<SOURCE>_<TIER>` row
above is the **manager-ratified** T12.4 vocabulary (executor recorded the exact strings in
`docs_Done/PLAN_phaseD_fusion.md` §8 T12.4 for approval; added here per the same §5-F protocol). **Cut-point (fixed,
cited, never swept):** a **direct external field join → HIGH**; a value **computed from** a joined field
(only `levels`-from-LiDAR-`height` today) **→ MED**; a **miss → null → fall through** to spatial/ml/
statistical (fusion never emits `_LOW`), structurally enforced in `fuse()` (two possible suffixes, gated
by each source's `derived: bool` return). Tokens parse cleanly under the existing
`provenance.parse_token` grammar (`FUSED_LIDAR_MED` → `("FUSED","LIDAR","MEDIUM")`) and
`provenance.CONFIDENCE_WEIGHT` already grades method="FUSED"≠"OBSERVED" as imputed-by-tier — no
`provenance.py` change needed. Fusion is opt-in (`fusion ∉ config.IMPUTE_ENABLED_TIERS` at CP-4a) and a
no-op unless a source path/endpoint is configured, so promoting it is byte-identical for an
unconfigured run (the T12-ship decision, see §8 2026-07-13).
...
> **📄 Detailed execution plan: `docs_Done/PLAN_phaseC_ml_imputer.md`** (manager-authored 2026-07-03). It
...
> executor works from `docs_Done/PLAN_phaseC_ml_imputer.md`.
...
- Artifacts: NEW `docs/docs_DONE/INPUTS/imputation/docs_Done/PLAN_phaseC_ml_imputer.md`; this file — §0 Phase-C block + §6 T11 header now point to it.
...
#### Manager — Phase-A results reproduction + consolidation (Sonnet-executed) — 2026-07-11
- Trigger: user asked to "execute Phase A with a Sonnet employee" and drive it to completion autonomously.
- What ran: a Sonnet executor re-ran the five Phase-A gate suites locally against the repo `.venv`
  (`test_tierB_provenance`, `test_vintage_donor`, `test_levels_groupwise`, `test_spatial_impute`, `test_provenance`).
- Result: **76 passed / 0 failed / 0 errored** (`76 passed in 7.96s`, pytest exit 0). Raw log saved as
  `results/phaseA_gate_tests_output.txt`.
- Reconciliation: CP-1 recorded **75**; reproduction shows **76**. Delta = `test_provenance` grew 20 → 21 tests
  since CP-1 (one added test, no regressions, all green). Benign; CP-1 verdict unchanged.
- Docs updated: `results/RESULTS_phaseA.md` narrative + tables corrected 75 → 76 with the 2026-07-11 reproduction
  note; the simple checklist at the top of this plan already links `results/RESULTS_phaseA.md`.
- Deviations: none — verification/documentation only; no source code, no git, no cluster touched.
- Notes: IDF byte-identical half (25/25 vs `e063865`) was NOT re-run — it needs an old-commit checkout + IDF
  regen; the CP-1 record stands. If the user wants that leg reproduced too, dispatch a separate Sonnet job.

#### Manager — Phase-D plan authored + arc kept ACTIVE (relocation reverted) — 2026-07-13
- Artifacts: NEW `PLAN_phaseD_fusion.md` (T12 decomposed into T12.1–T12.7 + CP-4a/CP-4 + T12-ship user-sign-off gate, symmetric to `docs_Done/PLAN_phaseC_ml_imputer.md`); this plan's §0 "Progress" checklist + §0 tracker Phase-D line updated (T12 → PLANNED, pointer to the new doc); `PROJECT_CHECKLIST.md` reconciled — imputation arc **kept in `docs_ACTIVE/input/imputation/`** (the 2026-07-13 premature move to `docs_DONE/` reverted via PowerShell `Move-Item` after this plan's own "Progress" checklist surfaced Phase D as NOT-STARTED-but-scoped + Phase E deferred).
- Deviations: none — planning + doc-location reconciliation only; NO code, NO cluster, NO git. Arc is NOT complete (Phase D is real remaining scoped work).
- Test status: n/a (plan doc).
- Notes: Scope set by user 2026-07-13 (AskUserQuestion): (1) **Overture-first + LiDAR/assessor config-path adapters** in one pluggable source registry; EUBUCCO/GHSL/street-view imagery **documented-deferred** (registry leaves room to add them). (2) **CP-4 gate = join-correctness + fill-rate + license/bundle guard** — one manually-run real Overture LIVE_SMOKE against a committed slice, **no cluster / no EnergyPlus EUI leg** (fused values are authoritative ground truth, not an EUI hypothesis). Load-bearing pins in the new doc: fusion is already the FIRST slot in `_CANONICAL_TIER_ORDER` and `_fusion_tier` is a live skeleton stub (`imputation.py:616-620`, raises `NotImplementedError("fusion tier is Phase D")`) — T12 only fills it in, no tier-order change; zero-fitted-params holds by construction (a join = observation) but is still test-guarded against EUI-column reads; direct join → HIGH / derived value (levels-from-LiDAR-height) → MED / never LOW (miss → null → fall through to spatial/ml/statistical); NO live network in the automated suite (mock + committed real slice), the ONE live query is the CP-4 LIVE_SMOKE run manually once (satisfies both the no-live-network project rule AND the synthetic≠live blind-spot memory); new deps `duckdb`+`rasterio` only, nothing large/restricted bundled (T12.5 license guard, allowlisting the existing legit `openubem/data/` reference tables); US-first with honest ex-US degradation (M07 §4 — imputation always carries a join miss). Opt-in only; `enrich_semantics`/default run untouched until the T12-ship user-sign-off. **Next: dispatch a fresh Sonnet executor on T12.1→CP-4a with the standard kickoff.**

#### Manager — Phase-D CP-4a MET + T12.6 dispatched; user delegated the ship decision — 2026-07-13
- Artifacts: NEW `openubem/semantic/fusion.py`, `openubem/acquisition/overture_fetcher.py`, rewritten `imputation.py::_fusion_tier`, `config.py` `FUSION_*` block (fusion NOT in `IMPUTE_ENABLED_TIERS`), `tests/test_fusion.py` + `tests/test_fusion_license_guard.py`, fixtures under `openubem/data/fixtures/fusion/` + `LICENSES.md`; `pyproject.toml` (`duckdb>=1.5,<2`, `rasterio>=1.5,<2`). Full detail + per-task entries in `PLAN_phaseD_fusion.md` §8 (T12.1–T12.5 + the manager CP-4a audit). `FUSED_<SOURCE>_<TIER>` tokens ratified into §5G above.
- Deviations: (a) `tests/test_imputation_routing.py` one stub-pinning test rewritten (fusion stub → real tier, exactly the Phase-C T11.3 precedent) — ratified; (b) DuckDB confined to the live `_fetch_live` branch (offline uses `gpd.read_parquet`) so the automated suite is robustly network-free — the live DuckDB path is UNEXERCISED until T12.6 (carry-forward to CP-4); (c) synthetic fixtures EPSG:32618 + `osm_to_use_class.json` crosswalk reuse — real EPSG:4326 + real-Overture crosswalk hit-rate deferred to the T12.6 LIVE_SMOKE. All flagged, none a scope violation.
- Test status: manager re-ran the CP-4a gate suite (`test_fusion + test_fusion_license_guard + test_imputation_routing + test_mask_recover + test_imputation + the CP-1 gate suite`) → **170 passed / 0 failed**. Manager read `_fusion_tier`/`fuse`/the rewritten routing test directly (not trusted).
- Notes: **CP-4a MET + self-signed** (autonomous-completion momentum; user asleep). `_fusion_tier` obeys the `(value, token)` contract, no other tier touched; `fuse()` first-hit-wins, HIGH/MED/never-LOW; no-EUI structural guard in every adapter + `fuse()`; no-fusion path byte-identical (spy + `assert_frame_equal`). **T12.6 (CP-4 LIVE_SMOKE — real Overture query, join/fill-rate/license) DISPATCHED to a fresh Sonnet.** **USER DELEGATION (2026-07-13, mid-turn message):** *"pas besoin [d'attendre pour T12-ship], si possible tu decides tu progress tu finis phase d"* → the T12-ship default-run ship decision is now the manager's to make on the CP-4 numbers (no user-sign-off owed). Manager stance to apply after CP-4: ship fusion as an enabled tier in the `impute_missing` router (byte-identical for any unconfigured run, since fusion is a no-op without a configured source), and **keep the `enrich_semantics` production reroute OUT of scope** (the parent §5D-quarantined, CP-1-byte-identity-breaking change — a separate future arc, not Phase D). Final decision recorded after the CP-4 audit.

#### Manager — Phase-D CP-4 MET + T12-ship DECISION taken + dispatched — 2026-07-13
- Artifacts: audit of Sonnet #2's T12.6 LIVE_SMOKE (see `PLAN_phaseD_fusion.md` §8 "T12.6" + "CP-4 REACHED" + the manager "CP-4 AUDIT + T12-ship DECISION" entry). Code fix audited: `openubem/acquisition/overture_fetcher.py::_fetch_live` (`year_built` dropped — absent from the real Overture Buildings schema; `ST_AsWKB` + `SET s3_region`). New real fixture `openubem/data/fixtures/fusion/overture_nyc_centre_slice.parquet` (279 KB, CDLA-Permissive-2.0). T12-ship dispatched to a fresh Sonnet (`config.IMPUTE_ENABLED_TIERS` one-liner + `_fusion_tier`/`impute_missing` docstring refresh + byte-identity regression test + gate-suite re-run).
- Deviations: none in the ship decision. The `year_built`-absent fix ratified (a genuine synthetic≠live blind-spot catch, not scope creep). Carry-forward items #3 (live DuckDB leg) and #4 (real crosswalk hit-rate) both CLOSED by the LIVE_SMOKE.
- Test status: no Opus pytest re-run (per the user's token-efficiency instruction — audit = code read + §8 report). CP-4 gate suite reported 170/0 by Sonnet #2; T12-ship's gate re-run to be reported by Sonnet #3.
- Notes: **CP-4 MET.** Real Overture release `2026-06-17.0`, 1,667 buildings via anonymous DuckDB/S3 (~70s), `height_m` fusion 87.6% ground-truth fill, `levels`/`use_class` ~0% (Overture coverage overlaps already-mapped rows in dense Manhattan — a real coverage property, handled by fall-through), `year_built` structurally 0% from Overture (assessor is its only source), `use_class` crosswalk 73.6% mapped / 26.4% unmapped (6 values). **T12-ship DECISION (user-delegated):** enable `fusion` in `config.IMPUTE_ENABLED_TIERS` → `("fusion","spatial","statistical")` — byte-identical for any unconfigured run (structurally guaranteed: `precedence_for` drops unavailable sources → `fuse` all-null → `_fusion_tier` no-op; duckdb/rasterio imported lazily inside `join()`, never reached at empty config). `enrich_semantics` reroute stays OUT of scope. The 6 unmapped `use_class` values = tracked follow-up, not a ship blocker. **T12-ship DISPATCHED to a fresh Sonnet #3.**

#### Manager — T12-ship SIGNED + 🔒 PHASE D CLOSED — 2026-07-13
- Artifacts: `openubem/config.py` (`IMPUTE_ENABLED_TIERS = ("fusion","spatial","statistical")`), `openubem/semantic/imputation.py` (`_fusion_tier` docstring), `tests/test_imputation_routing.py` (byte-identity test + rename), `tests/test_fusion.py` (pre-ship guard test repurposed → `test_default_impute_missing_calls_fusion_tier_as_byte_identical_noop`). Full detail in `PLAN_phaseD_fusion.md` §8 (T12-ship entry + the manager "T12-ship AUDIT + SIGN-OFF + PHASE D CLOSED" entry).
- Deviations: one pre-ship guard test in `tests/test_fusion.py` (asserting fusion is never called by default / not in the tuple) was repurposed to assert the post-ship byte-identical-no-op contract — the ship deliberately retires that guard; Phase-C T11.3 precedent. Executor flow: Sonnet #3 correctly HARD-STOPPED at the failing pre-ship test rather than patching outside its touch-list; manager made the repurpose decision; fresh Sonnet #4 applied it. No tier logic changed.
- Test status: gate suite **171 passed / 0 failed** (reported by Sonnet #4; manager audited by reading the config change + the rewritten test + the §8 entry, no Opus pytest re-run per the token-efficiency instruction).
- Notes: **T12-ship SIGNED** (user-delegated; no user-sign-off owed) and **PHASE D CLOSED.** `fusion` is now an enabled-by-default imputation tier, byte-identical for any run without a configured source (structurally guaranteed: unavailable sources dropped by `precedence_for` → `fuse` all-null → `_fusion_tier` no-op; duckdb/rasterio imported lazily inside `join()`), and delivers real ground-truth fill when a source is configured (NYC-centre `height_m` 87.6%). Zero-fitted-params preserved. **The input-imputation arc's scoped work (Phase A–D) is COMPLETE — ready to file to `docs_DONE`** (physical relocation left for the user given the prior docs_DONE/docs_ACTIVE churn — the arc is marked CLOSED in place). Deferred to future arcs: `enrich_semantics` reroute, EUBUCCO/GHSL/imagery sources, the 6 unmapped `use_class` crosswalk values, real per-municipality LiDAR/assessor config + a cluster EUI leg. Phase E stays documented-deferred / out of scope.

#### Manager — Phase-E T13 frontier ruling authored + `results/phase_D` & `results/phase_E` docs added — 2026-07-13
- Artifacts: `results/phase_E/RESULTS_phaseE.md` (manager-authored T13 frontier ruling); `results/phase_D/RESULTS_phaseD.md` + `phaseD_{cp4_verdict,fillrate,architecture,byte_identity}.png` (fresh-Sonnet-executed from a manager verified-fact sheet, mirroring the phase_C template); `results/README.md` (five-phase table + Phase-D/E narrative sections + folder map). §0 checklist: Phase E → `[x]` DOCUMENTED-DEFERRED, T13 → `[x]` DONE.
- Deviations: none. T13 is a documentation deliverable (parent §6 lines 849–863); the ruling is grounded verbatim in the M05/M06/M10 Part-C verdicts (deep-generative SKIP · GNN REJECT · LLM FIRM DISQUALIFICATION · TabPFN NOT READY) — nothing invented. Manager authored the Phase-E ruling directly (manager-territory documentation, not feature code); Sonnet built the Phase-D figures + results prose from a fact sheet of verbatim CP-4 numbers (no number outside the sheet).
- Test status: n/a (documentation only — no code, no config, no tests touched). Gate suite unchanged at 171/0 from T12-ship.
- Notes: closes T13 and gives every arc phase a self-contained `results/phase_{A,B,C,D,E}/` doc, so the arc's results story is complete end-to-end. **Arc scoped work (A–D executed + E documented) is now fully COMPLETE and self-documented — ready to file to `docs_DONE`** (physical relocation still deferred to the user per the prior churn). The TabPFN experimental-track contract in `RESULTS_phaseE.md` is the concrete spec for any future frontier revisit: opt-in/off-by-default, pinned weights, `TABPFN_IMPUTED` provenance, MAR geometry/semantics only (never HVAC/U-values), and the same CP-3-style attribute-win + EUI-do-no-harm gate before promotion.

#### Manager — housekeeping: phaseD plan filed to `docs_Done/`, REPORT completed, §9 backlog created — 2026-07-13
- Artifacts: **moved** `PLAN_phaseD_fusion.md` → `docs_Done/` (joins `PLAN_phaseC_ml_imputer.md`, already there); **fixed all live pointers** to both filed sub-plans (parent §0 Phase-D/T11/T12 lines + §5 citations + the §6C "Detailed execution plan" pointer; `results/phase_C/RESULTS_phaseC.md`, `results/phase_D/RESULTS_phaseD.md`, `results/README.md` source-of-record lines) to `docs_Done/…` — the phaseC pointers were already broken from its own earlier move, fixed here too; §8 dated history left untouched (append-only). **`REPORT_missing_input_handling.md`** — added a §4 resolution addendum mapping its §3 "gaps worth a decision" to the arc's closures (§3.1–3.3 HVAC/DHW/cooking Tier-B → CLOSED by Phase-A T02/T03, 25/25 byte-identical; rule-17a thresholds → E-R3-3; §3.4 EPW + §3.5 zoning → correctly out of arc scope); verdict = necessary + complete origin audit, stays with the arc. **NEW §9 backlog** — single trackable surface for every open/deferred/out-of-scope item (9.1 T10 · 9.2 ML clamp+re-A/B + T11.7 · 9.3 five fusion extensions · 9.4 TabPFN experimental track · 9.5 filing incl. the user-owned folder move · 9.6 two adjacent out-of-scope REPORT gaps). Pointers to §9 added to §0. `PROJECT_CHECKLIST.md` header line 6 lead refreshed (no longer says "arc NOT complete").
- Deviations: none. Filing completed sub-plans into the arc-local `docs_Done/` mirrors the existing Phase-C precedent; no content of any plan/report was rewritten except the additive REPORT §4 addendum and pointer-path corrections. The **whole-arc** folder move to `docs_DONE/` was deliberately NOT performed (user-owned per prior churn — logged as an open item in §9.5).
- Test status: n/a (documentation/housekeeping only — no code, config, or tests touched; gate suite unchanged at 171/0).
- Notes: with sub-plans filed, links repaired, the origin audit completed, and §9 giving one clean backlog surface, the arc is fully self-consistent and self-documented. Only the user-owned physical folder relocation remains before it can sit in `docs_DONE/`.

#### Manager — backlog execution: §9.3 crosswalk extended + §9.2 ML clamp built (footgun neutralized) — 2026-07-13
- Artifacts: **§9.3** — `openubem/data/osm_to_use_class.json` (+6 Overture tokens, manager-pinned: entertainment→commercial, train_station→institutional, parking/roof/service/toilets→unknown; schema 1.0.0→1.1.0; vocab unchanged) + 2 tests in `tests/test_building_classifier.py::TestUseClassMapping`; suite 162/0. **§9.2** — `openubem/semantic/imputation.py`: `MLImputer` gains `target_observed_min/max` (None-default), captured at fit (None for classifiers), and `MLImputer.predict()` clamps regression output to `[observed_min, observed_max]` via a guarded `_clamp_to_observed_range` (mirrors the KDE/PDE clamp); 5 tests in `tests/test_ml_imputer.py::TestObservedRangeClamp`; gate `test_ml_imputer+test_imputation+test_imputation_routing+test_fusion` **109/0**. Both by fresh Sonnet employees, manager-audited by reading the json + the clamp code + control flow directly.
- Deviations: none from what was pinned. **One analytical CORRECTION the audit forced (recorded honestly):** the §9.2 backlog text had bundled the `mice`/`linear` **AD-5000 extrapolation footgun** and the **−5.51% newer-skew EUI bias** as both fixable by the clamp. They are DISTINCT. CP-3's EUI leg failed with **`knn`**, whose neighbour-averaging regresses `year_built` toward newer/denser stock; `knn` predicts strictly within the observed range, so the observed-range clamp is a **no-op for `knn`** and does **not** flip the −5.51%. The clamp genuinely closes the extrapolation footgun (a real safety win — `mice`/`linear` are no longer catastrophic) but is **not** sufficient for CP-3 EUI. §9.2 re-scoped accordingly into two items: the clamp (DONE) and a still-open **knn newer-skew de-bias + cluster re-A/B** (design-first; a cluster A/B fired now would just re-confirm −5.51%, so NOT fired — cluster budget preserved).
- Test status: 162/0 (crosswalk suite) + 109/0 (ML+imputation gate). No regressions; `ml` still absent from `IMPUTE_ENABLED_TIERS`; `TestNoEUILeakage` structural guard still passes (clamp bounds are observed-only → zero-fitted-params intact); joblib backward-compat proven (None-default fields).
- Notes: user instruction "pas future, commence maintenant" → started the genuinely-actionable-now backlog items in local, non-harmful scope. Deliberately did NOT autonomously execute the items that need real external data (fusion source adapters / per-municipality LiDAR+assessor), break a guarantee (`enrich_semantics` reroute → CP-1 byte-identity), harm results (T11.7 ship, still blocked by the un-fixed knn bias), or ship an unvalidated method (TabPFN NOT READY). The clamp's real value: it removes the "any future ML method could extrapolate to AD-101,700 stamped HIGH-confidence" landmine, so a future de-biased per-target ML tier can be trusted not to produce catastrophic fills.

#### Manager — Phase-E figures added (results symmetry) + REPORT filed to `docs_Done/` — completed 2026-07-15
- Artifacts: 4 new figures in `results/phase_E/` — `phaseE_four_filters.png` (4×4 family-vs-gate matrix), `phaseE_scale_gap.png` (log-scale data-viability argument: OpenUBEM 10²–10³ band vs GAIN n>30k / TabDDPM n>10–20k thresholds and the ~2.2M-building Sinha 2026 precedent), `phaseE_frontier_verdict.png` (SKIP / REJECT / FIRM DISQUALIFICATION / NOT READY cards + net-ruling banner), `phaseE_tabpfn_contract.png` (5-clause quarantine contract). Built by a fresh Sonnet employee from a manager fact sheet (same workflow as the phase-D figures), phase-D house style matched; all 4 embedded into `RESULTS_phaseE.md` at their sections; `results/README.md` tree updated ("no figures" note removed). Separately, `REPORT_missing_input_handling.md` physically moved (git mv) into the arc-local `docs_Done/` on user request — §9.5 filing partially discharged.
- Deviations: none. Executor self-caught and removed reason-captions it had invented beyond the fact sheet (bare ✓/✗/◐ cells restored) — audited by the manager reading all 4 PNGs back as images; no invented numbers or claims.
- Test status: n/a (documentation only — no code, config, or tests touched).
- Notes: `results/phase_{A,B,C,D,E}/` is now fully symmetric (five phases × RESULTS + 4 figures each). Phase E remains DOCUMENTED-DEFERRED — the figures visualize the ruling, they do not change it. Whole-arc folder move to `docs_DONE/` remains user-owned (§9.5).

#### Manager — draw-tier illustration figures + "1951-mode / not-an-error" explanation recorded — completed 2026-07-17
- Artifacts: 4 new figures in `results/phase_C/` (built by fresh Sonnet employees, manager-audited by reading each PNG back): `phaseC_draw_scatter_{year_built,levels}.png` (Phase-A flat band vs `pmm` dispersed cloud, side by side) and `phaseC_draw_distribution_{year_built,levels}.png` (histogram + ECDF overlays of actual vs Phase-A vs `pmm` — the correct lens for a draw method). Copies of the two scatter figures also live in `openubem/outputs/`. Explanation written into `docs/docs_EXPLANATION/OpenUBEM_imputation_methods.md` §6.5 and `implementation/IMPLEMENTATION_phaseC_ml_imputer.md` §I.4b. All builders live in `openubem/results/impute_scatter.py` (new `_pooled_draw_pairs` helper mirrors `draw_leaderboard._draw_pairs`; new `build_phaseC_draw_scatter_*`/`build_phaseC_draw_distribution_*`).
- Deviations: figures written to `results/phase_C/` (not `openubem/outputs/`) per explicit user instruction to sit beside the existing `phaseC_scatter_*`; the CLAUDE.md flat-outputs rule is superseded here by that direct instruction (scatter copies also in `openubem/outputs/`). Explanation rendered in English per the deliverables-in-English convention (user quoted it in French conversationally).
- Test status: both honesty-guard cross-checks passed on every build (regenerated Phase-A MAE reproduces committed `RESULTS_phaseC.md`: `year_built` 26.43, `levels` 9.18 to 2dp). Report-only: `IMPUTE_ENABLED_TIERS` untouched, `IMPUTE_DRAW_METHOD_BY_TARGET` mutated then restored in `finally`; draw tier stays opt-in/OFF. No new pytest, no cluster, no EUI, no network.
- Notes: KEY FINDING recorded — the horizontal band surviving at ~1950 under `pmm` is REAL DATA (794/2247 = 35% of observed `year_built` are exactly 1951, a true mode spike), NOT the median fallback; and the `pmm` cloud cannot reach the 1:1 diagonal because `use_class`/`archetype_id` are absent from the Stage-1 `01_buildings.gpkg` schema, so `pmm` degenerates to a global marginal bootstrap (regenerates the histogram, not each building's value). `year_built`/`levels` are not inferable from footprint+location, so no method reaches the diagonal — the scatter-vs-1:1 is the wrong lens; histogram/ECDF is the right one. Does not change any ship decision (draw tier stays OFF; promotion remains a separate user call).
...
## 9. Remaining / deferred backlog — future-arc candidates

> **Single trackable surface for "what's left."** The arc's scoped work (Phase A–D executed + E
> documented) is **COMPLETE** — **nothing in this section blocks arc closure or filing to `docs_DONE`.**
> Every item below was either deliberately scoped out or deferred *with a decision on record*, so a
> future session can pick any one up cleanly. Legend as in §0 (`[ ]` open · `[x]` done · `[—]` deferred /
> out of scope · `[✗]` evaluated & closed not-met). **Last reviewed:** 2026-07-13.

### 9.1 Deliberately out of scope (never built, never gated)
- [—] **T10** — optional `--replicates M` uncertainty mode (`config.py` + orchestrator). A convenience
  feature; never a CP condition. Un-built by design; available as a future enhancement.

### 9.2 Phase C (classical-ML tier) — revisit only on user opt-in
- [x] **ML fill clamp (extrapolation footgun)** — **DONE + audited 2026-07-13** (fresh Sonnet). Observed-range
  `np.clip` to `[y.min(), y.max()]` (bounds captured at fit, `None` for classifiers) inside
  `MLImputer.predict()`, guarded, mirroring the existing KDE/PDE clamp — **zero-fitted-params** (bounds are
  observed data, not a fitted parameter; `TestNoEUILeakage` still passes). Neutralizes the `mice`/`linear`
  **AD-5000 extrapolation footgun**: local demo (seed 42, OOD `year_built`) shows raw pred ~AD-101,700 /
  MAE ~99,700 collapse to the observed range / MAE 52.1; RF/knn in-range preds unchanged (clamp is a no-op
  for them). 5 new tests + joblib backward-compat (None-default fields); gate **109/0**, no regressions,
  `ml` still OUT of the default tuple. Manager-audited (code read).
- [x] **knn newer-skew de-bias + cluster EUI re-A/B — CLOSED / RESOLVED 2026-07-14.** The −5.51 % was
  **re-attributed as a pooled-evaluation granularity artifact, not a production do-no-harm failure**, and the
  de-bias line is closed. Trail (full detail in `docs_Done/PLAN_phaseC_ml_imputer.md` §8, T11.8→T11.8c-diag):
  (1) **T11.8** built a reusable stratified empirical quantile-mapping corrector (opt-in, zero-fitted-params,
  byte-identical off, 12/12 + 190/0) — but its 1st CP-3b-local proof showed **0.0 % skew collapse**: the
  pinned `use_class`/`archetype_id` stratifier is absent from the Stage-1 `01_buildings.gpkg`, so the hook
  structurally no-ops on real data. (2) **T11.8b** global (unstratified) qmap fallback (192/0) — re-proof
  showed the skew **worsened** (+0.4593→+0.6016), because the global donor pool is LA-dominated (wrong,
  non-local reference for nyc_centre). (3) Manager REFRAME: knn needs pooling to clear its ≥200 fit floor
  (nyc_centre = 158 obs), but production imputes **per-cell**, where knn-`year_built` never fires on
  nyc_centre — the −5.5 % was measured at a granularity production never uses. (4) **T11.8c-diag** (per-cell,
  floor-clearing cells): raw knn's directional vintage-bin gap = **exactly +0.0000 on both la_suburban
  (0/1343) and la_urban (0/618)** ⇒ identical vintage bins → identical EUI → do-no-harm **and** do-no-good.
  **Terminal decision: ml tier stays built-but-off / opt-in — held off because it is EUI-neutral at
  production granularity (no benefit case), NOT because it harms; the Phase-C "systematic −5.5 % bias"
  verdict is superseded as a methodology artifact.** No cluster spend (a per-cell A/B would return NMBE≈0 by
  construction). De-bias corrector kept in-tree as harmless opt-in capability.
- [ ] **T11.7** — production wiring + `enrich_semantics` byte-identity reconcile (moves `ml` into a
  per-target default). **USER-SIGN-OFF-only; not pursued** — now for a *different* reason: the −5.5 % harm was resolved as an
  eval artifact (above), so shipping `ml` would no longer re-introduce harm, but at production granularity it
  is **EUI-neutral** (zero benefit) so there is no case to add an sklearn fit to the default. *(Superseded
  rationale, kept for trail: was "blocked until the newer-skew de-bias passes a do-no-harm A/B / would
  re-introduce the −5.51 % harm.")* Carries the parent CP-2 byte-identity reconcile if ever revisited.

### 9.3 Phase D (fusion tier) — extensions (all documented-deferred at ship)
- [ ] **`enrich_semantics` production reroute** — the CP-1-byte-identity-**breaking** change; a **separate
  future arc**, not Phase D (was §5D-quarantined). Shared concern with 9.2's T11.7 reconcile.
- [ ] **Additional source adapters** — EUBUCCO / GHSL / imagery. The source registry is pluggable; each
  needs an adapter + a `LICENSES.md` entry + a precedence slot in `FUSION_SOURCES_BY_TARGET`.
- [x] **Extend `osm_to_use_class.json`** — **DONE 2026-07-13** (fresh Sonnet, manager-audited). 6 Overture
  tokens mapped: `entertainment`→commercial, `train_station`→institutional (real classification gains);
  `parking`/`roof`/`service`/`toilets`→unknown (non-conditioned/ambiguous → do-no-harm, no fabricated
  archetype). `schema_version` 1.0.0→1.1.0, vocabulary unchanged (6 values), 2 crosswalk tests added,
  suite **162/0**. Pure data + test, no engine change.
- [ ] **Real per-municipality LiDAR + assessor config** — the `LidarSource` / `AssessorSource` config-path
  adapters exist and are unit-tested, but no real municipal nDSM/assessor dataset is wired. Needs
  `FUSION_LIDAR_NDSM_PATH` / `FUSION_ASSESSOR_PATH` + per-city `FUSION_ASSESSOR_FIELDS`. This is what would
  make `year_built` fusion actually fire (Overture has no `year_built` column — assessor is its only source).
- [ ] **Cluster EUI validation leg for fusion** — Phase-D's CP-4 was a join / fill-rate / license gate only
  (no EnergyPlus). A downstream-EUI A/B of fused-vs-imputed morphology is a future validity check (Phase D
  treated fused values as ground truth, so this was correctly out of CP-4 scope).

### 9.4 Phase E (advanced frontier) — experimental only, never default
- [ ] **TabPFN isolated experimental track** — the only frontier method that passes the constraint filters,
  but **NOT READY** (no building-domain validation exists). If ever built: opt-in / off-by-default behind an
  optional extra, pinned weights + fixed seed, `TABPFN_IMPUTED` provenance + calibrated posterior, **MAR
  geometry/semantics targets only (never HVAC / U-values)**, reuse the T08/T09 harness, and clear the same
  CP-3-style attribute-win + EUI-do-no-harm gate **before** any promotion. Full contract:
  `results/phase_E/RESULTS_phaseE.md`.
- [✗] **Deep-generative / GNN / LLM** — **CLOSED, ruled out with evidence** (out-scaled / constraint-breaking
  / hallucination). *Not* backlog — do not revisit without materially new evidence. Rulings: M05 / M06 / M10.

### 9.5 Filing / housekeeping
- [x] **Move completed sub-plans → `docs_Done/`** — `PLAN_phaseC_ml_imputer.md` + `PLAN_phaseD_fusion.md`
  (done 2026-07-13); all **live pointers** in the parent plan + results docs updated to `docs_Done/…`
  (§8 dated history left as append-only record).
- [x] **`REPORT_missing_input_handling.md` completeness** — resolution addendum (§4) added mapping its §3
  gaps to the Phase-A closures (§3.1–3.3 CLOSED; §3.4 EPW + §3.5 zoning correctly out of arc scope → tracked
  in 9.6 below). Necessary + complete as the arc's origin audit; stays with the arc.
- [ ] **Physical relocation of the whole arc folder `docs_ACTIVE/input/imputation/` → `docs_DONE/`** —
  **LEFT FOR THE USER.** A premature move on 2026-07-13 was user-reverted; **do not move autonomously.**
  This is the final step to fully close the arc. When it happens, also refresh `PROJECT_CHECKLIST.md`
  header line 6 (its lead still narrates the pre-completion state).

### 9.6 Adjacent gaps this arc surfaced but scoped OUT (for other arcs)
- [ ] **Missing-EPW-path silent fail** (REPORT §3.4) — Stage-2.3 climate/weather; arguably should be Tier-C
  (hard fail). A future climate/geometry arc.
- [ ] **Zoning-fallback warning-only** (REPORT §3.5) — Stage-1 geometry/zoning; no `data_quality_flag`
  column entry, unlike its geometry-simplification sibling. A future geometry arc.
