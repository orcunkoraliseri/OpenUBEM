...
- **T13 — Expand to complex/high-value archetypes.** *What:* add SmallHotel/LargeHotel (units+corridor, **after reconciling the L06-vs-L08 hotel-dim conflict** — stop-and-quote if unresolved), LargeOffice (corridor+core), School (hybrid: pack classroom wings, single-zone gym/cafeteria). *Why:* offices/schools/hotels are where L14 says accuracy actually moves (10–20%). *How:* extend `MODULE_SPECS` + classifier routing; per-space loads rows for each. *Test:* per-archetype zone-count + conservation tests.
