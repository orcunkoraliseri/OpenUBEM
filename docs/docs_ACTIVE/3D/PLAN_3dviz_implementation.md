...
- [ ] **Phase D — Delivery & validation**
  - [ ] T13 `viewer_export.py` Step-5 exporter → self-contained `<run_id>_viewer.html`
  - [ ] T14 Six validation checkpoints as tests (Geometry/Value/Provenance/LOD/Reproducibility + a11y walkthrough)
  - [ ] T15 LIVE_SMOKE: full pipeline run on a real cell → real viewer.html, manager opens + confirms faithful
  - [ ] **CP-3 — MVP acceptance** (USER-SIGN-OFF: the viewer is faithful, reproducible, self-contained)
