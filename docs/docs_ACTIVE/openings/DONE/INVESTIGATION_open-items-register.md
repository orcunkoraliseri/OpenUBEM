...
> # 🔒 CLOSED 2026-08-20 — this book is history. The live register is `INVESTIGATION_open-items-register-II.md`
>
> **Do not add open work here. Do not amend the table below.** This document ran from 2026-08-04 to
> 2026-08-20 and reached **8,205 lines, 61 items, 40 of them struck.** It was closed on the project's
> standing convention — *close a doc past roughly a thousand lines, open a successor* — because every
> executor dispatch and every director resume began by reading it in full, and what a reader needs is
> the ~20 items that are open, not the 40 that are shut.
>
> **The file stays at this path deliberately.** 79 documents cite into it; nothing was moved and no
> citation was broken. It remains the authority on **every closed item** and on **every live item's
> full history** — each live item keeps its `### OPEN-NN` section here, and the successor points back
> to it by heading rather than restating it.
>
> ## What was true at close
>
> - **21 live · 40 struck · 61 total**, exactly `OPEN-01 … OPEN-61`, no gap and no duplicate.
>   **Next free item ID: `OPEN-62`.** Recounted programmatically
>   (`scripts/analysis/open_register_recount_2026-08-18.py`), not by hand.
> - **No ruling is owed by the user. No plan is in force. Nothing is running on the cluster.**
> - Adopted fleet figure **153.8231 kWh/m²** pooled over 8,153 buildings — **not volume-correct**
>   (OPEN-56's ≈+1.0 is not in it) and ~1 % low by **OPEN-61** on the four buildings where that has
>   been measured.
...
> - Last pass to write here: `implemenation/previous/PLAN_four-board-items-2026-08-20.md`, all four board rows
>   done (AA7, S03, W9, C05), **one item opened — OPEN-61**, one item's stated population corrected
>   (OPEN-35), 206.79 GiB of disk recovered.
>
> ## 🔴 The late correction most likely to be missed
>
> **OPEN-61 was opened 2026-08-20 with a §-section and an amendment note but no §1 table row**, so
> this document's own recount script reported **20 live / next free `OPEN-61`** while its prose and
> the director prompt both said **21 / `OPEN-62`**. **The prose was right; the table was one row
> short.** The row was written and inserted at the foot of the §1 table **as part of closing this
> book**, and the script now agrees with the prose. If you are reading an older quotation of "20 live"
> anywhere in this arc, that is where it came from.
>
> **The two rules this document ran on carry over to the successor:** correct in place and never
> delete, and a closure must name what survives it.
...
**Amended 2026-08-20 (**director**, at CP-2/CP-3 of `implemenation/previous/PLAN_four-board-items-2026-08-20.md`, executed on the user's standing delegation *"pas necessaire de prendre mon decision, tu peux decides pour toutes des steps"*): **+1 opened, none closed, one item's stated population corrected. 20 → 21 tracked.** **(1) OPEN-61 opened** — the production meter query never requests any `DistrictHeating` meter, so `parse_building()`'s `total_eui_kwh_m2` silently drops the District Heating component of **Water Systems**, measured at **~1.0-1.2 % of total end uses** on the four buildings where it was found. 🔴 **The remedy is not a one-line query edit:** no `DistrictHeating` meter exists in the `.sql` at all, so the value is only reachable from the ABUPS tabular table or by requesting the meter in the IDF. Found by **T01's own reconciliation control**, which was not asked for — the third time in three passes that a defect surfaced from an executor checking its own arithmetic ([[OPEN-60]] was the second). **The adopted `153.8231 kWh/m²` is NOT restated here** — the blast radius across the fleet is unmeasured, and this register does not restate a published number on four buildings. ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations. **(2) OPEN-35's stated population is corrected, not its ID:** the headline *"1,031 mid-/high-rise apartments"* is **stale**. Re-derived programmatically over the 8 in-scope cells, only **25** apartment-archetype candidates remain and **24 of the 25** already build at the group-median storey count in today's production, following the OPEN-35 T05/T06 wiring fix of 2026-08-19; **0** are eligible for intervention. **(3) A structural null is recorded for `austin_rural`:** across all **244** of its affected buildings and all 11 archetypes present, the two storey fallbacks agree, so the mismatch cannot arise there at all — 0 of 244 eligible, versus 459 eligible across the other seven cells. **(4) OPEN-53's W9 half is discharged** — see its own heading. The invariant *struck rows − retired IDs = 2* (OPEN-02, OPEN-28) is unchanged: this edit adds one live row and strikes none. **Next free item ID: `OPEN-62`.**

**Amended 2026-08-20 (**director**, at CP-4 of `implemenation/previous/PLAN_four-board-items-2026-08-20.md` — T04b, the last task of the pass): **NO item opened, closed, struck or retired. The count stands at 21 live, next free `OPEN-62`, invariant *struck − retired = 2* (OPEN-02, OPEN-28) untouched.** 🟢 **OPEN-35 has its first paired within-building measurement.** 21 buildings across 7 cells, 42 EnergyPlus runs, base arm = the geometry fallback production actually uses (**1 storey for all 21**), treated arm = the archetype fallback (levels-group median, 2–19 storeys); same building, same weather, same archetype, one input changed. **All three pre-registered controls pass:** base EUI reproduces `on_record_total_eui_kwh_m2` to **0.0199 %** worst case (threshold 1 %), **19 of 21** treated runs clean (threshold ≥ 18, all 21 completed, 0 fatal), **21 of 21** treated IDFs differ in storey count read from the built IDF. **Result, within-cell medians only:** la_centre **+75.25 %**, austin_suburban **+73.58 %**, austin_centre **+73.09 %**, austin_urban **+70.28 %**, nyc_centre **+59.88 %**, la_urban **+37.89 %**, **la_suburban −2.58 %**, austin_rural structural null. 🔵 **The sign of the defect is not fixed** — the two `SmallOffice` buildings going 1 → 2 storeys in `la_suburban` get *better* (−11.31 %, −2.58 %), because a second floor on a small building in a mild climate halves roof and ground loss per unit area faster than it adds load. **Pooling would have produced one confident positive number and erased that.** The standing no-pooling prohibition now rests on a measured sign flip rather than only on a composition argument. ⚠️ **`153.8231 kWh/m²` is NOT restated** — 21 of 8,153 buildings do not move a fleet baseline — this is **not** a population estimate (the 21 were selected *because* the fallbacks disagree), and it does **not** decide **which** fallback is correct, which remains the DESIGN decision OPEN-35 has always been waiting on. ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations. Full detail under the **OPEN-35** heading and in `extra/MEASUREMENT_open-35_storey-intervention.md` §T4b. **This closes the four-board-item pass: AA7, S03, W9 and C05 are all done.**

**Amended 2026-08-20 (**director**, T02 of `implemenation/previous/PLAN_four-board-items-2026-08-20.md`, on the user's ruling that the classifier fixture be relabelled from raw evidence *"pour toutes des donnees"*): **NO item opened, closed, struck or retired. The count stands at 21 live, next free `OPEN-62`.** The classifier exam is rebuilt over the **whole** tag-rich pool rather than a sample: `tests/fixtures/labelled_archetypes_tagrich_v3.csv`, **592 graded rows** (Boston 233 / Chicago 359, reproducing v2's own pool definition exactly), plus `..._tagpoor_v3.csv` with the remaining **290 rows, deliberately ungraded**. Labels are written from tag evidence only; size may pick a band *inside* an already tag-determined class and may never invent the class — which is precisely the circularity the user objected to in v2, where roughly a third of the answers came from floor area. **589 of 592 rows are `strong` evidence; the 3 `thin` rows are all flagged.** 🔴 **The result is reported as two numbers and must always be quoted as two:** fine top-1 is **71.1 %** strict and **89.0 %** once the classifier's own deliberate `TallBuilding`/`SuperTallBuilding` height override is credited (it is emitted 106 times and v3's answer key is forbidden to produce it). **Coarse top-1 is 100.0 % on all 592.** ⚠️ **This is not an improvement on v2's 88.8 %/98 rows and may not be stated as one** — checklist ruling `2a` binds: no pass mark carries to a new exam. The honest statement is *comparable performance on an exam 6× larger and much less circular*. ⚠️ **The residual limit is named rather than hidden:** of the 65 errors that survive crediting the height rule, almost all are **size-band confusions inside the correct family**, and the band is the one part of a v3 label still permitted to touch a classifier input — v3 removed the circularity from the class and could not remove it from the band. **Gate set by the director under the standing delegation:** coarse ≥ 0.98 (measured 1.000, pass), fine-with-override-credited ≥ 0.85 (measured 0.890, pass), strict fine **ungated as a diagnostic**. **No ID is opened** — comparable performance on a larger, cleaner exam is a confirmation, not a defect. One real gap is **flagged and not opened**: `osm_to_use_class.json` has no `place_of_worship` entry. Full detail: `extra/MEASUREMENT_s03_tagrich-v3-exam.md`.
...
> 🟢 **Amended 2026-08-20 (**director**, T01 of `implemenation/previous/PLAN_four-board-items-2026-08-20.md`).
> THE −23.61 % IS NOW LOCALISED TO AN END USE AND A CAUSE, WITH NO NEW SIMULATION.** The four
> from-scratch buildings — the ones whose loads are identical in both modes, so that loads cannot
> explain any of their gap — were diffed end use by end use.
>
> **Control first:** all four per-building ratios reproduce, and so does the pooled figure —
> **−23.6090 %** against the −23.61 % on record. Each arm's seven end uses reconcile against its own
> ABUPS total to within **0.031 %**, far inside the 0.5 % bound.
>
> | end use | Δ kWh/m² (`layout_assign` − `auto`) | share of gap |
> |---|---:|---:|
> | **Heating** | **−42.20** | **87.6 %** |
> | Fans | −4.98 | 10.3 % |
> | Cooling | −2.37 | 4.9 % |
...
>
> 🔴 **The cause is envelope area, not loads and not vintage.** Floor area, zone count and WWR are
> **identical** between arms in all four buildings. But `nyc_centre/265424467` — which dominates the
> pool by floor area — has **44 % less exterior wall (16,270 → 9,122 m²) and 44 % less window
> (6,502 → 3,645 m²) under `layout_assign` at the same floor area, same 81 zones and same 39.96 %
> WWR.** That is exactly the shape of its lower heating. The other three buildings' envelopes are
> byte-identical between arms and differ only in modest HVAC autosizing.
>
> **This settles the load question for this item.** Lighting and equipment contribute **exactly zero**
> to the gap on the buildings where loads are shared — consistent with, and stronger than, the earlier
> refutation that loads explain ≈8 % rather than "half". OPEN-03 is a **geometry** finding wearing a
> loads-shaped name.
>
> ⚠️ Each arm's **absolute** EUI here is ~1 % low because of **[[OPEN-61]]**, opened out of this very
> task's reconciliation control. The **paired** difference above is unaffected — both arms drop the
> same term.
>
> **Evidence:** `extra/MEASUREMENT_open-03_enduse-localisation.md`;
...
> 🔴 **Amended 2026-08-20 (**director**, T04/CP-3 of `implemenation/previous/PLAN_four-board-items-2026-08-20.md`).
> THIS ITEM'S STATED POPULATION IS STALE AND IS CORRECTED HERE. The ID stays open; the headline
> "1,031 mid-/high-rise apartments" does not.**
>
> A census of the 8 in-scope cells (790 rows,
> `openubem/outputs/comparisons/open35_storey_intervention_2026-08-20_census.csv`) finds:
>
> - **Only 25 apartment-archetype candidates remain in those cells at all**, and **24 of the 25**
>   already carry `archetype_consumed_group_median_today = True` — today's `derive_num_floors` builds
>   them at the group-median storey count, not at 1. **0 of the 25 are eligible for intervention.**
>   The cause is the item's own **T05/T06 wiring fix of 2026-08-19**: the population largely
>   self-corrected. *"As built today"* no longer equals *"levels = 1.0"* for most of the originally
>   cited buildings.
> - **`austin_rural` is a structural null: 0 of 244.** Across all 244 of its affected buildings and
>   all 11 archetypes present, the two fallbacks **agree**, because that cell's levels-group-median is
>   1 for every `use_class` with an observed row. In a cell with no storey evidence anywhere, the
>   mismatch cannot arise. The other seven cells carry **459** eligible buildings between them
>   (`austin_centre` 242, `nyc_centre` 99, `austin_urban` 43, `la_centre` 31, `la_urban` 26,
>   `la_suburban` 15, `austin_suburban` 3).
>
> 🟢 **The executor stopped rather than substitute a cell.** T04's pre-registered control demanded 24
> buildings across 8 cells with a genuine fallback disagreement; `austin_rural` cannot supply one, so
> it halted **before EnergyPlus was invoked** instead of cell-shopping for a non-null or quietly
> dropping the cell. The director accepted the failure and amended the scope: **21 buildings, 7 cells**
> (`LargeOffice`/`Courthouse`/`SmallOffice`, base 1 storey vs treated 2–19), reported as
> *"what the mismatch costs over the buildings where it still exists today"*.
>
> **Standing prohibition, restated because the smaller n makes it more tempting, not less:** within-cell
> medians only. The 21 are **never** pooled into a headline figure, **never** restated as a fleet EUI
> correction, and **never** carried back onto the 1,031-building framing. OPEN-35's naive
> +62.20 kWh/m² remains a composition artifact.
>
...
> ---
>
> **Amended again 2026-08-20 (later the same night, **director**, at CP-4 — T04b): THE INTERVENTION
> RAN. This is the first paired within-building measurement OPEN-35 has ever had.** 21 buildings,
> 42 EnergyPlus runs, base arm = the geometry fallback that production actually uses (1 storey for all
> 21), treated arm = the archetype fallback (the levels-group median, 2–19 storeys). Same building,
> same weather, same archetype, **one input changed**.
>
> 🟢 **All three pre-registered controls pass, and control 1 is the one that makes the rest usable.**
> Every base-arm EUI reproduces its `on_record_total_eui_kwh_m2` to within **0.0199 %** worst case
> (threshold 1 %, 0 of 21 outside) — the harness reproduces the production fleet number on the
> untouched arm, so the treated arm's movement is the intervention and not the harness. This is
> exactly the control **OPEN-58** taught the project to run. **19 of 21** treated runs are clean
> (threshold ≥ 18); all 21 completed, **0 fatal**; the two carrying one severe error each
> (`way/516285449`, `way/382991813`) are **retained and flagged, not dropped**. **21 of 21** treated
> IDFs differ from their base in storey count, read from the built IDF rather than trusted from the
> selection table.
>
> 🔴 **Result — within-cell medians, per the standing prohibition immediately above.**
> la_centre **+75.25 %** · austin_suburban **+73.58 %** · austin_centre **+73.09 %** · austin_urban
> **+70.28 %** · nyc_centre **+59.88 %** · la_urban **+37.89 %** · **la_suburban −2.58 %** ·
> austin_rural **structural null (0 of 244)**. So for the buildings where the two fallbacks genuinely
> disagree, **the choice of fallback is not a rounding matter** — it moves a single building's EUI by
> tens of percent, and production uses the **lower** arm in all 21 cases.
>
> 🔵 **The finding the plan did not anticipate, and the concrete vindication of the no-pooling rule:
> the sign of this defect is NOT fixed.** The cell ordering tracks **how many storeys the treated arm
> adds**, not climate and not density tier — and the two `SmallOffice` buildings going 1 → 2 storeys
> in `la_suburban` get *better*, not worse (`way/449558400` **−11.31 %**, `way/449558402` **−2.58 %**).
> A second floor on a small building in a mild climate halves roof and ground loss per unit area
> faster than it adds load. By archetype: `LargeOffice` n=15 median **+68.02 %**, `Courthouse` n=4
> **+67.63 %**, `SmallOffice` n=2 **−6.95 %**. **Pooling these 21 would have produced a single
> confident positive number and erased the one structurally interesting result in the set.** Until
> tonight the prohibition rested on a composition argument; it now rests on a measured sign flip.
>
> ⚠️ **What this does NOT do, stated so it does not drift.** It does **not** restate `153.8231 kWh/m²`
> — 21 of 8,153 buildings do not move a fleet baseline and no attempt was made.
> ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations.
> It is **not** a
> population estimate: the 21 were selected *because* the fallbacks disagree, and the 459 eligible
> across seven cells are the sampling frame. It does **not** decide **which fallback is correct** —
> that remains the DESIGN decision this item has always been waiting on; tonight only sizes the
> disagreement. It is **not** about apartments (see the population correction above). And
> **[[OPEN-61]]'s omission is present in both arms and untreated** — a common-mode term in a paired
> difference, far below these effect sizes, but on the record.
>
...
> 🟢 **Amended 2026-08-20 (**director**, T03 of `implemenation/previous/PLAN_four-board-items-2026-08-20.md`,
> on the user's W9 ruling *"prune, move out of Temp"*). THE CUSTODY EXPOSURE IS DISCHARGED. The
> `.sql` are gone and the citable evidence is out of Temp.**
>
> **First, the X06 census above is superseded, not retracted.** A full re-inventory on 2026-08-20
> found **six** corpora, not three: **140,261 files / 233.898 GB**, of which **222.04 GB** was `.sql`.
> `open48_refleet4` (81.09 GB) postdates the 2026-08-18 count. X06 was right about what existed when
> it ran.
>
> | stage | result |
> |---|---|
> | copied out of Temp | **116,128 files / 11.73 GB** → `C:\Users\o_iseri\Desktop\OpenUBEM\evidence\<corpus>\…` |
> | extensions kept | `.err .eio .end .gpkg .csv .geojson .parquet .json .idf` |
> | count verification | per-extension destination count == source count, all 9 extensions × 6 corpora, **zero missing, zero extra** |
> | hash verification | SHA-256 on **380** files (200 random ∪ all 180 `.gpkg`/`.parquet`) — **380/380 match** |
> | deleted | **23,862 `.sql` files, 206.79 GiB** (= 222.04 GB), under `%LOCALAPPDATA%\Temp\ubem_validation` **only** |
> | remaining `.sql` under that root | **0** |
>
> Director controls before authorising deletion: destination file count and byte size re-derived
> independently; **0** `.sql` present in the evidence tree; source `.sql` still intact at the moment of
> signing; `git check-ignore` confirms `evidence/` is at `.gitignore:42` and cannot enter a commit.
>
> 🔴 **One cost is now on the record, and it is not hypothetical.** T01 of the same plan was reading
> `auto`-arm `.sql` **from this very root** while the copy was being audited. It finished before the
> deletion, so nothing was lost — but the pre-deletion safety check (*"nothing modified in the last 90
> minutes"*) **could not have caught it, because a reader does not update mtime.** Any future
> re-derivation of an `open48_*` end-use number now needs a re-simulation rather than a re-read, and
> **[[OPEN-61]] is already a live item that would have been sized from that corpus.** The
> `layout_assign` arm's `.sql` under `scratchpad/open03-untrimmed-sample/` were never in scope and
> survive.
>
> **What this amendment does NOT do:** it does not close OPEN-53. The item's original finding — 874 of
> 875 harvest directories missing their evidence files — is untouched by a storage prune. Only the
> custody-exposure half (board row W9) is discharged.
...
**Hypothesis 1 is refuted by direct measurement.** T02 of `implemenation/previous/PLAN_gap-decomposition-2026-08-19.md`
...
`implemenation/previous/PLAN_gap-decomposition-2026-08-19.md` was commissioned to split 48 buildings by end use.
...
### OPEN-61 — `total_eui_kwh_m2` drops the District Heating component of Water Systems 🔴 **OPENED 2026-08-20**

**Opened by the director 2026-08-20**, under the user's standing delegation of the same day. Found by
**T01 of `implemenation/previous/PLAN_four-board-items-2026-08-20.md`**, which — like the task that produced
[[OPEN-60]] — began by testing whether its end-use pieces summed to the total the parser reports.
⚠️ **Three passes in a row, the defect came from an executor auditing its own arithmetic, and none of
the three plans asked for that control.** That is now a pattern worth naming rather than a coincidence.

#### Mechanism — verified at the `.sql`, not inferred

`METER_QUERY` in `openubem/results/parser.py:48-54` enumerates its meters by name. The list contains
`WaterSystems:NaturalGas` and `WaterSystems:Electricity` and **no `DistrictHeating` meter of any kind**
— not for Water Systems, not for Heating. `dhw_eui_kwh_m2` is then built from exactly those two names
(`openubem/results/parser.py:469`), so any district-heated DHW energy is dropped before
`total_eui_kwh_m2` is formed.

Director control, run directly on
`scratchpad/open03-untrimmed-sample/austin_centre/sim/way_1008727470/eplusout.sql`:

| source | Water Systems | Total End Uses |
|---|---|---|
| ABUPS `End Uses`, District Heating column | **0.72 GJ** | **0.72 GJ** |
| ABUPS, all fuels | 11.68 GJ gas + 0.72 GJ district + water | 42.02 GJ elec + 16.48 GJ gas + 0.72 GJ district |
| Run Period meters matching `%DistrictHeating%` | **none exist** | **none exist** |

0.72 / 59.22 GJ = **1.22 %** of total end uses on this building, unreported. T01 measured
**1.03-1.10 %** across its four buildings by the same route. The two figures agree in size; the
difference is which total the share is taken against.

#### Why this is not a one-line fix

🔴 **Adding the meter name to `METER_QUERY` would change nothing.** The `.sql` contains no
`DistrictHeating` Run Period meter to read — the value survives only in the ABUPS tabular table. Any
remedy must either request the meter in the IDF (changing what every future run emits, and leaving
every existing `.sql` unfixable) or read the ABUPS table (a second, differently-shaped source of
truth inside the parser). **Both are design decisions and neither is taken here.**

#### Blast radius — explicitly unmeasured

Four buildings, one cell family, both modelling modes. **Nothing is restated on this evidence.** The
adopted `153.8231 kWh/m²` over 8,153 buildings stands unchanged, and the honest statement is that it
is low by an unknown amount that is ~1 % *on the four buildings where it has been measured*.
⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations.
Sizing it
fleet-wide is the obvious next task and is not commissioned yet.

⚠️ 🔴 **The fleet `.sql` corpus that would have sized this was deleted on 2026-08-20** under W9's
prune (see OPEN-53). Sizing it now requires a re-simulation, not a re-read. **That is a direct,
foreseeable-in-hindsight cost of the prune, and it is recorded here rather than discovered later.**

#### Relationship to other items

- **[[OPEN-60]]** is the same class of defect — a `total_eui_kwh_m2` that omits energy it should carry
  — reached by a different mechanism. Two independent omissions in one formula, found four days apart,
  is an argument for a reconciliation control **inside** `parse_building()` rather than in whichever
  task happens to check.
- **OPEN-03**'s end-use decomposition is affected identically in both arms, so its **−23.61 %** paired
  result is sound while each arm's absolute EUI is ~1 % low.
