...
| | |
|---|---|
| 🟢 **CURRENT STATE — 2026-09-24 (read this row first; every row below it is history)** | **12 live / 54 retired / 66 total — next free `OPEN-67`.** Since the 2026-09-09 amendment: 🔒 **four closed** — OPEN-17, OPEN-56, OPEN-60, OPEN-64, all discharged by `implemenation/PLAN_accuracy-restatement-2026-09-09.md` (T01–T09 complete, CP-1–CP-4 signed 2026-09-10); ➕ **two opened** — OPEN-65 and OPEN-66. **Adopted figure: 153.95 kWh/m² pooled over 8,139 buildings, carbon 881,743 t CO₂e on the same population (user, CP-4, 2026-09-10).** 🔴 **It is KNOWN LOW again**: the T08 harvest parser never read district heating, so OPEN-61's district-heated hot water is missing from it (OPEN-65). Measured 2026-09-24 on 2,741 of the 8,139 buildings (49.6 % of fleet floor area, same simulations): **+19.97 kWh/m² pooled on that subset (159.27 → 179.24, +12.5 %)**. ⚠️ **Subset only — not a fleet restatement, never applied as an offset, and 153.95 stays the adopted figure until the user adopts a replacement.** **Plans in force:** none (the 2026-09-09 plan is complete but not yet archived). **Running on the cluster for this arc:** nothing. **Rulings owed by the user: one** — authorise the CPU-only re-parse (§4). |
...
**Live IDs (12, as of 2026-09-24):** `03, 19, 35, 38, 53, 58, 59, 61, 62, 63, 65, 66` · **Retired (54):** the 46 listed below + `09, 10, 14, 18` (closed 2026-08-21) + `17, 56, 60, 64` (closed 2026-09-24). **12 + 54 = 66 = `OPEN-01 … OPEN-66`.** The lines below are the 2026-08-20 state, kept for the record.
...
## 2. The live items — ~~**16**~~ ~~**12**~~ ~~**13**~~ ~~**14**~~ **12** as of 2026-09-24 (this section was written 2026-08-20 at 19)

🟢 **AMENDED 2026-09-24 by the director — read the §7 entry of that date first.** 🔒 **OPEN-17, OPEN-56, OPEN-60 and OPEN-64 are CLOSED** (each discharged by `implemenation/PLAN_accuracy-restatement-2026-09-09.md`; closing notes in §6). ➕ **OPEN-65 and OPEN-66 are OPENED** (rows at the foot of this table). 🔴 **OPEN-61 and OPEN-63 were supposed to be discharged by that plan and are not**: the adopted figure was computed by a parser that never reads district heating (OPEN-65).
...
| OPEN-63, OPEN-64, OPEN-65, OPEN-66 | **This book only** — §6 below, plus the progress log of `implemenation/PLAN_accuracy-restatement-2026-09-09.md` for 63 / 64 / 66 |
...
🟢 **REWRITTEN 2026-09-24 by the director. This ordering supersedes every list below it**, which is
kept as history. Twelve items are live; only one of them blocks a published number.

**A. The one decision waiting on the user.**

1. 🔴 **OPEN-65 (with OPEN-61 and OPEN-63 riding on it) — authorise a CPU-only re-parse of the
   adopted run.** Re-read all 8,139 `.sql` of Speed job `1315099` with production `parse_building()`,
   recompute the pooled EUI and the carbon total on the same 8,139, and bring the result to the user
   at an adoption checkpoint. No re-simulation, no geometry change, no new population. The subset
   says the answer will land near **+20 kWh/m²**, the same size OPEN-61's census measured in August.
   The same pass should decide whether the harvest scripts keep their own parser at all, and whether
   the `abups_ok` reconciliation gate (carried from OPEN-60) is wired, since it would have caught this.
   **Director recommends: yes, one plan, CPU only.** ⚠️ The TechTransfer arc builds its scenario
   baselines on the same T08 table (`scripts/analysis/fleet06a_campaign_2026-09-18.py:37`,
   `scripts/analysis/fleet06b_local_run_2026-09-18.py:74`) — tell it before it quotes a scenario
   difference against 153.95.

**B. Live, not blocking any published number — nothing owed until the user picks one up.**

2. **OPEN-66** — 8 IDFs flagged `success` with no `Zone` object. Small population; the real defect is
   the success flag. Cheap to diagnose (8 buildings, local, no cluster).
3. **OPEN-62** — what a storey IS for prototypes with attics, plenums and high bays. A definition,
   not a measurement; not an executor's question.
4. **OPEN-35** — which of the two storey fallbacks is right. The oldest undecided item.
5. **OPEN-03** — `layout_assign` vs `auto` load-source gap; measured. Its remedy was OPEN-18's
   territory, and OPEN-18 closed 2026-08-21. No action proposed.
6. **OPEN-19** — LA ~+40 % hot; needs a Title 24 table acquired or authored before it can be tested.
7. **OPEN-38** — 44 `layout_assign` fatals, 38 in one family. The cheap test (re-run a few with
   `Zone.Volume` written) is now free to run, since the volume fix is in production.
8. **OPEN-53** — re-fetch the 874 meter-only E02 files, or leave them. Custody only, no numbers.
9. **OPEN-58** — the scratch helper `open56_zone_volume_experiment.py` still has no isolated working
   directory; newer scripts use `run_ep_isolated()`. Blast radius measured; nothing published depends
   on it.
10. **OPEN-59** — Unknown buildings run 1.7× classified; a calibration question.
...
`MidriseApartment`) onto 8,153. ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations. **It sizes the exposure; it does not settle it. Do not publish this
...
adopted 153.8231 kWh/m²** — a pooled statistic over exactly this population. ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations. Precision *within* the
...
table above.** ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations.
...
8,153 — different row set, different provenance. ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations. **Do not restate the adopted figure.** See the
...
energy restatement that stands on its own. ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations.
...
**Evidence.** §5 fact F10 of `implemenation/previous/PLAN_open61-dh-remedy-2026-08-22.md`.
...
152.3011 → 171.7718.** ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations. This item changes nothing that has ever been published; it says that a
...
`implemenation/previous/PLAN_open61-dh-remedy-2026-08-22.md`; task T01b of the same plan.

### OPEN-17 — 🔒 CLOSED 2026-09-24 (director), on the user's rulings D-A and D-E of 2026-09-09

**Closes on:** T04 of `implemenation/PLAN_accuracy-restatement-2026-09-09.md`. The draw tier is wired
into the router (`openubem/semantic/imputation.py`, `_draw_tier` and its handler registration) and
the fifteen dormant tests are awake and pass (CP-1: suite +15 passed, −15 skipped, failures
unchanged). The user then ruled (D-E) that the signed-off deterministic storey ladder
(`openubem/semantic/building_classifier.py:145-155`) is **not** replaced by a random draw, so the
per-target fill count was withdrawn, not deferred. The plan's own closure line: *"OPEN-17 is
discharged in the only form the production build path allows."*

**What survives it.** (1) The draw tier is reachable only per call; `IMPUTE_DRAW_METHOD_BY_TARGET`
and `IMPUTE_DEBIAS_NEWERSKEW` in `openubem/config.py` default empty, so it fires on nothing in the
fleet build. (2) `IMPUTE_ENABLED_TIERS` stays `("fusion", "spatial", "statistical")` by contract —
the `ml` tier is out, and a live test pins that. (3) `openubem/results/impute_scatter.py:63` still
imports `recover_pairs`, which `openubem/validation/mask_recover.py` does not define — an orphaned
analysis module, recorded as `[OPEN]` in the debug references, not a production path.

### OPEN-56 — 🔒 CLOSED 2026-09-24 (director): the fix is inside the adopted figure

**Closes on:** three measurements, all from `implemenation/PLAN_accuracy-restatement-2026-09-09.md`.
T01 rebuilt three buildings at HEAD and found `Indicated Zone Volume` **0 / 0 / 0** times, with 11 / 11
zone volumes within 0.0002 % of floor area × ceiling height. The T06 addendum read every `Zone.Volume`
field of the rebuilt fleet: **8,152 / 8,160 IDFs positive on every zone, none autocalculated, none at
the 10 m³ stub.** T07 simulated exactly those IDFs, and the user adopted the result at CP-4. The
remedy the register once called unauthorised (`write_zone_volumes()`, `openubem/idf/builder.py`) has
therefore been applied to a published number, which is the closure OPEN-61's history taught this
register to wait for.

**What survives it.** (1) The eight IDFs with no `Zone` object at all — a different defect, now
OPEN-66. (2) The winding itself was never changed; geomeppy's `by_storey` path still extrudes an
unoriented footprint, and only the written volume masks it. (3) OPEN-56's own size (≈ +1 %) was
never re-measured cleanly — T01's per-building deltas mix every code change since run 4 and must not
be quoted as OPEN-56's effect. (4) The European arc was never exposed (0 of 81 preserved Bologna
runs carry the warning).

### OPEN-60 — 🔒 CLOSED 2026-09-24 (director): fixed in production, zero exposure in the adopted run

**Closes on:** `parse_eio_zone_multipliers()` (`openubem/results/parser.py`) scales each zone variable
by its multiplier since 2026-08-21, with tests in `tests/test_parser_open60_multiplier.py`; and the
adopted run is an `auto` build, which writes **no** zone multiplier above one (census of 8,160 IDFs in
August; T01 found none again). The adopted figure was never exposed.

**What survives it.** (1) The 6 / 48 reconciliation that first showed the defect was never re-measured
after the fix (those `.sql` were pruned) — it concerns `layout_assign` only. (2) **The decision on the
`abups_ok` integrity gate moves to OPEN-65**, where it would have caught a real, published miss.

### OPEN-64 — 🔒 CLOSED 2026-09-24 (director), fixed under the user's ruling D-C

**Closes on:** T03 of `implemenation/PLAN_accuracy-restatement-2026-09-09.md`. All fourteen ABUPS
district-heating rows fold into their own end-use columns (`openubem/results/parser.py`,
`_DISTRICT_HEATING_ROWS` and `_compute_eui`); the three golden expected values were raised with old
and new side by side in the progress log; fleet exposure re-measured at **0 of 8,152** buildings (no
district heat on any row other than Water Systems). T06 then charged every district-heat row at the
ruled carbon factor.

**What survives it.** Nothing on the fleet. ⚠️ **But the fix only helps a caller that uses the
production parser** — and the adopted figure was computed by one that does not (OPEN-65).

### OPEN-61 / OPEN-63 — amended 2026-09-24: the restatement that was meant to discharge them did not

The 2026-09-09 plan named its T08 as the task that discharges OPEN-61 (ruling D-D), and T06 applied
OPEN-63's factor in code. Both were expected to be finished by the adopted figure of 2026-09-10. **They
are not**, because T08 parsed the 8,139 results with a meter-only function copied from an older
harvest script, not with `parse_building()`. That function has no district-heating meter in its list
and never reads the ABUPS table, so both the adopted EUI and the adopted carbon total contain **no
district heat at all.** The code fixes are correct and sit unused on this number. Both items stay
live; OPEN-65 holds the mechanism and the remedy.

### OPEN-65 — OPENED 2026-09-24: the adopted figure was computed by a second parser that never reads district heating

**Found** by the director while refreshing this register, by reading the parser T08 cites
(`scripts/cluster/t08_harvest_results.py:163-214`) against what OPEN-61 and OPEN-64 had fixed.

**Measured, not estimated.** The T08 `.sql` files were not kept locally, but the TechTransfer arc
re-simulated the same 8,139 IDFs as its scenario baseline (`%TEMP%\ubem_validation\
fleet06b_local_2026-09-18\out\baseline\`). Identity check first: 40 random buildings re-parsed with
the T08 function reproduce the adopted per-building EUI to ≤ 0.0003 kWh/m² (39 / 40 exactly). Then,
over all 2,741 baseline `.sql` available on 2026-09-24:

- **2,736 of 2,741** carry a non-zero ABUPS `District Heating / Water Systems` cell; **no** district
  heat on any other row (OPEN-64's exposure stays zero).
- Pooled EUI on that subset: **159.27 kWh/m² as adopted → 179.24 kWh/m² with district heat
  (+19.97, +12.5 %)**. OPEN-61's August census measured 19.47 over 8,144 — the same size.
- **79 %** of the missing energy is in `TallBuilding` (44 %) and `SuperTallBuilding` (36 %); the
  per-building median is only 2.05 kWh/m² — the same concentration OPEN-61 recorded.
- Carbon at the ruled 0.226 kg/kWh: **≈ 53,400 t CO₂e** missing on the subset.
- ⚠️ The subset is 49.6 % of fleet floor area and NYC-heavy. **None of these is a fleet figure**, and
  none may be added to 153.95.

**Where else the same shape lives.** A look-alike `_parse_sql`, imported from
`t19_harvest_layout_assign`, is used by `scripts/analysis/b05e_measure_energy_delta.py:39`,
`b08b_measure_energy_delta.py:41` and `c01_storey_matching_regression.py:44`. Whether their results
are exposed depends on whether their buildings had district-heated hot water; not measured.

**Remedy (not authorised).** One CPU-only plan: re-parse all 8,139 `.sql` of job `1315099` with
`parse_building()` (fetch or run on Speed as an array — never on the login node), recompute EUI and
carbon on the same 8,139, bring it to an adoption checkpoint, then sweep. Decide at the same time
whether the harvest scripts may keep a parser of their own, and whether the `abups_ok` gate is
wired. **The lesson, third time in this arc:** a fix in the production parser does not reach a number
computed by a copy of it.

### OPEN-66 — OPENED 2026-09-24: eight IDFs flagged `success` contain no zone

**Found** 2026-09-09 by the T06 addendum of `implemenation/PLAN_accuracy-restatement-2026-09-09.md`,
which read every IDF rather than trusting `generation_status`. Excluded at CP-2 on the user's words
*"ok exclude and continue"*, so the adopted population is 8,152 → 8,139 and these 8 are not in it.
It never received an item; it has one now so it is not forgotten.

**Population.** All `perimeter_core`, 33–385 zones: `nyc_centre/way_266170758` (SecondarySchool),
`la_centre/way_425993506` (Courthouse), `la_urban/way_388772955` (LargeOffice),
`austin_urban/way_199742458` (Hospital), `austin_urban/way_381803065` (SmallOffice),
`austin_urban/way_381805563` (SmallOffice), `austin_urban/way_381810576` (LargeOffice),
`austin_urban/way_381810583` (MediumOffice). Each IDF carries People / Lights / Equipment /
Infiltration objects naming zones that do not exist, so EnergyPlus would fail at once.

**What is known and what is not.** Not caused by the 2026-09-09 code changes (none touch zoning).
The cause is not investigated. The defect that matters is the flag: a builder that reports
`success` for an IDF with no zones will hide the next one too. Remedy unproposed.
...
fact, not two.** ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations. Why a storey over-read and a simulation failure travel together is **untraced** and
...
- 🔴 **OPEN-53's second question is measured, and the director's audit overturned the executor's
  ...
  `LargeHotel`, `HighriseApartment` match **0 %**. ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations. The cause is the **elevator adder**:
...
by up to 0.18. ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations. **The director's own independent re-derivation during the CP-A audit produced
...
**the adopted baseline stands unchanged at 153.8231 pooled over 8,153** ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations. — but the adopted figure is
...
**The 153.8231 loose end is now a definite negative, not an open question.** ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations. T01 tested every row-set
...
sorted `osm_id` set. **Nothing on disk reproduces 153.8231.** ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations. The gap is 0.0073 kWh/m² (0.005 %) and
changes no conclusion anywhere. **The adopted baseline is unchanged at 153.8231 pooled over 8,153** ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations. —
...
stands.** ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations.
...
### Amendment — 2026-08-22 — `implemenation/previous/PLAN_open61-dh-remedy-2026-08-22.md` — **the remedy that was called a design question is decided**
...
run 4's over a different 8,153-building population. ⚠ superseded 2026-09-10: restated at 153.95 kWh/m² over 8,139 buildings (different population from 8,153 — see PLAN_accuracy-restatement-2026-09-09.md T08, CP-4). Do not diff the two numbers without stating both populations. **The two must never be differenced.**
...
### Amendment — 2026-09-09 — **four user rulings, two stale rows, and the first plan of this project authorised to re-simulate the fleet**

Plan now in force: `docs/docs_ACTIVE/openings/implemenation/PLAN_accuracy-restatement-2026-09-09.md`
(T01–T09, CP-1–CP-4, none executed at the time of writing). The predecessor
`PLAN_open61-dh-remedy-2026-08-22.md` was archived into `implemenation/previous/` earlier the same
day with its citation sweep run (**15 refs / 7 files: 9 path-qualified repaired, 4 bare-filename left
alone; plus 12 pre-existing dead citations in `.py` files, missed by every earlier sweep because
those swept `.md` only**).

🔴 **THE RULING THAT CHANGES THE SHAPE OF EVERY FUTURE PLAN.** The user was offered the low-risk
shape for **OPEN-17** — connect the imputation router but leave it switched off, so the 15 dormant
tests wake and no published number moves — and **chose the accurate shape instead**: *"you
recommend do it later, but we are not hurry, our focus is accuracy, so if we need to re-simulate lets
do it. i am okay with that."* **A full fleet re-simulation is authorised.** Every plan since
2026-08-19 was written around the assumption that it was not, and that constraint is now lifted.

**The other three rulings, taken the same day.** **OPEN-63** — *find one published
district-heating emission factor and use it*; the value is gathered by an executor with full
citations and **chosen by the director**, never invented. **OPEN-64** — *fix the code and raise
the reference values*; all fourteen ABUPS district-heating rows are folded into their own end-use
columns and the three golden expected values move upward, old and new recorded side by side.
**OPEN-61** — 🔴 **STAYS OPEN.** The director proposed closing it and the user ruled otherwise,
so the pending swap stays visible on the open list. **T08 of the new plan is what discharges it.**

⚠️ **TWO ROWS IN §1 ABOVE ARE STALE, AND THE ERROR IS THE SAME IN BOTH.** Both fixes landed during
the **European** arc and neither was carried back into this register.

- **OPEN-56** — the row reads *"a THIRD remedy shape now exists … Neither applied — the remedy
  ruling is still owed to the user."* **False.** `write_zone_volumes()` is defined at
  `openubem/idf/builder.py:217` and called unconditionally from the per-building orchestrator at
  `openubem/idf/builder.py:663`, with a regression test at `tests/test_eu_zone_volume_written.py`;
  introduced in commit `8d816be0`. Evidence at the artifact rather than in the source: across the
  **81** preserved Bologna EnergyPlus working directories the string `Indicated Zone Volume` appears
  **0 times**, against **8,160 / 8,160** in run 4. **What OPEN-56 owes is a re-run, not a fix.**
- **OPEN-60** — the row reads *"Remedy not proposed and not authorised."* **False.**
  `parse_eio_zone_multipliers()` is defined at `openubem/results/parser.py:416`, called at `:910` and
  consumed by `_compute_eui()` (`:493`, `:534-540`); tests at
  `tests/test_parser_open60_multiplier.py`. **What OPEN-60 owes is a re-parse, not a fix.**

🟢 **OPEN-62 is the only one of the three that is genuinely still open** — two analysis parsers
still read wall Z without adding `Z_Origin`
(`scripts/analysis/open03_storey_census_2026-08-20.py:138`, `:299`;
`scripts/analysis/open03_envelope_decomposition_2026-08-20.py:244`), while the correct handling has
existed in production all along at `openubem/geometry/layout_assigner.py:471` and `:491-493`.

**The lesson, and it is the second time this arc has paid for it.** *A fix that lands in one arc does
not update the item in another arc's register unless someone carries it across.* Both rows were read
as "unfixed" for eighteen days after the code had changed. **Before any plan is written on the
strength of a register row, the row's code claim is re-checked at the source.**

🟢 **Checked before the plan was written, because it would have changed its scope: no European
number moves.** Zone multipliers, a **census of all 4,171** recut IDFs — **zero** non-unity.
District heating, all **81** preserved Bologna `.sql` — **no `District*` end-use cell above zero
in any file**. Negative zone volumes, the same 81 — **none**. The four delivered European EUIs and
their layout side-cars are untouched and are explicitly out of scope. The partner session was told,
with the measurements rather than the assertion.

**Register unchanged by these rulings: 14 live / 50 retired / 64 total, next free `OPEN-65`.**

### Amendment — 2026-09-24 — **register refresh after a two-week gap: four closed, two opened, and the adopted figure is known low again**

Done by the director in one pass, on the user's standing instruction to read the register against
`docs/docs_EXPLANATION/` and bring both up to date. **12 live / 54 retired / 66 total, next free
`OPEN-67`.**

**Why the register was stale.** `implemenation/PLAN_accuracy-restatement-2026-09-09.md` finished on
2026-09-10 (CP-4 signed, 153.95 over 8,139 adopted), and its T09 swept only the old number — its own
entry says *"no separate remedy work was in scope for T09"*. So no row recorded what the plan had
actually closed. The same failure this register named on 2026-09-09 — *a fix in one plan does not
reach the register unless someone carries it* — happened again, one plan later.

**Closed (closing notes in §6):** OPEN-17 (rulings D-A / D-E, T04), OPEN-56 (volume fix inside the
adopted run), OPEN-60 (fixed, zero exposure in `auto`), OPEN-64 (ruling D-C, T03).

**Opened:** 🔴 **OPEN-65** — the adopted EUI and carbon total were parsed by a meter-only harvest
function that never reads district heating; measured +19.97 kWh/m² pooled on 2,741 of 8,139
buildings. This also means **OPEN-61 and OPEN-63 are not discharged**, contrary to the plan's
expectation; both stay live. **OPEN-66** — 8 IDFs flagged `success` with no zone, excluded at CP-2,
never given an item until now.

**Checked and unchanged:** OPEN-03, 19, 35, 38, 53, 58, 59 — no new evidence in the code or the
explanation docs since their last entries. OPEN-62's row gains T02's result (the fix was already
there; only the definition question remains).

**`docs/docs_EXPLANATION/` brought in line the same pass:** `OpenUBEM_debug_References.md` — a new
`[OPEN]` entry for OPEN-65 (§8), the §18 snapshot replaced (it listed the 2026-08-20 book-I state,
20 items, 17 of them since closed), OPEN-56 and OPEN-17 entries marked as since applied, and one dead
plan path repaired (`implemenation/` → `implemenation/previous/`). `OpenUBEM_fundamentals.md` —
OPEN-03's "roughly half" corrected to the measured ≈ 8 %; OPEN-18 marked closed-without-fix (also in
`Results/OpenUBEM_results_Resolution.md`, whose link to book I was dead and is repaired).

**Handoff owed to another arc.** The TechTransfer arc takes its baseline building list and EUIs from
`openubem/outputs/comparisons/t08_restated_fleet_eui_2026-09-10.csv`
(`scripts/analysis/fleet06a_campaign_2026-09-18.py:37`, `fleet06b_local_run_2026-09-18.py:74`). Any
scenario-versus-baseline difference it computes must parse both sides with `parse_building()`, or the
baseline side will be missing district heat that the scenario side has.

**Ruling owed by the user: one** — authorise the CPU-only re-parse (§4 A.1). Director recommends yes.
