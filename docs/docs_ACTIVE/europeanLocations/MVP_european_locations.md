...
| **EU-07** | **In progress -- three of four folds pinned.** The `BLOCKED_NO_CDS_CREDENTIALS` status was stale and false: credentials work and every archive below was acquired live. `fr` Lyon-Bron 2023, `es` Madrid 2010 and `uk` London 2015 are all `RULED_PINNED_EXCEPTION` -- six DR08 gates each, gate 5 carrying a documented winter-GHI exception (`fr` November 13.8 %; `es` January 14.1326 %; `uk` November 11.0776 % and December 10.6578 %, the last granted automatically under **D-EU-18** with an annual difference of 4.3116 %, the largest of the four). London's 2014 was **refused** by the same pre-authorisation on four counts, which is the first evidence that the seven bounds discriminate (S12.25). `it` Bologna 2014 is the only fold outstanding: ERA5 acquisition in progress, `RULED_NOT_PINNED`. Executable campaign cells stand at **300 of 510**. The licence provenance for all three pinned folds was found to be unevidenced and has been repaired -- see **FINDING EU-S2-11** (S12.26). |
