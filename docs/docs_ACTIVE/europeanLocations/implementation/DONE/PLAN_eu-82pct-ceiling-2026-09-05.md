...
#### Full harvest + 3D regen — completed — 2026-09-06

All 10 EU-11 Speed jobs (backlog x4, Step2-delta x4, FINDING-253-remedy x2) confirmed fully drained (last:
Bologna `1305186`, `1184 COMPLETED + 16 FAILED = 1200/1200`, 0 RUNNING/PENDING). Full detail, per-district
harvest numbers, and the 3D-regen diff findings are in
`prompts/DIRECTOR_PROMPT_eu82pct-ceiling-harvest_2026-09-05.md` §1/§4 (updated after each sub-step per
standing instruction) -- not duplicated here in full, only the closing numbers:

New script `scripts/cluster/harvest_eu11_ceiling82_final.py` pulled and merged all 3 waves per district
(precedence: FINDING-253-remedy overrides backlog for its 7 stems) against each district's
`<DISTRICT>_ceiling82_2026-09-05/` final-population snapshot:
...
**3,344 total run == stop point 3's own population figure (3,344/4,186 = 79.9%)** -- confirms the
final-population join is correct. **23 total FAILED == the 30 originally-classified (backlog wave) minus the
7 `FINDING 253` stems fixed by the remedy** -- fleet-level arithmetic checks out exactly; per-district
distribution shifted slightly from real population churn between the backlog snapshot and the final
`ceiling82` snapshot (documented mechanism, same as the Step 2 delta wave above), not a new failure. No
unclassified failure signature. Hard stop-condition (>10% measured-vs-predicted disagreement) not tripped
(23/3344 = 0.7%).

`scripts/generate_eu_3d_viewers.py` regenerated and mirrored for all 4 districts. `diff` against the
previously-committed mirror is not byte-clean (3 `buildings.csv` + all 4 `viewer.html` changed) but for a
benign, already-understood reason (stale energy columns dropped per the script's own geometry-only design,
`EU-18c`/`D-EU-59`) -- not a regression. One new finding recorded during the check: the "colour: EUI" button
added to the viewer template in a prior session's edit has no backing Python data (`b.eui` is never set) --
cosmetic gap, out of this arc's scope, not fixed here. Full detail in the director prompt §4.

**This closes the EU-11 ceiling82 harvest arc.** Owner report follows.
