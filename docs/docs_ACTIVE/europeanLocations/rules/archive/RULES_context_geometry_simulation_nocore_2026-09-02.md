...
> `RULES_context_geometry_simulation_2026-08-30.md`, which now lives in `rules/archive/`. This document rules
> EnergyPlus context shading and adiabatic party walls, unrelated to the dwelling-layout cutter, and is
> republished unchanged except for this header and the citation fix in section 4 below, which now points at the
> no-core dwelling-layout document.
...
- ⚪ It does **not** change the dwelling-layout scheme, which is `D-EU-39` / `EU-15` and, under the
  no-core regime ruled 2026-09-02, `EXAMPLE_dwelling_layout_validation_nocore_2026-09-02.md` (the original,
  `EXAMPLE_dwelling_layout_validation_2026-08-28.md`, now lives in `rules/archive/`).
