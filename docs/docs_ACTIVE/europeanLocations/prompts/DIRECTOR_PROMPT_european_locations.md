...
> - **When Bologna finishes, the remaining path is scripted and needs no ruling:** `scripts/convert_era5_eu_folds_to_epw.py --fold it --all-years` → `scripts/run_eu_t06_weather_promotion.py --fold it --dry-run` then `--commit` → fill `it`'s `licence_text_at_download` / `licence_status` from `openubem/outputs/eu_evidence/EU-07/cds_licence_text_at_download_2026-08-26.txt` (11,382 bytes, sha256 `57ab1c14…`) → freeze v1.0 with `scripts/freeze_eu_campaign_cell_spec.py` **without `--allow-unpinned`** → verify independently, expecting **`510 510 22 FROZEN`** → write `CLOSURE_eu_boundary_contract_v1.0.md` → sign MVP Table 26 and close §9.4.
> - **Campaign spec draft:** `openubem/data/campaign/eu_campaign_cell_spec_v1.0_DRAFT.json`, **510 cells** (102 archetypes × 5 `f` levels; es 120 / uk 180 / it 210), **22 caveats**, status `DRAFT_WEATHER_NOT_PINNED`, **300 `RULED_PINNED_EXCEPTION` / 210 `RULED_NOT_PINNED`**. France is the baseline, not a campaign fold.
> - **Suite baseline:** `pytest -q tests/` → **2,245 passed / 55 skipped**, 0 failed, ~26 min (2026-08-26 night). This is the number the closure document must record.
> - 🔴 **Expect the EU-S2-09 coupling to recur a third time at the `it` promotion.** Promoting a fold removes it from `load_fold_targets()` (which returns only `RULED_NOT_PINNED` folds by design), and it also creates files under the shipped `OUTPUT_ROOT`. It broke tests at the `es` promotion and again, in a different file, at the `uk` one (`tests/test_eu_fold_epw_conversion.py`). **Decouple the test — synthetic registry under `tmp_path`, `monkeypatch.setattr(module, "OUTPUT_ROOT", tmp_path / "out")` — never soften the assertion.**
...
> - **One housekeeping decision is OPEN and blocks nothing:** `D-SWEEP` — keep or revert 58 out-of-scope closed-arc documents whose citations a sweep rewrote inside commit `8d816be`. Measured: new paths resolve **61/64**, old paths **2/50**, so reverting re-breaks ~48 live citations. Record: `debugs/docs/DECISION_REQUEST_D-SWEEP_citation_sweep_scope_2026-08-26.md`.
> - **Do not disturb the running acquisition.** No `--poll`, no `--submit`, no second `--run-sequential`. Read progress by counting `openubem/data/weather/raw/era5_bologna_2013_2014/*.zip` and the `jobs/*.json` `status`/`finished` fields; read liveness from the process list. **Never infer death from a frozen log tail.**
>
