...
>
> **CURRENT BASELINE (adopted):** `phaseD2` metered PTAC HVAC + V16 reconstruction on **regional CBECS fractions** — zero fitted parameters; city-Overall ±9% all 3 cities (NYC +2.1 / LA −3.7 / Austin −8.6); national CBECS **NMBE + R² passing all 3 regions**. Phase-D arc COMPLETE (CP-8 setback + CP-2 regional, both user-ratified 2026-06-26); `REPORT_phaseD_final.md` regenerated. **All published figures in `openubem/outputs/` refreshed to the adopted model 2026-06-26** (65 PNGs). **Process note:** refreshing `openubem/outputs/` figures is now a required step after any model change.
>
> **🔬 IN FLIGHT — Phase-E "full realism" (supersedes phaseD2 once it lands):** physical archetype HVAC (central VAV/PSZ/FCU/WLHP → real fans+pumps) + physical DHW/cooking/refrigeration, reconstruction RETIRED. CP-A/CP-B/Phase-C cleared; CP-D NO-GO remediated. True B1 root cause = **kitchen-exhaust bug (CP-R1.9)**, fixed; + single-zone HVAC guard (CP-R1.8) + 2 B2 plumbing bugs found+fixed by the pilot. **CP-D2 PASSED 2026-06-27 → user ruled "Accept + fan out".** Honest scores: **CBECS NMBE −17.6%** (the CP-D −3.1% was a bug artifact), **R² 0.40→0.91**, EBEWE −8.6%, G2 PASS. Under-prediction = structural DOE-prototype-vs-CBECS office offset (not a bug; R6-4B residual; closing it = fitting → rejected). **🟢 T17 11-cell fan-out RUNNING** (bg `bb7vpmqix`, ~8h, NO monitoring). **T18 final report HELD for user** — see §E.
...
## E. Phase-E "full realism" — IN FLIGHT
Plan: `docs/docs_ACTIVE/hvac-ServiceLoads/PLAN_phaseE_full_realism.md`. CP-D ruling: `RESULT_phaseE_CPD_gonogo.md`.
...
- [~] **T17 11-cell fan-out — LAUNCHED 2026-06-27** (bg `bb7vpmqix`, `scratchpad/phaseE_fanout.sh`, log `phaseE_fanout.log`). All 12 matrix cells EXCEPT la_urban, sequential (one SLURM array at a time), reconstruction OFF, ~8h, **NO monitoring** (user). Both B2 fixes in the run_cell path; cells are fresh (no staging poison). A cell hard-stop doesn't abort the loop.
- [ ] **T18** re-score 3 cities + national CBECS + refresh `openubem/outputs/` figures + `REPORT_phaseE_final.md` (CP-E final audit) — **HELD for user**; only after fan-out completes. Lead narrative: R²≈0.9 + real-data city anchors; report CBECS NMBE residual honestly.
- [x] **REMEDIATION Phase R1 — CP-R1 AUDITED + ACCEPTED 2026-06-27.** Plan: `PLAN_phaseE_CPD_remediation.md` (rulings in §9). R1 delivered: SAT reset (RD2) on VAV/PVAV/CRAH (regression-clean), B2 logged-drop tolerance, G2 per-archetype gate, W1 confirmed. B1 reframed to geometry defect (see above). Manager rulings D1–D4: D1 RD3 dead/0.30 locked; D2 geometry-screen folded into B2 drops; D3 G2 verdict = gates (a)+(b) only, gate (c) report-only; D4 don't block on 2 pre-existing fails BUT fix `dhw.py:50` no-zone crash + **verify `total_eui` doesn't double-count fans (STOP P1 if it does)**.
