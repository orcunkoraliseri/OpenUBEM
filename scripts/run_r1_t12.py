"""R1-T12: Step-5 results regeneration against the merged R1 fleet manifest.

Adapted from scripts/run_t12_boston.py — uses the merged 483-building manifest
from ubem_boston_r1 as the source of truth. sql_paths span both ubem_boston_c4/sim
and ubem_boston_r1/sim; aggregate_results needs no awareness of the split.
"""
...
R1_BASE = Path(tempfile.gettempdir()) / "ubem_boston_r1"
C4_BASE = Path(tempfile.gettempdir()) / "ubem_boston_c4"
SIM_DIR_R1 = R1_BASE / "sim"
STEP3_DIR_R1 = R1_BASE / "step3"
STEP3_DIR_C4 = C4_BASE / "step3"

MERGED_MANIFEST = R1_BASE / "04_simulation_manifest_merged.parquet"
OUT_DIR = Path(tempfile.gettempdir()) / "ubem_boston_r1_results"
OUT_DIR.mkdir(exist_ok=True)

print(f"[R1-T12] Merged manifest: {MERGED_MANIFEST}")
print(f"[R1-T12] Output dir: {OUT_DIR}")

sim_mf = pd.read_parquet(MERGED_MANIFEST)
print(f"[R1-T12] Sim manifest: {len(sim_mf)} rows, status counts: {sim_mf['status'].value_counts().to_dict()}")

# Build merged IDF manifest: C4 base + R1 overrides for the 8
TARGET_OSM_IDS = [
    "29716487", "240391795", "241978446", "1281831066",
    "29573070", "241186243", "788015166", "458718877",
]
idf_mf_c4 = pd.read_parquet(STEP3_DIR_C4 / "03_idf_manifest.parquet")
idf_mf_r1 = pd.read_parquet(STEP3_DIR_R1 / "03_idf_manifest.parquet")

idf_mf_c4_clean = idf_mf_c4[~idf_mf_c4["osm_id"].astype(str).isin(TARGET_OSM_IDS)].copy()
idf_mf = pd.concat([idf_mf_c4_clean, idf_mf_r1], ignore_index=True)
print(f"[R1-T12] IDF manifest: {len(idf_mf)} rows (C4 base={len(idf_mf_c4_clean)}, R1 fresh={len(idf_mf_r1)})")
...
print("[R1-T12] Building enriched GDF from SQL Zones tables...")
t_gdf = time.monotonic()
enriched_gdf = _build_enriched_gdf(idf_mf, sim_mf)
print(f"[R1-T12] Enriched GDF: {enriched_gdf.shape} ({time.monotonic()-t_gdf:.1f}s)")
...
print(f"[R1-T12] aggregate_results completed in {wall:.1f}s")
...
if abups_results:
    ...
    print("ABUPS: no TabularDataWithStrings in SQL (check skipped)")
if meter_results:
