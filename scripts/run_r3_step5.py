"""C09: Step-5 aggregate_results on the C08 cluster simulation manifest.

Input:  TEMP/ubem_boston_r3/04_simulation_manifest.parquet  (from C08 adapter)
        TEMP/ubem_boston_r3/step3/03_idf_manifest.parquet   (from C07 generation)
Output: TEMP/ubem_boston_r3_results/
"""
...
STEP3_DIR = R3_BASE / "step3"
SIM_MANIFEST_PATH = R3_BASE / "04_simulation_manifest.parquet"
IDF_MANIFEST_PATH = STEP3_DIR / "03_idf_manifest.parquet"
RESULTS_DIR = Path(tempfile.gettempdir()) / "ubem_boston_r3_results"
RESULTS_DIR.mkdir(exist_ok=True)

print(f"[C09] Sim manifest:  {SIM_MANIFEST_PATH}")
print(f"[C09] IDF manifest:  {IDF_MANIFEST_PATH}")
print(f"[C09] Results dir:   {RESULTS_DIR}")

sim_mf = pd.read_parquet(SIM_MANIFEST_PATH)
idf_mf = pd.read_parquet(IDF_MANIFEST_PATH)

print(f"[C09] sim manifest: {len(sim_mf)} rows, status counts: {sim_mf['status'].value_counts().to_dict()}")
print(f"[C09] idf manifest: {len(idf_mf)} rows")

_floor_rx = _re.compile(r"_F(\d+)_")


def _build_enriched_gdf(idf_mf_: pd.DataFrame, sim_mf_: pd.DataFrame) -> gpd.GeoDataFrame:
    rows = []
    for _, idf_row in idf_mf_.iterrows():
        osm_id = str(idf_row["osm_id"])
        sim_row = sim_mf_[sim_mf_["osm_id"].astype(str) == osm_id]
...
print("[C09] Building enriched GDF from SQL Zones tables...")
t_gdf = time.monotonic()
enriched_gdf = _build_enriched_gdf(idf_mf, sim_mf)
print(f"[C09] Enriched GDF: {enriched_gdf.shape} ({time.monotonic()-t_gdf:.1f}s)")

if "csv_path" not in sim_mf.columns:
    sim_mf = sim_mf.copy()
    sim_mf["csv_path"] = None

print("[C09] Step 5: aggregate_results...")
t0 = time.monotonic()
...
print(f"[C09] aggregate_results completed in {wall_step5:.1f}s")
...
if len(failed_parse) > 0:
    print(f"  Examples: {list(failed_parse['osm_id'].head(5))}")

print("\n[C09] Gas/ABUPS/meter check on 20 sample buildings...")
...
print(f"NaturalGas:Facility = 0 ({checked} checked): {gas_nonzero} non-zero (PASS={gate_gas})")
if abups_results:
...
print("\n=== CBECS GATES: R3 (cluster) vs BASELINE (R2) ===")
...
print(f"\n[C09] step5 wall clock: {wall_step5:.1f}s")
print("[C09] DONE")
