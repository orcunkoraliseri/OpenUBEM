"""T17 — Full 12-cell layout_assign cluster sweep: Steps 1-3 locally + fire-and-forget sbatch.

Minimal derivative of t08_full_sweep.py (PLAN §5 T17: "Copy t08_full_sweep.py minimally
... change only what's needed to run a single mode (layout_assign) instead of iterating
ALL_MODES, keep the sbatch header/array sizing/node config/fleet packaging logic
byte-for-byte identical.").

Diffs vs. t08_full_sweep.py (all documented in the T17 §8 progress-log entry):
  1. ALL_MODES -> single-element ["layout_assign"] (was the 4 standard modes).
  2. _FLEET_TAG "t08" -> "t17" (remote dir / job-name prefix only; the sbatch
     ...
     task's remote fleet dirs with T08's already-harvested ones; it does not
     change array sizing/node config/packaging mechanics in any way).
  3. work_base tmp dir "ubem_t08_sweep" -> "ubem_t17_sweep" (own job-ids file,
     does not collide with/overwrite T08's own sweep state).
  4. Runs Steps 1-3 for ALL 12 cells (same CELL_CONFIGS dict, byte-for-byte).
...
The _verify_orient_gate() check (unchanged from T08) still applies: it asserts
`if self.resolution_mode != "auto":` guards the orient() call in builder.py.
That call happens BEFORE the layout_assign branch is taken (builder.py:432-436)
and operates on the footprint polygon consumed by build_zones()/decide_zoning_
strategy(), independent of the baseline-IDF scaling path -- i.e. it is a
general non-auto-mode regression guard, equally applicable and safe for
layout_assign as for building/floor/fast_zone. Kept unchanged, not layout_assign-
specific, no modification needed.

After cluster completion, check with:
    ssh o_iseri@speed.encs.concordia.ca "squeue -u o_iseri"
Then harvest:
    py -3 scripts/cluster/t17_harvest_layout_assign.py

Usage (local machine only):
    py -3 scripts/cluster/t17_layout_assign_full_sweep.py
    py -3 scripts/cluster/t17_layout_assign_full_sweep.py --cells la_rural   (subset for testing)
"""
...
# Identical to t08_full_sweep.py's CELL_CONFIGS -- byte-for-byte.
...
def run_step3_mode(gdf: gpd.GeoDataFrame, schedule_lib: object,
    ...
    manifest = run_step3(
        ...
        trim_outputs=True,   # same as T08: skip hourly zone variables to keep SQL small
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="T17 full layout_assign x 12-cell sweep")
    ...
    work_base = Path(tempfile.gettempdir()) / "ubem_t17_sweep"
    work_base.mkdir(parents=True, exist_ok=True)

    print("=" * 72)
    print(f"T17 layout_assign full sweep — {len(cells)} cells x {len(modes)} mode(s)")
    ...
    job_id_path = work_base / "t17_job_ids.json"
    ...
    print("\n" + "=" * 72)
    print(f"T17 FIRE-AND-FORGET SUBMISSION COMPLETE ({elapsed/60:.1f} min total)")
    print(f"  Work dir:     {work_base}")
    ...
    print("  py -3 scripts/cluster/t17_harvest_layout_assign.py")
    print("=" * 72)
