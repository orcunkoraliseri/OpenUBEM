"""T18 — Harvest layout_assign cluster results, post-debug-fix.

Run AFTER all 12 sbatch arrays (12 cells x layout_assign) submitted by
t18_layout_assign_full_sweep.py have completed on Speed. Fetches SQL+end+err
from each (cell, layout_assign) fleet, parses the 9-end-use EUI, and writes the
fleet-median table -- same meter-based methodology as T17's harvest, applied to
the T18 (post-debug-fix) fleet so a real before/after comparison is possible.

Diffs vs. t17_harvest_layout_assign.py (all documented here, per plan §5 T11 /
hard rule 2 -- never overwrite T17's original harvest artifacts):
  1. _FLEET_TAG "t17" -> "t18" (reads from the T18 remote fleet dirs that
     t18_layout_assign_full_sweep.py shipped to -- distinct from T17's, so this
     harvest cannot accidentally read T17's already-fetched output).
  2. OUTPUT_ALL_CSV / OUTPUT_SUMMARY_CSV -> t18_layout_assign_eui.csv /
     t18_layout_assign_cell_summary.csv (NEW filenames; T17's
     t17_layout_assign_eui.csv / t17_layout_assign_cell_summary.csv are never
     written to by this script).
  3. work_base tmp dir "ubem_t17_harvest" -> "ubem_t18_harvest"; sweep_work
     (for reading the local manifest, if present) "ubem_t17_sweep" ->
     "ubem_t18_sweep"; job_id_file "t17_job_ids.json" -> "t18_job_ids.json".
  4. _parse_sql/_METER_NAMES/_METER_SQL/_ZONE_SQL, parse_cell, build_cell_info:
     byte-for-byte identical to T17 -- same meter-based, mode-agnostic parser,
     proven immune to the E-LA-05 zone-naming mismatch (see T17's own docstring
     for the full rationale, unchanged and still applicable here).

METHODOLOGY NOTE (carried forward verbatim from T17's own docstring, still
...
Usage:
    py -3 scripts/cluster/t18_harvest_layout_assign.py [--cells ...]

Outputs:
    openubem/outputs/comparisons/t18_layout_assign_eui.csv
    openubem/outputs/comparisons/t18_layout_assign_cell_summary.csv
"""
...
OUTPUT_ALL_CSV = OUTPUT_DIR / "t18_layout_assign_eui.csv"
OUTPUT_SUMMARY_CSV = OUTPUT_DIR / "t18_layout_assign_cell_summary.csv"

J_TO_KWH = 1.0 / 3.6e6

# Identical meter set to t17_harvest_layout_assign.py -- write_outputs() (openubem/idf/
...
def main() -> None:
    parser = argparse.ArgumentParser(description="T18 harvest: fetch + parse layout_assign (post-debug-fix)")
    ...
    work_base = Path(tempfile.gettempdir()) / "ubem_t18_harvest"
    work_base.mkdir(parents=True, exist_ok=True)
    print(f"T18 harvest — work dir: {work_base}")
    print(f"  Cells: {args.cells}")

    local_work = Path(args.local_work_dir) if args.local_work_dir else work_base

    sweep_work = Path(tempfile.gettempdir()) / "ubem_t18_sweep"
    job_id_file = sweep_work / "t18_job_ids.json"
    ...
    # E-LA-14 prevalence flag (plan §5 T11 point 8 / CP-D directive): report
    # CheckWarmupConvergence Severe prevalence in this fleet-scale harvest.
    if "n_warmup_convergence" in results.columns:
        print("\n### E-LA-14 CheckWarmupConvergence prevalence (fleet-scale, this harvest's raw .err) ###")
    ...
    print(f"\nRun date: {run_date}")
    print("Provenance: T18 full cluster sweep (post-debug-fix), Speed/Linux, EnergyPlus 23.1, trim_outputs=True")
