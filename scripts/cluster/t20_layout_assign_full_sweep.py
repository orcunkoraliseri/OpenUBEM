"""T20 — R06: full 12-cell / 8,160-building layout_assign fleet re-run, post
storey-matching REMAINder fixes R01+R02+R03+R10 (Steps 1-3 locally + fire-and-forget sbatch).

Minimal derivative of t19_layout_assign_full_sweep.py (same "minimal derivative"
pattern T17/T18/T19 each used forking from their predecessor): reuse T19's
proven infrastructure byte-for-byte, changing only the fleet tag / output paths
so this run's job-ids file and remote fleet dirs don't collide with T17's/T18's/
T19's already-completed ones.

Diffs vs. t19_layout_assign_full_sweep.py:
  1. _FLEET_TAG "t19" -> "t20" (remote dir / job-name prefix only; the sbatch
     ...
     task's remote fleet dirs with T17's/T18's/T19's already-harvested ones; it
     does not change array sizing/node config/packaging mechanics in any way).
  2. work_base tmp dir "ubem_t19_sweep" -> "ubem_t20_sweep" (own job-ids file,
     does not collide with/overwrite T17's/T18's/T19's own sweep state).
  3. job_id_path filename "t19_job_ids.json" -> "t20_job_ids.json".
  4. CELL_CONFIGS, ALL_MODES (single-element ["layout_assign"]), _verify_orient_gate(),
     ...
     to T19 -- this run applies the storey-matching REMAINder plan's R01 (E-LA-35
     Cause A, ZoneGroup-aware compute_band_map()), R02 (E-LA-35 Cause B,
     WaterUse:Equipment/People double-count carve-out), R03 (E-LA-32, PV scaled by
     roof area not area_scale_ratio) and R10 (E-LA-36, match_storeys() solves for
     the residual multiplier instead of compounding it) fixes, which are already
     live in the imported openubem package (layout_assigner.py) -- no script-level
     change needed to pick them up.

This run is against the SAME 12 cells / SAME 8,160-building fleet as T17/T18/T19,
with R01+R02+R03+R10 applied on top of every prior CP-D-signed fix. E-LA-21,
E-LA-22, E-LA-23, E-LA-24, E-LA-33 remain OPEN/carried-forward and are NOT
addressed by this run -- this sweep does not change layout_assigner.py/builder.py/
envelope_patcher.py, it only re-runs the pipeline with the already-committed R01-
R03+R10 code. Per the REMAINder plan's framing constraint, a clean T20-vs-T19
delta is NOT available while E-LA-22 stands; report T20's own measurements, not
deltas attributed to this arc.
...
Then harvest:
    py -3 scripts/cluster/t20_harvest_layout_assign.py

Usage (local machine only):
    py -3 scripts/cluster/t20_layout_assign_full_sweep.py
    py -3 scripts/cluster/t20_layout_assign_full_sweep.py --cells la_rural   (subset for testing)
"""
...
# Identical to t18_layout_assign_full_sweep.py's CELL_CONFIGS -- byte-for-byte.
...
def run_step3_mode(gdf: gpd.GeoDataFrame, schedule_lib: object,
    ...
    manifest = run_step3(
        ...
        trim_outputs=True,   # same as T17/T18: skip hourly zone variables to keep SQL small
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="T20 full layout_assign x 12-cell sweep (post-structural-fixes)")
    ...
    work_base = Path(tempfile.gettempdir()) / "ubem_t20_sweep"
    work_base.mkdir(parents=True, exist_ok=True)

    print("=" * 72)
    print(f"T20 layout_assign full sweep (post-structural-fixes) — {len(cells)} cells x {len(modes)} mode(s)")
    ...
    job_id_path = work_base / "t20_job_ids.json"
    ...
    print("\n" + "=" * 72)
    print(f"T20 FIRE-AND-FORGET SUBMISSION COMPLETE ({elapsed/60:.1f} min total)")
    print(f"  Work dir:     {work_base}")
    ...
    print("  py -3 scripts/cluster/t20_harvest_layout_assign.py")
    print("=" * 72)
