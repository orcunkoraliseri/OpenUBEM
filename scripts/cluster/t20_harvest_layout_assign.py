"""T20 — Harvest layout_assign cluster results, post-structural-fixes.

Run AFTER all 12 sbatch arrays (12 cells x layout_assign) submitted by
t20_layout_assign_full_sweep.py have completed on Speed. Fetches SQL+end+err
...
applied to the T20 (post-structural-fixes) fleet so a real 3-way T17->T18->T20
...
artifacts):
  1. _FLEET_TAG "t18" -> "t20" (reads from the T20 remote fleet dirs that
     t20_layout_assign_full_sweep.py shipped to -- distinct from T17's/T18's, so
     this harvest cannot accidentally read T17's/T18's already-fetched output).
  2. OUTPUT_ALL_CSV / OUTPUT_SUMMARY_CSV -> t20_layout_assign_eui.csv /
     t20_layout_assign_cell_summary.csv (NEW filenames; T17's/T18's own
     t17_layout_assign_eui.csv/t18_layout_assign_eui.csv and their
     cell_summary siblings are never written to by this script).
  3. work_base tmp dir "ubem_t18_harvest" -> "ubem_t20_harvest"; sweep_work
     (for reading the local manifest, if present) "ubem_t18_sweep" ->
     "ubem_t20_sweep"; job_id_file "t18_job_ids.json" -> "t20_job_ids.json".
...
Usage:
    py -3 scripts/cluster/t20_harvest_layout_assign.py [--cells ...]
...
OUTPUT_ALL_CSV = OUTPUT_DIR / "t20_layout_assign_eui.csv"
OUTPUT_SUMMARY_CSV = OUTPUT_DIR / "t20_layout_assign_cell_summary.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="T20 harvest: fetch + parse layout_assign (post-structural-fixes)")
    ...
    work_base = Path(tempfile.gettempdir()) / "ubem_t20_harvest"
    work_base.mkdir(parents=True, exist_ok=True)
    print(f"T20 harvest — work dir: {work_base}")
    ...
    job_id_file = sweep_work / "t20_job_ids.json"
    ...
    print(f"\nRun date: {run_date}")
    print("Provenance: T20 full cluster sweep (post-structural-fixes), Speed/Linux, EnergyPlus 23.1, trim_outputs=True")
