"""V12 la_suburban recovery: main array 965462 submitted headless (ssh dropped
the sbatch response), 1342/1343 COMPLETED, 1 FAILED.

Failed building: way/442763908 (RetailStandalone, perimeter_core, 38 zones) —
E+ Fatal, 3 Severe "Vertex size mismatch" (6 vs 7) on core ceiling/floor
interzone pairs. Same root-cause class as la_urban/la_centre perimeter_core
failures. Fix: single_zone fallback.

This script:
  1. Regenerates way_442763908 with single_zone, ships, submits a 1-task array,
     polls to terminal, fetches by explicit path.
  2. Bulk-fetches the other 1342 buildings (cluster-side tgz + scp; 17 GB out/).
  3. Updates IDF manifest, builds sim manifest (1343/1343), runs Step 5,
     writes gates report + deliverables.

Working dir: %TEMP%/ubem_validation/cases/la_suburban/ (raw from headless run).
"""
...
REPAIR_DIR_REMOTE = REMOTE_FLEET_DIR + "_repair"
...
MAIN_JOB_ID = "965462"
PROBE_COUNT = 1054
EPW_STATION = "Torrance.Muni.AP-Zamperini.Field.722955"

REPAIR_BUILDINGS = [
    {"osm_id_raw": "way/442763908", "osm_id_stem": "way_442763908"},
]
...
def regenerate_idf(osm_id_raw: str, osm_id_stem: str, epw_path: Path,
                   gdf_raw: gpd.GeoDataFrame) -> Path:
...
def upload_and_submit(idf_paths: list[Path], epw_path: Path) -> str:
    print(f"[repair] Creating remote repair dir {REPAIR_DIR_REMOTE} ...")
    _ssh(f"mkdir -p {REPAIR_DIR_REMOTE}/idfs {REPAIR_DIR_REMOTE}/weather {REPAIR_DIR_REMOTE}/out")

    # fleet.lst: Unix LF via write_bytes locally, upload via scp (NOT cat > pipe).
    fleet_local = STEP3_DIR / "repair_fleet.lst"
    fleet_local.write_bytes(("\n".join(p.stem for p in idf_paths) + "\n").encode("utf-8"))
    subprocess.run(["scp", str(fleet_local), f"{REMOTE_HOST}:{REPAIR_DIR_REMOTE}/fleet.lst"],
                   check=True, timeout=30)
    print(f"  fleet.lst uploaded (scp): {[p.stem for p in idf_paths]}")

    print(f"[repair] Uploading {len(idf_paths)} IDFs (scp) ...")
    for idf_p in idf_paths:
        subprocess.run(["scp", str(idf_p), f"{REMOTE_HOST}:{REPAIR_DIR_REMOTE}/idfs/"],
                       check=True, timeout=60)
    print(f"  IDFs uploaded.")
...
#SBATCH --output={REPAIR_DIR_REMOTE}/repair_%A_%a.log
...
exit $RC
"""
    sbatch_remote_path = f"{REPAIR_DIR_REMOTE}/repair_array.sbatch"
    # write sbatch via scp (LF) to avoid cat> pipe drop on tcsh login shell
    sb_local = STEP3_DIR / "repair_array.sbatch"
    sb_local.write_bytes(sbatch_script.encode("utf-8").replace(b"\r\n", b"\n"))
    subprocess.run(["scp", str(sb_local), f"{REMOTE_HOST}:{sbatch_remote_path}"],
                   check=True, timeout=30)

    out = _ssh(f"sbatch --array=1-{n_tasks}%32 {sbatch_remote_path}", timeout=60)
...
def poll_cluster(job_id: str, poll_interval_s: int = 30) -> None:
    print(f"[repair] Polling job {job_id} (every {poll_interval_s}s) ...")
    while True:
...
def fetch_repaired(buildings: list[dict]) -> None:
    print(f"[repair] Fetching {len(buildings)} repaired outputs by explicit path ...")
    for b in buildings:
        stem = b["osm_id_stem"]
        ...
        if stderr_data:
            ...
            if msg:
                print(f"  {stem} fetch stderr: {msg[:300]}")
        assert stdout_data, f"Empty fetch response for {stem}"
        ...
        err_path = SIM_OUT_DIR / stem / "eplusout.err"
        n_severe = 0
        if err_path.exists():
            etxt = err_path.read_text(errors="replace")
            matches = _re.findall(r"(\d+)\s+Warning;\s*(\d+)\s+Severe", etxt)
            if matches:
                n_severe = int(matches[-1][1])
        print(f"  {stem} n_severe={n_severe}")

        if "EnergyPlus Completed Successfully" not in end_text:
            print(f"  ERROR: {stem} still failed after single_zone repair: {end_text.strip()}", file=sys.stderr)
            sys.exit(2)
        if n_severe > 0:
            print(f"  WARNING: {stem} has {n_severe} Severe — STOP", file=sys.stderr)
            sys.exit(2)
        print(f"  PASS: {stem} EnergyPlus Completed Successfully, 0 Severe")


def bulk_fetch_main(exclude_stems: set[str], expected_total: int) -> None:
    """Fetch all main-array outputs except the excluded (repaired) buildings.
    If sim_out already holds the full fleet (e.g. harness pre-extracted the tgz),
    skip. Otherwise scp the pre-built cluster-side tgz down and extract."""
    n_have = len(list(SIM_OUT_DIR.glob("*/eplusout.end")))
    if n_have >= expected_total:
        print(f"[bulk] sim_out already has {n_have}/{expected_total} .end files; skipping fetch.")
        return

    tgz_local = WORK_BASE / "sim_out_fetch.tgz"
    if not tgz_local.exists():
        tgz_remote = f"{REMOTE_FLEET_DIR}/sim_out_fetch.tgz"
        print(f"[bulk] scp {tgz_remote} -> {tgz_local} ...")
        t0 = time.monotonic()
        subprocess.run(["scp", f"{REMOTE_HOST}:{tgz_remote}", str(tgz_local)],
                       check=True, timeout=3600)
        print(f"  downloaded {tgz_local.stat().st_size/1e6:.1f} MB in {time.monotonic()-t0:.0f}s")

    SIM_OUT_DIR.mkdir(parents=True, exist_ok=True)
    print(f"[bulk] extracting {tgz_local} ...")
    with tarfile.open(tgz_local, mode="r:gz") as tf:
        tf.extractall(str(SIM_OUT_DIR))
    tgz_local.unlink()
    n_end = len(list(SIM_OUT_DIR.glob("*/eplusout.end")))
    print(f"[bulk] extracted: {n_end} eplusout.end files in sim_out")


def update_idf_manifest(buildings: list[dict]) -> pd.DataFrame:
    mf = pd.read_parquet(STEP3_DIR / "03_idf_manifest.parquet")
    for b in buildings:
        row_idx = mf.index[mf["osm_id"].astype(str) == b["osm_id_raw"]]
        assert len(row_idx) == 1, f"Expected 1 row for {b['osm_id_raw']}"
        i = row_idx[0]
        idf_path = STEP3_DIR / "idfs" / f"{b['osm_id_stem']}.idf"
    ...
    print(f"[repair] IDF manifest updated: {len(mf)} rows, "
          f"success={(mf['generation_status']=='success').sum()}")
    return mf


def verify_all_success(idf_manifest: pd.DataFrame) -> None:
    success_rows = idf_manifest[idf_manifest["generation_status"] == "success"]
    stems = [Path(str(r["idf_path"])).stem for _, r in success_rows.iterrows()]
    failed = []
    for stem in stems:
        end_p = SIM_OUT_DIR / stem / "eplusout.end"
        if not end_p.exists():
            failed.append((stem, "missing .end"))
            continue
        txt = end_p.read_text(errors="replace")
        if "EnergyPlus Completed Successfully" not in txt:
            failed.append((stem, txt.strip()[:80]))
    print(f"[verify] {len(stems) - len(failed)}/{len(stems)} success")
    if failed:
        for name, msg in failed[:50]:
            print(f"  FAIL: {name}: {msg}", file=sys.stderr)
        print(f"[verify] ZERO-FAIL VIOLATION: {len(failed)} failed", file=sys.stderr)
        sys.exit(2)
    print(f"[verify] ZERO-FAIL: all {len(stems)} buildings completed successfully")
...
def build_sim_manifest(idf_manifest: pd.DataFrame, epw_path: Path) -> pd.DataFrame:
    ...
    print(f"[sim] Sim manifest: {len(sim_mf)} rows, status={status_counts}")
    return sim_mf
...
def run_step5(idf_manifest: pd.DataFrame, sim_mf: pd.DataFrame,
    ...
    print(f"[step5] aggregate_results ...")
    t0 = time.monotonic()
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
    ...
    print(f"\n[{CELL_NAME}] F12 GATE SUMMARY ({n_gen}/{n_total}):")
    ...
    lines = [
        "=" * 72,
        f"V12 {CELL_NAME.upper()} GATES REPORT ({n_gen}/{n_total} — 1 single_zone repair)",
        ...
        "",
        "=== SIMULATION STATUS ===",
        f"  cluster_job_ids: {MAIN_JOB_ID} (main, {n_sim_success-len(REPAIR_BUILDINGS)}/{n_total}) + {repair_job_id} (repair, {len(REPAIR_BUILDINGS)}/{len(REPAIR_BUILDINGS)})",
        f"  sim_manifest_rows: {len(sim_mf)}",
        f"  status_counts: {sim_mf['status'].value_counts().to_dict()}",
        "",
        "=== REPAIR RECORD ===",
        f"  way/442763908: perimeter_core vertex size mismatch (3 Severe, 6 vs 7 core ceiling/floor) -> single_zone, job {repair_job_id}_1, 0 Severe",
        "",
        "=== F12 GATE TABLE ===",
    ...
    print(f"[step5] Gates report -> {report_path}")

    print(f"[step5] Copying final deliverables -> {FINAL_DIR}")
    ...
    return results_gdf, cbecs_gates, summary


def clean_cluster_out() -> None:
    out = _ssh(f"rm -rf {REMOTE_FLEET_DIR}/out {REPAIR_DIR_REMOTE}/out 2>/dev/null && echo CLEANED || echo FAILED", timeout=120)
    print(f"[clean] Cluster out/ clean: {out.strip()}")


def move_to_runtime() -> None:
    runtime_dest = REPO / "runtime" / "ubem_validation" / "cases" / CELL_NAME
    if runtime_dest.exists():
        print(f"[runtime] dest exists, removing: {runtime_dest}")
        shutil.rmtree(str(runtime_dest))
    runtime_dest.parent.mkdir(parents=True, exist_ok=True)
    # Drop bulky fleet_staging before move (it duplicates step3/idfs)
    staging = WORK_BASE / "fleet_staging"
    if staging.exists():
        shutil.rmtree(str(staging))
    print(f"[runtime] Moving {WORK_BASE} -> {runtime_dest} ...")
    shutil.move(str(WORK_BASE), str(runtime_dest))
    print(f"  Done: {runtime_dest}")


def main() -> None:
    print(f"[recover] V12 la_suburban recovery (main job {MAIN_JOB_ID}, 1 single_zone repair)")
    print(f"[recover] Working dir: {WORK_BASE}")
    ...
    print(f"[recover] EPW: {epw_path.name}")

    gdf_raw = gpd.read_file(str(WORK_BASE / "01_buildings.gpkg"))
    print(f"[recover] Loaded {len(gdf_raw)} buildings from 01_buildings.gpkg")

    # 1-2. Repair: skip if already complete (idempotent re-entry).
    job_id = "967407"  # repair array already submitted+completed on first attempt
    already_done = all(
        (SIM_OUT_DIR / b["osm_id_stem"] / "eplusout.end").exists()
        and "EnergyPlus Completed Successfully"
        in (SIM_OUT_DIR / b["osm_id_stem"] / "eplusout.end").read_text(errors="replace")
        and (STEP3_DIR / "idfs" / f"{b['osm_id_stem']}.idf").exists()
        for b in REPAIR_BUILDINGS
    )
    if already_done:
        print(f"[recover] Repair already complete (job {job_id}); skipping regen/submit/fetch.")
    else:
        idf_paths = []
        for b in REPAIR_BUILDINGS:
            idf_path = regenerate_idf(b["osm_id_raw"], b["osm_id_stem"], epw_path, gdf_raw)
            idf_paths.append(idf_path)
        job_id = upload_and_submit(idf_paths, epw_path)
        poll_cluster(job_id, poll_interval_s=30)
        fetch_repaired(REPAIR_BUILDINGS)

    # 3. Bulk-fetch the 1342 main-array successes (excluding the repaired building)
    exclude = {b["osm_id_stem"] for b in REPAIR_BUILDINGS}
    idf_mf_pre = pd.read_parquet(STEP3_DIR / "03_idf_manifest.parquet")
    expected_total = int((idf_mf_pre["generation_status"] == "success").sum())
    bulk_fetch_main(exclude, expected_total)

    # 4. Update IDF manifest, verify zero-fail across all sim_out
    idf_manifest = update_idf_manifest(REPAIR_BUILDINGS)
    verify_all_success(idf_manifest)

    # 5. Build sim manifest (all success)
    sim_mf = build_sim_manifest(idf_manifest, epw_path)
    n_fail = int((sim_mf["status"] != "success").sum())
    if n_fail > 0:
        print(f"[recover] ZERO-FAIL VIOLATION: {n_fail} failed sim rows", file=sys.stderr)
        sys.exit(2)

    # 6. Step 5
    results_gdf, cbecs_gates, summary = run_step5(idf_manifest, sim_mf, epw_path, job_id)

    # 7. Clean cluster out/, move raw to runtime
    clean_cluster_out()
    move_to_runtime()

    eui_map = summary.get("neighbourhood_eui_weighted_kwh_m2", {})
    print(f"\n[recover] COMPLETE.")
    print(f"  Repaired: {[b['osm_id_raw'] for b in REPAIR_BUILDINGS]}  (repair job {job_id})")
    print(f"  Simulation: {len(sim_mf)}/{len(sim_mf)} success")
    print(f"  total_eui_kwh_m2: {eui_map.get('total_eui_kwh_m2')}")
    print(f"  GWP: {summary.get('neighbourhood_gwp_total_kgco2')}")
