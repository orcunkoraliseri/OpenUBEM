"""V12 la_centre: fetch all 225 sim outputs then run Step 5.

Uses the streamed find|tar pipe pattern to avoid Windows 32k cmd limit.
"""
...
def fetch_all_results() -> None:
    print(f"[{CELL_NAME}] Fetching all sim outputs (streamed find|tar) ...")
    ...
    print(f"  extracted: {n_end} .end files")
...
def step5_and_report(idf_manifest: pd.DataFrame, sim_mf: pd.DataFrame,
    ...
    lines = [
        "=" * 72,
        f"V12 {CELL_NAME.upper()} GATES REPORT (225/225 — 2 single_zone repairs)",
        ...
        "",
        "=== SIMULATION STATUS ===",
        f"  cluster_job_ids: {MAIN_JOB_ID} (main, 223/225 tasks) + {REPAIR_JOB_ID} (repair, 2/2 tasks)",
...
def main() -> None:
    print(f"[{CELL_NAME}] Starting fetch+step5")
    print(f"  work_base: {WORK_BASE}")
    print(f"  fleet_dir: {REMOTE_FLEET_DIR}")
    ...
    # Fetch all outputs (streamed)
    fetch_all_results()

    # Get list of stems from successful IDFs
    success_rows = idf_manifest[idf_manifest["generation_status"] == "success"]
    ...
    print(f"  Repair job: {REPAIR_JOB_ID}")
