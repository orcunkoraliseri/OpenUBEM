"""V12 la_urban: regenerate 3 failed buildings with single_zone, resim, run Step 5.

Failures (all perimeter_core geometry bugs):
  relation/6374725 - vertex size mismatch (6 Severe), Fatal
  way/402036180    - non-planar surfaces / CheckConvexity (24 Severe), Fatal
  way/428643335    - 100 degenerate surfaces, no eplusout.end (did not complete)

Fix: single_zone fallback for all 3.
Working dir: %TEMP%/ubem_validation/cases/la_urban/  (raw files from original run)
"""
...
CELL_NAME = "la_urban"
REMOTE_FLEET_DIR = "/speed-scratch/o_iseri/fleets/la_urban"
REPAIR_DIR_REMOTE = REMOTE_FLEET_DIR + "_repair3"
...
REPAIR_BUILDINGS = [
    {"osm_id_raw": "relation/6374725", "osm_id_stem": "relation_6374725"},
    {"osm_id_raw": "way/402036180",    "osm_id_stem": "way_402036180"},
    {"osm_id_raw": "way/428643335",    "osm_id_stem": "way_428643335"},
]
...
def regenerate_idf(osm_id_raw: str, osm_id_stem: str, epw_path: Path,
    ...
    print(f"[repair3] Regenerating {osm_id_raw} ({osm_id_stem}) ...")
...
def upload_and_submit(idf_paths: list[Path], epw_path: Path) -> str:
    print(f"[repair3] Creating remote repair dir {REPAIR_DIR_REMOTE} ...")
    ...
    print(f"  fleet.lst written: {[p.stem for p in idf_paths]}")

    print(f"[repair3] Uploading {len(idf_paths)} IDFs via tar stream ...")
    tar_buf = io.BytesIO()
    with tarfile.open(fileobj=tar_buf, mode="w:gz") as tf:
        for idf_p in idf_paths:
            tf.add(str(idf_p), arcname=f"idfs/{idf_p.name}")
    tar_buf.seek(0)
...
#SBATCH --job-name=openubem_{CELL_NAME}_repair3
#SBATCH --output={REPAIR_DIR_REMOTE}/repair3_%A_%a.log
...
exit $RC
"""
    sbatch_remote_path = f"{REPAIR_DIR_REMOTE}/repair3_array.sbatch"
    with subprocess.Popen(
    ...
    if not job_id:
        ...
        sys.exit(2)
    print(f"  Repair3 array job ID: {job_id}")
    return job_id


def poll_cluster(job_id: str, poll_interval_s: int = 60) -> None:
    print(f"[repair3] Polling job {job_id} (every {poll_interval_s}s) ...")
    while True:
        ...
        if pending == 0:
            ...
            print(sacct_full[:3000])
            break


def fetch_and_verify(buildings: list[dict]) -> None:
    print(f"[repair3] Fetching {len(buildings)} repaired outputs by explicit paths ...")
    for b in buildings:
        stem = b["osm_id_stem"]
        ...
        if "EnergyPlus Completed Successfully" not in end_text:
            ...
            sys.exit(2)
        if n_severe > 0:
            print(f"  WARNING: {stem} has {n_severe} Severe errors — STOP", file=sys.stderr)
            sys.exit(2)
...
def update_idf_manifest(buildings: list[dict]) -> pd.DataFrame:
    ...
    print(f"[repair3] IDF manifest updated: {len(mf)} rows, "
...
def run_step5(idf_manifest: pd.DataFrame, sim_mf: pd.DataFrame,
    ...
    print(f"[repair3] Building enriched GDF ({len(idf_manifest)} buildings) ...")
    ...
    print(f"[repair3] Step 5: aggregate_results ...")
    t0 = time.monotonic()
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
    ...
    print(f"\n[{CELL_NAME}] F12 GATE SUMMARY (614/614):")
    ...
    lines = [
        "=" * 72,
        f"V12 {CELL_NAME.upper()} GATES REPORT (614/614 — 3 single_zone repairs)",
        ...
        "",
        "=== FUNNEL ===",
        f"  V10 Overpass probe count (lower bound): 533",
        ...
        "",
        "=== SIMULATION STATUS ===",
        f"  cluster_job_ids: {MAIN_JOB_ID} (main, 611/614) + {repair_job_id} (repair3, 3/3)",
        f"  sim_manifest_rows: {len(sim_mf)}",
        f"  status_counts: {sim_mf['status'].value_counts().to_dict()}",
        "",
        "=== REPAIR RECORD ===",
        f"  relation/6374725: perimeter_core vertex size mismatch (6 Severe) -> single_zone, job {repair_job_id}_1, 0 Severe",
        f"  way/402036180: perimeter_core non-planar/CheckConvexity (24 Severe) -> single_zone, job {repair_job_id}_2, 0 Severe",
        f"  way/428643335: perimeter_core 100 degenerate surfaces (no .end) -> single_zone, job {repair_job_id}_3, 0 Severe",
        "",
        "=== F12 GATE TABLE ===",
    ...
    print(f"[repair3] Gates report -> {report_path}")

    print(f"[repair3] Copying final deliverables -> {FINAL_DIR}")
...
def verify_614_success() -> None:
    subdirs = [d for d in SIM_OUT_DIR.iterdir() if d.is_dir()]
    n_total = len(subdirs)
    failed = []
    for d in subdirs:
        end_p = d / "eplusout.end"
        if not end_p.exists():
            failed.append((d.name, "missing .end"))
            continue
        txt = end_p.read_text(errors="replace")
        if "EnergyPlus Completed Successfully" not in txt:
            failed.append((d.name, txt.strip()[:80]))
    print(f"[repair3] Final verification: {n_total - len(failed)}/{n_total} success")
    if failed:
        for name, msg in failed:
            print(f"  FAIL: {name}: {msg}", file=sys.stderr)
        sys.exit(2)
    print(f"[repair3] ZERO-FAIL: all {n_total} sim_out dirs have EnergyPlus Completed Successfully")


def clean_repair3_scratch() -> None:
    out = _ssh(f"rm -rf {REPAIR_DIR_REMOTE}/out 2>/dev/null && echo CLEANED || echo FAILED", timeout=60)
    print(f"[repair3] Cluster repair3 out/ clean: {out.strip()}")


def move_to_runtime() -> None:
    runtime_dest = REPO / "runtime" / "ubem_validation" / "cases" / CELL_NAME
    if runtime_dest.exists():
        print(f"[repair3] runtime dest already exists: {runtime_dest}, removing ...")
        shutil.rmtree(str(runtime_dest))
    print(f"[repair3] Moving {WORK_BASE} -> {runtime_dest} ...")
...
def main() -> None:
    print(f"[repair3] V12 la_urban repair3: 3 single_zone repairs")
    print(f"[repair3] Working dir: {WORK_BASE}")
    ...
    print(f"[repair3] EPW: {epw_path.name}")

    gdf_raw = gpd.read_file(str(WORK_BASE / "01_buildings.gpkg"))
    print(f"[repair3] Loaded {len(gdf_raw)} buildings from 01_buildings.gpkg")

    # Step 1: regenerate 3 IDFs with single_zone
    idf_paths = []
    for b in REPAIR_BUILDINGS:
        idf_path = regenerate_idf(b["osm_id_raw"], b["osm_id_stem"], epw_path, gdf_raw)
        idf_paths.append(idf_path)

    # Step 2: upload + submit ONE 3-task array
    job_id = upload_and_submit(idf_paths, epw_path)

    # Step 3: poll synchronously until terminal
    poll_cluster(job_id, poll_interval_s=60)

    # Step 4: fetch each by explicit path, verify
    fetch_and_verify(REPAIR_BUILDINGS)

    # Step 5: confirm all 614 success
    verify_614_success()

    # Step 6: update IDF manifest
    idf_manifest = update_idf_manifest(REPAIR_BUILDINGS)

    # Step 7: build sim manifest (fresh, all 614)
    print(f"\n[repair3] Building sim manifest ...")
    ...
    if n_fail > 0:
        ...
        for _, row in failed_rows.iterrows():
            print(f"  FAIL: osm_id={row['osm_id']}, n_severe={row['n_severe']}")
        print(f"[repair3] ZERO-FAIL VIOLATION: {n_fail} failed rows in sim manifest", file=sys.stderr)
        sys.exit(2)

    print(f"[repair3] Sim manifest: {len(sim_mf)} rows all success")

    # Step 8: run Step 5 aggregation
    print(f"\n[repair3] Running Step 5 ...")
    results_gdf, cbecs_gates, summary = run_step5(idf_manifest, sim_mf, epw_path, job_id)

    # Step 9: clean cluster out/
    clean_repair3_scratch()

    # Step 10: move raw to runtime/
    move_to_runtime()

    eui_map = summary.get("neighbourhood_eui_weighted_kwh_m2", {})
    gwp = summary.get("neighbourhood_gwp_total_kgco2")
    total_eui = eui_map.get("total_eui_kwh_m2")

    print(f"\n[repair3] COMPLETE.")
    print(f"  Repaired: {[b['osm_id_raw'] for b in REPAIR_BUILDINGS]}")
    print(f"  Repair job: {job_id}")
    print(f"  Simulation: 614/614 success")
    print(f"  total_eui_kwh_m2: {total_eui}")
    print(f"  GWP: {gwp}")
    print(f"  CBECS CV(RMSE): {cbecs_gates.get('cbecs_cv_rmse')}")
    print(f"  CBECS NMBE: {cbecs_gates.get('cbecs_nmbe')}")
    print(f"  Deliverables: {FINAL_DIR}")
    print(f"  Raw moved to: {REPO / 'runtime' / 'ubem_validation' / 'cases' / CELL_NAME}")
