"""V12 la_suburban SQL-repair Step5: all 12 corrupt SQLs replaced; re-run Step5.

SQL paths in 04_simulation_manifest.parquet still point to the old temp dir.
This script rebuilds the sim manifest with paths corrected to the runtime dir,
then runs aggregate_results + compute_validation_gates, writes the gates report
and overwrites the deliverables in docs/validations/overAll/results/cases/la_suburban/.
"""
...
EPSG = 32611
MAIN_JOB_ID = "965462"
REPAIR1_JOB_ID = "967407"
REPAIR2_JOB_ID = "967450"
EPW_STATION = "Torrance.Muni.AP-Zamperini.Field.722955"

CORRUPT_STEMS = [
    "way_442622402","way_442622448","way_442624555","way_442624560",
    "way_442624563","way_442624565","way_442624569","way_442624570",
    "way_442624571","way_442624576","way_442624584","way_442624585",
]
...
def rebuild_sim_manifest(idf_mf: pd.DataFrame, epw_path: Path) -> pd.DataFrame:
    """Rebuild 04_simulation_manifest.parquet with runtime-correct sql_paths."""
    print(f"[manifest] Rebuilding sim manifest with runtime paths ...")
    success_rows = idf_mf[idf_mf["generation_status"] == "success"]
    sim_rows = []
    for _, r in success_rows.iterrows():
        oid_raw = str(r["osm_id"])
        oid_stem = Path(str(r["idf_path"])).stem
    ...
    sc = sim_mf["status"].value_counts().to_dict()
    print(f"  Sim manifest rebuilt: {len(sim_mf)} rows, status={sc}")
    return sim_mf
...
def build_enriched_gdf(idf_mf: pd.DataFrame, sim_mf: pd.DataFrame) -> gpd.GeoDataFrame:
    ...
    for _, idf_row in idf_mf.iterrows():
        ...
        footprint_area_m2, num_floors, height_m = 200.0, 1.0, 3.5
...
def run_step5(idf_manifest: pd.DataFrame, sim_mf: pd.DataFrame, epw_path: Path) -> None:
    ...
    by_status = summary.get("n_buildings_by_status")
    ...
    print(f"  n_buildings_by_status: {by_status}")

    lines = [
        "=" * 72,
        f"V12 {CELL_NAME.upper()} GATES REPORT (SQL-repair: 1343/1343 parsed)",
        ...
        "",
        "=== SIMULATION STATUS ===",
        f"  cluster_job_ids: {MAIN_JOB_ID} (main, 1343/1343 COMPLETED 0 Severe) + {REPAIR1_JOB_ID} (repair1 way/442763908 single_zone) + {REPAIR2_JOB_ID} (SQL-repair 12 buildings, 0 Severe)",
        f"  sim_manifest_rows: {len(sim_mf)}",
        f"  status_counts: {sim_mf['status'].value_counts().to_dict()}",
        "",
        "=== REPAIR RECORD ===",
        f"  way/442763908: perimeter_core vertex size mismatch -> single_zone, job {REPAIR1_JOB_ID}, 0 Severe",
        f"  12 buildings had corrupt local SQLite files from earlier truncated-fetch;",
        f"  re-simulated as job {REPAIR2_JOB_ID}, fetched with integrity verification, re-parsed -> 1343/1343.",
        f"  Corrupt stems: {', '.join(CORRUPT_STEMS)}",
        "",
        "=== F12 GATE TABLE ===",
    ...
    lines.append(f"  n_buildings_by_status: {by_status}")
    ...
    mf_src = WORK_BASE / "04_simulation_manifest.parquet"
    if mf_src.exists():
        shutil.copy2(mf_src, FINAL_DIR / "04_simulation_manifest.parquet")
    print(f"  Done.")


def main() -> None:
    print(f"[{CELL_NAME}] SQL-repair Step5: sim_out pre-populated (1343/1343)")
    ...
    if n_fail > 0:
        failed = sim_mf[sim_mf["status"] != "success"]["osm_id"].tolist()
        print(f"ZERO-FAIL VIOLATION: {n_fail} failed: {failed[:10]}", file=sys.stderr)
        sys.exit(2)

    print(f"  All {len(sim_mf)} buildings: status=success. Proceeding to Step 5.")
    run_step5(idf_manifest, sim_mf, epw_path)
