"""T02 (OPEN-55 acceptance test) attempt 4 — instrumented single-cell launcher.
...
This is a copy of open48_t02_attempt3.py. Attempts 1-3 all died at the remote
completeness probe (`_remote_results_complete`), which OPEN-57 has since fixed
(the tcsh `Unmatched '.` failure at fleet sizes above ~565 ids, and separately
the CRLF-corruption of stdin-piped ids under `text=True` on Windows). Nothing
else changes from attempt 3: same frozen source GDF, same instrumentation.
Only NEW_SUBDIR and the run directory move to a fresh, never-scored path.

Instrumentation carried over from attempt 3 because attempts 1 and 2 both died
unexplained:
...
Usage:
    .venv\\Scripts\\python.exe scripts\\validation\\open48_t02_attempt4.py
"""
...
NEW_SUBDIR = "open48_refleet3_t02a4"    # brand-new subdir -> brand-new remote fleet dir
EXPECTED_MD5 = "1198ed01bfd3b4463e50da0ae39d8e27"

RUN_DIR = Path(r"C:\Users\o_iseri\AppData\Local\Temp\open48_t02_attempt4")
...
def main() -> int:
    ...
    _append(LOG_PATH, f"[{t_start}] === T02 attempt 4 launcher starting ===")
