"""V12 la_rural repair: regenerate way/472961100 with single_zone fallback.

Diagnosis: failed_interzone_vertex_mismatch on perimeter_core polygon.
Fix: single_zone fallback (zero interzone pairs, no geomeppy intersect needed).

Working dir: runtime/ubem_validation/cases/la_rural/ (already in runtime).
"""
...
CELL_NAME = "la_rural"
REMOTE_FLEET_DIR = "/speed-scratch/o_iseri/fleets/la_rural"
...
OSM_ID_RAW = "way/472961100"
OSM_ID_STEM = "way_472961100"

LAT, LON = 34.7420, -118.2130
STATE = "CA"
EPSG = 32611
MAIN_JOB_ID = "967463"
EPW_STATION = "Lancaster-Fox.Field.723816"
...
def upload_and_submit(idf_path: Path, epw_path: Path) -> str:
    ...
    fleet_bytes = fleet_content.encode("utf-8")
    idf_path.with_suffix(".fleet_tmp").write_bytes(fleet_bytes)
    subprocess.run(
        ["scp", str(idf_path.with_suffix(".fleet_tmp")),
         f"{REMOTE_HOST}:{fleet_remote}"],
        check=True, timeout=30,
    )
    idf_path.with_suffix(".fleet_tmp").unlink()
    print(f"  fleet.lst written via scp: [{OSM_ID_STEM}]")

    subprocess.run(["scp", str(idf_path), f"{REMOTE_HOST}:{REPAIR_DIR_REMOTE}/idfs/"],
                   check=True, timeout=60)
    print(f"  IDF uploaded: {idf_path.name}")
...
"""
    ...
    sbatch_local = idf_path.parent / "repair1_array.sbatch"
    sbatch_local.write_bytes(sbatch_script.encode("utf-8").replace(b"\r\n", b"\n"))
    subprocess.run(
        ["scp", str(sbatch_local), f"{REMOTE_HOST}:{sbatch_remote_path}"],
        check=True, timeout=30,
    )
    sbatch_local.unlink()
...
def verify_sql_integrity() -> None:
    import sqlite3 as _sqlite3
    sql_path = SIM_OUT_DIR / OSM_ID_STEM / "eplusout.sql"
    assert sql_path.exists(), f"SQL not found: {sql_path}"
    conn = _sqlite3.connect(f"file:{sql_path}?mode=ro", uri=True)
    ic_rows = conn.execute("PRAGMA integrity_check;").fetchall()
    ic = ic_rows[0][0] if ic_rows else ""
    print(f"  integrity_check: {ic}")
    assert ic == "ok", f"SQL integrity FAIL: {ic}"
    zone_count = conn.execute("SELECT COUNT(*) FROM Zones;").fetchone()[0]
    conn.close()
    print(f"  zone_count: {zone_count}")
    assert zone_count == 1, f"Expected 1 zone (single_zone), got {zone_count}"
    print(f"  SQL integrity PASS: ok, zones=1")
...
def rebuild_sim_manifest(idf_mf: pd.DataFrame, epw_path: Path) -> pd.DataFrame:
    """Rebuild 04_simulation_manifest.parquet with runtime paths (all 149 buildings)."""
    print(f"[repair1] Rebuilding sim manifest (all 149 buildings, runtime paths) ...")
    success_rows = idf_mf[idf_mf["generation_status"] == "success"]
    sim_rows = []
    for _, r in success_rows.iterrows():
        oid_raw = str(r["osm_id"])
        idf_p = str(r["idf_path"])
        oid_stem = Path(idf_p).stem
...
def build_enriched_gdf(idf_mf: pd.DataFrame, sim_mf: pd.DataFrame) -> gpd.GeoDataFrame:
    ...
    for _, idf_row in idf_mf.iterrows():
        ...
        if len(sim_row) > 0 and sim_row.iloc[0]["status"] == "success":
            ...
            if sql_p.exists():
                try:
                    conn = _sqlite3.connect(f"file:{sql_p}?mode=ro", uri=True)
                    zones = conn.execute(
...
def run_step5(idf_manifest: pd.DataFrame, sim_mf: pd.DataFrame,
    ...
    print(f"[repair1] Building enriched GDF ({len(idf_manifest)} buildings) ...")
    ...
    print(f"[repair1] Step 5: aggregate_results ...")
    t0 = time.monotonic()
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
    ...
    print(f"\n[{CELL_NAME}] F12 GATE SUMMARY (149/149):")
    ...
    lines = [
        "=" * 72,
        f"V12 LA_RURAL GATES REPORT (149/149 — after repair1 {OSM_ID_RAW})",
        f"  Cell:   {CELL_NAME}  ({LAT}, {LON}) r=1500m",
        f"  EPW:    {EPW_STATION}",
        f"  Date:   2026-06-12",
        "=" * 72,
        "",
        "=== FUNNEL ===",
        f"  V10 Overpass probe count (lower bound): 143",
        f"  Actual OSM fetch:    149",
        ...
        "",
        "=== SIMULATION STATUS ===",
        f"  cluster_job_ids: {MAIN_JOB_ID} (main, 148/149) + {repair_job_id} (repair1, 1/1)",
        f"  sim_manifest_rows: {len(sim_mf)}",
        f"  status_counts: {sim_mf['status'].value_counts().to_dict()}",
        "",
        "=== REPAIR RECORD ===",
        f"  {OSM_ID_RAW}: perimeter_core interzone vertex mismatch -> single_zone, job {repair_job_id}_1, 0 Severe",
        "",
        "=== F12 GATE TABLE ===",
    ...
    if len(iod_vals):
        lines.append(f"  n={len(iod_vals)}, mean={iod_vals.mean():.4f}C, p95={iod_vals.quantile(0.95):.4f}C")
    lines += [
        "",
    ...
    print(f"[repair1] Gates report -> {report_path}")

    print(f"[repair1] Copying final deliverables -> {FINAL_DIR}")
...
def clean_repair1_scratch() -> None:
    out = _ssh(f"rm -rf {REPAIR_DIR_REMOTE}/out 2>/dev/null && echo CLEANED || echo FAILED", timeout=60)
    print(f"[repair1] Cluster repair1 out/ clean: {out.strip()}")


def main() -> None:
    print(f"[repair1] V12 la_rural repair1: {OSM_ID_RAW}")
    print(f"[repair1] Working dir: {WORK_BASE}")
    assert WORK_BASE.exists(), f"Working dir not found: {WORK_BASE}"

    epw_path = WORK_BASE / "weather" / "USA_CA_Lancaster-Fox.Field.723816_TMYx.2011-2025.epw"
    ...
    # Step 2: upload + submit 1-task array (Unix LF fleet.lst via scp)
    job_id = upload_and_submit(idf_path, epw_path)

    # Step 3: poll synchronously to terminal
    poll_cluster(job_id, poll_interval_s=60)

    # Step 4: fetch SQL explicitly by path, verify .end
    fetch_result(job_id)

    # Step 5: verify SQL integrity + zone count
    verify_sql_integrity()

    # Step 6: update IDF manifest (149/149 success)
    idf_manifest = update_idf_manifest(idf_path)

    # Step 7: rebuild sim manifest (all 149, runtime paths)
    sim_mf = rebuild_sim_manifest(idf_manifest, epw_path)
    ...
    if n_fail > 0:
        ...
        print(f"[repair1] ZERO-FAIL VIOLATION: {n_fail} failed rows", file=sys.stderr)
        sys.exit(2)
    print(f"[repair1] Sim manifest: {len(sim_mf)} rows all success")

    # Step 8: re-run Step 5 aggregation, write gates report, copy deliverables
    results_gdf, cbecs_gates, summary = run_step5(idf_manifest, sim_mf, epw_path, job_id)

    # Step 9: clean cluster out/ (keep fleet.lst + idfs/)
    clean_repair1_scratch()
    ...
    print(f"  Exception:  failed_interzone_vertex_mismatch (perimeter_core)")
    print(f"  Fix:        single_zone fallback")
    print(f"  Cluster job: {job_id}")
    print(f"  Generation: {len(idf_manifest)}/{len(idf_manifest)}")
    print(f"  Simulation: {len(sim_mf)}/{len(sim_mf)}")
    print(f"  total_eui_kwh_m2: {total_eui}")
    print(f"  GWP: {gwp}")
    print(f"  SQL: {SIM_OUT_DIR / OSM_ID_STEM / 'eplusout.sql'}")
