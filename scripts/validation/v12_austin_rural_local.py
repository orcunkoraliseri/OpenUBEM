"""V12 austin_rural — Steps 1-3 generation + local EnergyPlus 23.1 simulation (n_jobs=10, approved deviation V-R5-6).
...
This script:
  ...
  8. Moves raw work dir from TEMP to runtime\\ubem_validation\\cases\\austin_rural.

Usage:
  py -3 scripts/validation/v12_austin_rural_local.py
"""
...
CELL_NAME = "austin_rural"
LAT, LON = 30.5788, -98.2700
RADIUS_M = 1000.0
STATE = "TX"
EPSG = 32614
N_JOBS = 10
...
def write_gates_report(idf_mf: pd.DataFrame, sim_mf: pd.DataFrame,
    ...
    epw_name = "Horseshoe.Bay.Resort.AP.720639_TMYx.2011-2025"
    ...
    lines = [
        ...
        "",
        "=== FUNNEL ===",
        f"  V10 Overpass probe count (lower bound): 184",
...
def append_progress_log(headline: str, sim_mf: pd.DataFrame, idf_mf: pd.DataFrame,
    ...
    entry = f"""
#### V12.austin_rural — Matrix cell Austin rural end-to-end — completed 2026-06-15
- Artifacts: `docs\\validations\\overAll\\results\\cases\\austin_rural\\` ({n_final_files} final files: `05_neighbourhood_summary.json`, `05_results.{{csv,geojson,gpkg,schema.json}}`, `figures\\{{eui_choropleth,eui_violin_by_archetype,gwp_stacked_by_archetype}}.png`, `v12_austin_rural_gates_report.txt`, `04_simulation_manifest.parquet`); raw intermediates in `runtime\\ubem_validation\\cases\\austin_rural\\` (`01_buildings.gpkg` {n_fetched} rows, `02a_climate_epw.parquet`, `step3\\03_idf_manifest.parquet` {n_total} rows, `sim_out\\` {n_total} buildings); glue script `scripts/validation/v12_austin_rural_local.py`.
...
- Notes: {headline} CZ 2A, EPSG:{EPSG}, EPW Horseshoe.Bay.Resort.AP.720639_TMYx.2011-2025. Actual OSM fetch: {n_fetched} (V10 lower bound 184). Headline (floor-area-weighted, **{n_total} buildings**): heating {eui_map.get('heating_eui_kwh_m2', 'N/A')} / cooling {eui_map.get('cooling_eui_kwh_m2', 'N/A')} / lighting {eui_map.get('lighting_eui_kwh_m2', 'N/A')} / equipment {eui_map.get('equipment_eui_kwh_m2', 'N/A')} / **total {total_eui} kWh/m²/yr**; neighbourhood GWP {gwp_fmt} kgCO₂e (TX eGRID factor). IOD: {iod_line}. Raw dir moved to `runtime\\ubem_validation\\cases\\austin_rural\\` post-close.
"""
...
def main() -> None:
    print(f"\n{'='*72}")
    print(f"[{CELL_NAME}] austin_rural LOCAL pipeline (Steps 1-3 gen + sim)")
