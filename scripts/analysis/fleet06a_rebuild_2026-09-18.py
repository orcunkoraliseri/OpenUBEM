"""FLEET-06a (techtransfer block-6 plan) -- fleet-wide today's-code rebuild orchestrator.

Produces the fresh, today's-code (PV_INJECTION_ENABLED=True baked in) starting IDFs that
the 7 non-baseline scenario cells build on top of. Reuses
`scripts/analysis/t06_cell_rebuild_2026-09-09.py` UNMODIFIED (same Step 1-3 harness, same
frozen `evidence/open48_refleet/<cell>/01_buildings.gpkg` seed, no live OSM fetch, no
EnergyPlus) -- only the output subdirectory changes, exactly the "same launcher pattern as
t06_fleet_rebuild_2026-09-09.py, all 12 in parallel, never sequential" instructed by
`PLAN_techtransfer-block6-2026-09-18.md`'s FLEET-06a correction. The `baseline` scenario
cell does NOT come from this rebuild -- it is the existing 2026-09-09 T07 IDFs fetched from
Speed, unmodified, fetched separately.

Usage: py fleet06a_rebuild_2026-09-18.py
"""
...
OUTPUT_SUBDIR = "fleet06a_rebuild_2026-09-18"
LOG_DIR = Path(tempfile.gettempdir()) / "fleet06a_rebuild_logs"
...
def _preflight() -> int:
    ...
    if total != 8160:
        print(f"PREFLIGHT WARNING -- frozen total {total} != 8160, proceeding but flag this before use")
    return 0
...
def main() -> int:
    ...
    while procs:
        for cell in list(procs.keys()):
            ...
            if rc is not None:
                ...
                if not first_done_reported:
                    print(f"MEASURED first-cell elapsed: {elapsed:.0f}s ({cell})", flush=True)
                    first_done_reported = True
                del procs[cell]
        if procs:
            time.sleep(5)
    ...
    (base / "fleet06a_rebuild_summary.json").write_text(
    ...
    print(f"Fleet summary written to {base / 'fleet06a_rebuild_summary.json'}")
