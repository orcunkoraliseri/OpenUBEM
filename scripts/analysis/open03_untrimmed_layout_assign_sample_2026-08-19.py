"""OPEN-03/OPEN-18/OPEN-59 -- untrimmed `layout_assign` sample (n=48).

Scales the T18 n=1 result (extra/MEASUREMENT_open-03_vintage-at-head.md) to a
stratified 48-building sample: 4 buildings per cell x 12 cells, drawn from run 4
(open48_refleet4) at the 10th/35th/65th/90th footprint_area_m2 percentile
positions. Rebuilds each in layout_assign with trim_outputs=False, simulates
locally in an isolated run_dir, parses with the production parse_building(),
and joins to the run-4 auto results on osm_id.

Structure copied verbatim from
scripts/analysis/open03_t18_trim_hypothesis_check_2026-08-19.py:36-88
(PLAN_layout-assign-untrimmed-2026-08-19.md T01-T03, section 4).
"""
...
OUT_DIR = REPO / "scratchpad" / "open03-untrimmed-sample"
EUI_CSV = REPO / "openubem" / "outputs" / "comparisons" / "open03_untrimmed_sample_eui.csv"
JOIN_CSV = REPO / "openubem" / "outputs" / "comparisons" / "open03_untrimmed_sample_join.csv"

PERCENTILES = [10, 35, 65, 90]


def select_sample(cell: str, cfg: dict) -> pd.DataFrame:
    results_csv = FIXTURE_ROOT / cell / "results" / "05_results.csv"
    df = pd.read_csv(results_csv, dtype={"osm_id": str})
    df = df[df["simulation_status"] == "success"].copy()
    df = df.sort_values("footprint_area_m2", kind="mergesort").reset_index(drop=True)
    n = len(df)
    rows = []
    for p in PERCENTILES:
        ...
        rows.append({
            ...
            "auto_total_eui_kwh_m2": r["total_eui_kwh_m2"],
            "auto_floor_area_m2": r["floor_area_m2"],
            "percentile_slot": p,
        })
...
def main():
    ...
    import os
    cells = sorted(CELL_CONFIGS.keys())
    smoke_cells = os.environ.get("OPEN03_SMOKE_CELLS")
    if smoke_cells:
        cells = [c for c in cells if c in set(smoke_cells.split(","))]
    ...
    for cell in cells:
        ...
        if not gpkg.exists():
            raise FileNotFoundError(f"missing fixture: {gpkg}")
        sample_frames.append(select_sample(cell, CELL_CONFIGS[cell]))
