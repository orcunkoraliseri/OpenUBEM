"""D-EU-27 T02 -- the certified re-run: all 510 spec cells, 3 independent replicates each.

Reuses run_campaign_cell (openubem/campaign/eu_cell_runner.py) and the eplusout.err
screening from scripts/diagnostics/f181_t03_err_screen.py verbatim. Each replicate gets
its own run directory. dry_run=False.
...
RUN_ROOT_BASE = ROOT / "openubem/outputs/eu_certified_rerun_2026-08-28"
OUT_DIR = ROOT / "docs/docs_ACTIVE/europeanLocations/outputs"
CELLS_CSV = OUT_DIR / "deu27_rerun_cells.csv"
...
CELLS_FIELDS = [
    "cell_id", "archetype_id", "survey_fold", "sensitivity_f", "replicate", "class",
...
def _run_one(job: tuple[dict, int]) -> dict:
    cell, replicate = job
    run_root = RUN_ROOT_BASE / f"rep{replicate}"
...
def _worker_entry(job: tuple[dict, int]) -> dict:
    return _run_one(job)
...
def main() -> None:
    ...
    jobs = [(cell, rep) for cell in cells for rep in (1, 2, 3)]
    print(f"loaded {len(cells)} cells -> {len(jobs)} jobs (3 replicates each)", flush=True)

    rows: list[dict] = []
    t0 = time.time()
    with ProcessPoolExecutor(max_workers=14) as pool:
        for i, row in enumerate(pool.map(_worker_entry, jobs), start=1):
            rows.append(row)
            if i % 50 == 0:
                write_cells(rows)
                print(f"  {i}/{len(jobs)} done in {time.time() - t0:.1f}s", flush=True)
